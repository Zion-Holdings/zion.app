...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { error: insertError } = await supabase.from('points_ledger').insert({'      user_id: userId,
      delta: amount,
      reason: reason ?? null,
      order_id: null
    });
    if (insertError) {
      console.error('points_ledger insert error:', insertError);      return res.status(500).json({ error: Failed to log points' });    }

    const { data: profile, error: fetchError } = await supabase
      .from('profiles')      .select('points')      .eq('id', userId)      .single();
    if (fetchError) {
      console.error('fetch profile error:', fetchError);      return res.status(500).json({ error: Failed to fetch user' });    }

    const current = profile?.points ?? 0;
    const { error: updateError } = await supabase
      .from('profiles')      .update({ points: current + amount })
      .eq('id', userId);    if (updateError) {
      console.error('update points error:', updateError);      return res.status(500).json({ error: Failed to update points' });    }
