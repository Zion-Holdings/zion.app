...
class CursorAutomatedPopup {
    ...
    setupEventListeners() {
        // Automation toggle
        document.getElementById('automationToggle').addEventListener('click', () => {
            this.toggleAutomation();
        });

        // File monitoring toggle
        document.getElementById('fileMonitoringToggle').addEventListener('click', () => {
    ...
    updateUI() {
        ...
        document.getElementById('connectionStatus').textContent = this.status.connectionStatus;
        ...
        // Update stats
        document.getElementById('improvementsCount').textContent = this.status.improvements;
    ...
    async updateImprovementsList() {
        try {
            const response = await chrome.runtime.sendMessage({ type: 'GET_IMPROVEMENTS' });
            ...
            if (recentImprovements.length === 0) {
                improvementsList.innerHTML = '<div class="improvement-item">No improvements yet</div>';
                return;
            }
    ...
    async toggleAutomation() {
        try {
            const response = await chrome.runtime.sendMessage({ type: 'TOGGLE_AUTOMATION' });
