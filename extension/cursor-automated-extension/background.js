...
class CursorAutomatedBackground {
    ...
    async sendHeartbeat() {
        ...
        try {
            ...
            await fetch(`http://localhost:${this.config.coordinationPort}/heartbeat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(heartbeat)
            });
    ...
    async syncWithOtherComputers() {
        ...
        try {
            ...
            // Send to coordination server
            const response = await fetch(`http://localhost:${this.config.coordinationPort}/sync`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    monitorFileChanges() {
        ...
        if (Math.random() > 0.8) { // 20% chance of file change
            const randomFile = simulatedChanges[Math.floor(Math.random() * simulatedChanges.length)];
