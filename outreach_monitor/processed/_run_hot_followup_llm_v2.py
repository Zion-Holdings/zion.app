#!/usr/bin/env python3
"""Cron job: hot-follow-up reply check - using label ID directly"""
...
# Label IDs to check
LABEL_IDS = [
    'Label_946',   # !!!hot-follow-up
    'Label_4207916705207178948',  # !!!!HOT FOLLOW-UP
]
...
# System/bot email domains to exclude
...
def is_system_email(email):
    """Check if email is a system/bot notification."""
    email_lower = email.lower() if email else ''
    for domain in SYSTEM_EMAIL_DOMAINS:
        if domain in email_lower:
            return True
    # Also exclude emails from the same domain as the CEO's email
    if '@ziontechgroup.com' in email_lower:
        return True
    return False

# Search all folders using label IDs
results = []
for label_id in LABEL_IDS:
    import urllib.request
    url = f'https://gmail.googleapis.com/gmail/v1/users/me/messages?labelId={label_id}&maxResults=50'
    req = urllib.request.Request(url, headers=gog_headers())
    resp = json.loads(urllib.request.urlopen(req).read())
    msgs = resp.get('messages', [])
    results.extend(msgs)
    print(f"Label {label_id}: {len(msgs)} messages")
...
print(f"Total unique messages across all labels: {len(results)}")
...
for tid, msgs in threads.items():
    ...
    # Skip system/bot emails
    if is_system_email(reply_to):
        print(f"  Thread {tid} ({subject}): system/bot email {reply_to}, skipping")
        duplicates_skipped += 1
        continue
