...
serve(async (req) => {
  ...
  try {
    ...
    - Project Scope: "${scope}""    - Project Timeline: ${startDate} to ${endDate}
    ...
    // Parse the AI-generated content to ensure it's valid JSON'    try {
      ...
      console.error('Failed to parse AI response as JSON:', parseError);'      throw new Error('Failed to parse AI response');'    }
  } catch {
    console.('Error generating milestones:', );'    return new Response(
