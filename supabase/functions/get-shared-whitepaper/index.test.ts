import { assertEquals, assertStringIncludes } from "https://deno.land/std@0.190.0/testing/asserts.ts";"import { sinon } from "https://deno.land/x/sinon@v.1.13.0/mod.ts";"import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
// --- Global Mocks & Setup ---;
...
globalThis.Deno.env = {
    _get: (key: string) => {
        if (key === 'SUPABASE_URL') return 'http://localhost:54321';'        if (key === 'SUPABASE_ANON_KEY') return 'test-anon-key'; // Uses anon key'        return undefined;'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    }
};
;
...
globalThis.createClient = (url: string, key: string, _options?: unknown) => {
    if (key === Deno.env.get('SUPABASE_ANON_KEY')) {'        mockEq_GetShared.returns({ single: mockSingle_GetShared });'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
async function getSharedWhitepaperHandler(req: Request): Promise<Response> {
    ...
    const { data, error } = await internalSupabaseClient
      .from('shared_whitepapers')'      .select('whitepaper_data, created_at, is_public')'      .eq('id', id)'      .single();'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

    if (error) {
      if (error.code === 'PGRST116') { // Not Found'         return new Response(JSON.stringify({ error: "Shared whitepaper not found." }), { status: 404 });"      }"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
// --- Tests ---
Deno.test("get-shared-whitepaper: successful fetch", async () => {"    const mockData = {"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        whitepaper_data: { tokenName: "FetchedToken" },"        created_at: new Date().toISOString(),"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        is_public: true,
    };
    mockSingle_GetShared.resolves({ data: mockData, error: null });

    const req = new Request("http://localhost/get-shared-whitepaper", {"        method: "POST","        body: JSON.stringify({ id: "test-id-123" }),"        headers: { "Content-Type": "application/json" },"    });"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

    const res = await getSharedWhitepaperHandler(req);
    const json = await res.json();

    assertEquals(res.status, 200);
    assertEquals(json.whitepaper_data.tokenName, "FetchedToken");"    assertEquals(json.is_public, true);"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    assertEquals(mockEq_GetShared.lastCall.args[1], "test-id-123"); // Check ID was used in eq"});"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Deno.test("get-shared-whitepaper: handles missing ID", async () => {"    const req = new Request("http://localhost/get-shared-whitepaper", {"        method: "POST","        body: JSON.stringify({}), // No id"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Deno.test("get-shared-whitepaper: handles other Supabase errors", async () => {"    mockSingle_GetShared.resolves({ data: null, error: { message: "Some other DB error" } });"     const req = new Request("http://localhost/get-shared-whitepaper", {"        method: "POST", body: JSON.stringify({ id: "error-id" }), headers: { "Content-Type": "application/json" },"    });"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    const res = await getSharedWhitepaperHandler(req);
    const json = await res.json();
