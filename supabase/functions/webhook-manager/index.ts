  ...
  try {
    ...
    if (req.method === 'POST') {
      if (path === 'create') {
        ...
        const { _webhookId } = await req.json();
...
async function createWebhook(userId: string, name: string, url: string, eventTypes: string[], secret?: string) {
  try {
    ...
    console.('Error in createWebhook:', );
    return new Response(JSON.stringify({ : 'Internal server ' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
...
async function getUserWebhooks(userId: string) {
  try {
    ...
    console.('Error in getUserWebhooks:', );
    return new Response(JSON.stringify({ : 'Internal server ' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
...
async function toggleWebhook(userId: string, webhookId: string, isActive: boolean) {
  try {
    ...
    console.('Error in toggleWebhook:', );
    return new Response(JSON.stringify({ : 'Internal server ' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
...
async function deleteWebhook(userId: string, webhookId: string) {
  try {
    ...
    console.('Error in deleteWebhook:', );
    return new Response(JSON.stringify({ : 'Internal server ' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
