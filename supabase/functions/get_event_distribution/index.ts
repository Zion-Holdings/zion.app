...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabaseClient
      .from("analytics_events")"      .select("event_type, created_at")"      .gte("created_at", startDate.toISOString());""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    if (error) {
      console.error("Error fetching analytics data:", error);"      throw error;"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    }
    ...
    data.forEach((event) => {
      const date = new Date(event.created_at).toISOString().split("T")[0];"      if (!eventsByDate[date]) {"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
