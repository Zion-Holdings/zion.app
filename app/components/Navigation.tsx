  ...
  return (
    <header id="site-navigation" className={`sticky top-0 z-50 w-full border-b transition-all ${scrolled ? 'border-slate-800 bg-slate-950/95 backdrop-blur-2xl shadow-lg shadow-black/20' : 'border-slate-800/60 bg-slate-950/80 backdrop-blur-xl'}`}>
      <nav className="container-page flex h-16 items-center justify-between gap-4" aria-label="Main navigation">
        ...
        <button
          className="lg:hidden p-2 rounded-lg text-slate-300 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-500 touch-manipulation active:scale-95"
      ...
      {mobileOpen && (
        <div ref={mobilePanelRef} className="lg:hidden border-t border-slate-800 bg-slate-950/98 backdrop-blur-2xl px-4 py-5 space-y-1 max-h-[85vh] overflow-y-auto overscroll-contain -mx-4 px-4">
