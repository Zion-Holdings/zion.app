import type { Metadata } from "next";

export const metadata = {
  title: "AI Factory Intelligence for Manufacturing | Zion Tech Group",
  description:
    "Improve throughput and quality with AI factory intelligence, predictive control, and maintenance.",

};

export default function Page() {
  return (
    <main style={{ minHeight: "100vh", background: "#020617", color: "#e2e8f0", padding: "2rem" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        <h1 style={{ fontSize: "2.2rem", fontWeight: 700, marginBottom: "1rem" }}>AI Factory Intelligence</h1>
        <p style={{ color: "#cbd5e1", marginBottom: "0.75rem" }}>
          Increase manufacturing reliability with AI-driven quality and maintenance control.
        </p>
        <ul style={{ paddingLeft: "1.25rem", marginBottom: "1rem", display: "grid", gap: "0.4rem" }}>
          <li>Defect prediction and root-cause analysis</li>
          <li>Process parameter optimization</li>
          <li>Predictive maintenance</li>
          <li>Production scheduling assistance</li>
        </ul>
        <p style={{ marginBottom: "1rem" }}>
          <a href="https://ziontechgroup.com" style={{ color: "#a78bfa", textDecoration: "underline" }}>Visit ziontechgroup.com</a>
        </p>
        <p>
          <a href="https://calendly.com/kleber-ziontechgroup" style={{ color: "#34d399", textDecoration: "underline" }}>Review factory intelligence AI</a>
        </p>
      </div>
    </main>
  );
}
