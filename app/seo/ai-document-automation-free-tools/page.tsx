import type { Metadata } from "next";

export const metadata = {
  title: "AI Document Automation Free Tools | Zion Tech Group",
  description:
    "Free AI document automation tools and resources to help teams move from manual paperwork to structured workflows.",

};

export default function Page() {
  return (
    <main style={{ minHeight: "100vh", background: "#020617", color: "#e2e8f0", padding: "2rem" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        <h1 style={{ fontSize: "2.2rem", fontWeight: 700, marginBottom: "1rem" }}>AI Document Automation Free Tools</h1>
        <p style={{ color: "#cbd5e1", marginBottom: "0.75rem" }}>
          Evaluate document automation with free tools, templates, and guided workflows.
        </p>
        <ul style={{ paddingLeft: "1.25rem", marginBottom: "1rem", display: "grid", gap: "0.4rem" }}>
          <li>Extraction and classification templates</li>
          <li>Validation and workflow starter kits</li>
          <li>Integration checklists</li>
          <li>ROI calculator</li>
        </ul>
        <p style={{ marginBottom: "1rem" }}>
          <a href="https://ziontechgroup.com" style={{ color: "#a78bfa", textDecoration: "underline" }}>Visit ziontechgroup.com</a>
        </p>
        <p>
          <a href="https://calendly.com/kleber-ziontechgroup" style={{ color: "#34d399", textDecoration: "underline" }}>Schedule demo</a>
        </p>
      </div>
    </main>
  );
}
