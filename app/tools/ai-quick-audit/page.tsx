...
export default function AIQuickAudit() {
  ...
  const getMaturity = (score: number) => {
    if (score >= 85) return { label: 'Ready to Scale', color: 'text-emerald-400', desc: 'You have strong foundations; move to managed expansion.' };
    if (score >= 70) return { label: 'Advancing', color: 'text-blue-400', desc: 'Core building blocks exist; tighten execution and measurement.' };
    if (score >= 55) return { label: 'Developing', color: 'text-amber-400', desc: 'Good progress; focus on data and ops reliability.' };
    if (score >= 35) return { label: 'Exploring', color: 'text-orange-400', desc: 'Early stage; start with one high-value use case.' };
    return { label: 'Nascent', color: 'text-red-400', desc: 'No formal AI initiative yet; build strategy and data basics first.' };
  };
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 px-4 py-12">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-8 rounded-2xl bg-slate-900/60 p-8 text-center ring-1 ring-slate-800">
            <div className="text-6xl font-bold" style={{ color: maturity.color.replace('text-', '') }}>
              {pct}%
            </div>
            <div className="mt-2 text-2xl font-semibold" style={{ color: maturity.color.replace('text-', '') }}>
              {maturity.label}
            </div>
              ...
              {catResults.map(cat => {
                ...
                return (
                  <div key={cat.category} className="rounded-xl border border-slate-800 bg-slate-900/50 p-5 text-left">
                    ...
                    <div className="mt-2 text-3xl font-bold" style={{ color: cm.color.replace('text-', '') }}>
                      {cat.score}%
                    </div>
                    <div className="text-xs" style={{ color: cm.color.replace('text-', '') }}>{cm.label}</div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-8 rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="text-slate-400">{Object.keys(answers).length} / {questions.length}</div>
          </div>
                    ...
                    <div key={q.id} className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          { value: 1, label: 'Somewhat', color: 'orange' },
                          { value: 2, label: 'Mostly', color: 'amber' },
                          { value: 3, label: 'Yes', color: 'blue' },
        ...
        <div className="mt-8 rounded-xl border border-slate-800 bg-slate-900/50 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Want a human review? Contact us at <Link href="/contact" className="underline">/contact</Link> or email <Link href="mailto:kleber@ziontechgroup.com" className="underline">kleber@ziontechgroup.com</Link>. Calendly: <Link href="https://calendly.com/kleber-ziontechgroup" className="underline">Book a call</Link>.
          </p>
        </div>
      </div>
    </div>
