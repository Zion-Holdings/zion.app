...
# Enhanced version with better conflict resolution and error handling
set -e
...
# Function to resolve conflicts in a file with improved logic
resolve_conflicts() {
    local file="$1"
    local branch="$2"
    ...
    if grep -q "<<<<<<< HEAD" "$file"; then
        ...
        if [[ "$file" == "package.json" || "$file" == "package-lock.json" ]]; then
            ...
            sed -i '/<<<<<<< HEAD/,/=======/d' "$file"
            sed -i '/>>>>>>> /d' "$file"
        elif [[ "$file" == "next.config.js" || "$file" == "tsconfig.json" || "$file" == "vite.config.ts" || "$file" == "tailwind.config.ts" ]]; then
            echo "⚙️  Config file detected, keeping main version..."
            sed -i '/<<<<<<< HEAD/,/=======/d' "$file"
            sed -i '/>>>>>>> /d' "$file"
        elif [[ "$file" == ".gitignore" || "$file" == "README.md" ]]; then
            echo "📝 Documentation file, merging both versions..."
...
process_batch() {
    ...
    echo "🚀 Processing batch $batch_num (branches $((batch_start + 1)) to $batch_end)"
    echo "---"
    ...
    echo "✅ Batch $batch_num completed: $batch_success successful, $batch_failures failed"
    echo "---"
...
for ((i=0; i<total_branches; i+=BATCH_SIZE)); do
    ...
    if process_batch $i $batch_end $current_batch; then
        echo "✅ Batch $current_batch completed successfully"
    else
        echo "⚠️  Batch $current_batch completed with some failures"
    fi
...
Batches processed: $current_batch
Retry attempts: $RETRY_ATTEMPTS
Backup branch: $BACKUP_BRANCH
EOF

echo "📄 Summary saved to merge-summary-$(date +%Y%m%d-%H%M%S).txt"