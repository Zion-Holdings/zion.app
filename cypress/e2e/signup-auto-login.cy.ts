...
describe('Signup and Auto-Login Flow', () => {'  it('should register a new user, auto-login, and redirect to marketplace', () => {'    const testTimestamp = Date.now();
    const uniqueEmail = `testuser_${testTimestamp}@example.com`;
    const userName = 'Test User';'    const password = 'Password123!'; // Ensure this meets password strength requirements'
    const mockSupabaseUserID = `test-user-id-${testTimestamp}`;

    // Mock the API response for registration
    cy.intercept('POST', '/api/auth/register', (req) => {'      expect(req.body.email).to.equal(uniqueEmail);
      ...
      req.reply({
        ...
        body: {
          user: {
            ...
            aud: 'authenticated','            role: 'authenticated','            email: uniqueEmail,
    ...
    cy.intercept('GET', '/api/users/me', {'      statusCode: 200,
      body: {
        id: mockSupabaseUserID,
        email: uniqueEmail,
        name: userName,
    ...
    cy.visit('/signup');'
    // Fill out the form
    cy.get('[data-testid="display-name-input"]').type(userName);'    cy.get('[data-testid="email-input"]').type(uniqueEmail);'    cy.get('[data-testid="password-input"]').type(password);'    cy.get('[data-testid="confirm-password-input"]').type(password);'
    cy.contains('I agree to the')'      .closest('div.space-y-1')'      .prev()
    ...
    cy.wait('@registerRequest');'    cy.url().should('include', '/marketplace');'
    cy.getCookie('authToken').should('exist');'
    cy.request({
      method: 'GET','      url: '/api/users/me','    }).then((response) => {
