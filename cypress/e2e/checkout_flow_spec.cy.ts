...
describe('Checkout Flow with Mocked Stripe', () => {
  it('should successfully complete the checkout process with mocked Stripe API calls', () => {
    ...
    cy.intercept('POST', 'https://api.stripe.com/v1/tokens', (req) => {
      req.reply(mockStripeTokenCreation());
    }).as('createStripeToken');

    // Intercept Stripe API call for payment confirmation
    // Using a wildcard for the payment intent ID part of the URL
    cy.intercept('POST', 'https://api.stripe.com/v1/payment_intents/*', (req) => {
      req.reply(mockStripePaymentConfirmation());
    ...
    cy.wait('@createStripeToken');
    cy.wait('@confirmStripePayment');
