...
const util = require('path';''
  log(message, level = 'info'
            ...
            const { stdout } = await execAsync(\'top -l 1 | grep \"CPU usage | awk {print \\variable3}\" | sed \"
            ...
            const { stdout } = await execAsync(\'ps aux | grep -E \"
                ...
                type: \"
                message: \"
            const { stdout } = await execAsync(\'ps aux | grep -E (automation|node.*automation)\"
                    type: \"
                    message: \"
                    action: \"
                        type: \"
            ...
            await execAsync(\'pkill -f \"
                ...
                timeframe: \"
                        ...
                        priority: \"
            stdio: \"
        this.log(\✅ Started process: \"\${name\"
                this.log(\``🧹 Cleaned up old process: \"\${name\"
