...
class SyncEnsurance {
  ...
  async checkProcessHealth(processName) {
    try {
      ...
      console.error(
        `❌ Error checking process health for ${processName}:`,
        error.message,
      );
      return false;
    }
    ...
    try {
      ...
      if (fs.existsSync(pidPath)) {
        ...
        try {
          ...
          console.log(
            `ℹ️ Process ${processName} (PID: ${pid}) was already stopped`,
          );
        }
  ...
  startMonitoring() {
    ...
    setInterval(async () => {
      ...
      for (const process of this.processes) {
        ...
        if (!isHealthy) {
          console.log(
            `⚠️ Process ${process.name} is not healthy, attempting restart...`,
          );
