...
class $1 {
  ...
  async initialize() {
    try {
      ...
      // Load the factory
      const $1 = require('./performance-automation-factory.js');
  ...
  async checkAgentHealth() {
    try {
      ...
      const $1 = path.join(this.logsDir, 'performance-lo'g's', "health-${timestamp}.json");
  ...
  async generateSystemReport() {
    try {
      ...
      // Save system report
      const $1 = new Date().toISOString().replace(/[:.]/g, '-');
      const $1 = path.join(this.reportsDir, 'performance-repor't's', "system-${timestamp}.json");
...
// Main execution
async function main() {
  const $1 = new PerformanceAutomationLauncher();
