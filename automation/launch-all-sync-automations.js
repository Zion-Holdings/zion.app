...
class SyncAutomationLauncher {
  ...
  async initialize() {
    try {
      ...
      console.error('❌ Error initializing Sync Automation Launcher: ', error.message);
      this.status = 'error';
      throw error;
    }
  }
    ...
    try {
      ...
      if (fs.existsSync(pidPath)) {
        ...
        try {
          ...
          console.log(`ℹ️ Process ${scriptName} (PID: ${pid}) was already stopped`);
        }
  ...
  handleProcessExit(scriptName, code, signal) {
    console.log(`ℹ️ Process ${scriptName} exited with code ${code} and signal ${signal}`);
  ...
  async checkAllProcesses(scriptName) {
    try {
      ...
      try {
        ...
        console.log(`❌ Process ${scriptName} (PID: ${pid}) is not running, restarting...`);
        const script = this.syncScripts.find(s => s.name === scriptName);
        if (script) {
          await this.restartProcess(script);
        }
  ...
  async handleError(operation, error) {
    ...
    const errorPath = path.join(__dirname, 'logs', `launcher-error-${Date.now()}.json`);
    fs.writeFileSync(errorPath, JSON.stringify(errorLog, null, 2));
  }

  generateReport() {
    const statusPath = path.join(__dirname, 'launcher-status', 'current-status.json');
...
launcher.initialize().then(() => {
  ...
  console.error('❌ Sync Automation Launcher initialization failed: ', error.message);
  process.exit(1);
});
