...
const LOG = path.join(__dirname, 'logs', 'linkedin-marketing-orchestrator.log');
...
function cycle() {
  run('node', [path.join(__dirname, 'linkedin-marketing-analyzer.cjs')]);
  run('node', [path.join(__dirname, 'linkedin-marketing-factory.cjs')]);
  const poster = path.join(__dirname, 'linkedin-agents', 'post-latest.cjs');
  if (fs.existsSync(poster)) run('node', [poster]);
}

function start(mode = 'continuous') {
  log('🚀 LinkedIn marketing orchestrator started');
  if (mode === 'continuous') {
