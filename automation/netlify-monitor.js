...
class NetlifyBuildMonitor {
  ...
  async getSiteBuilds() {
    try {
      const builds = await this.makeNetlifyRequest(`/sites/${this.config.netlifySiteId}/builds?per_page=10`);
      return builds;
    } catch (error) {
  ...
  async getBuildDetails(buildId) {
    try {
      const build = await this.makeNetlifyRequest(`/sites/${this.config.netlifySiteId}/builds/${buildId}`);
      return build;
    } catch (error) {
  ...
  async triggerBuild() {
    try {
      ...
      const build = await this.makeNetlifyRequest(`/sites/${this.config.netlifySiteId}/builds`, 'POST', {
        clear_cache: true
      });
...
`;
  ...
  async fixGenericError(error) {
    ...
    try {
      ...
      execSync('git commit -m "Auto-fix: Apply build error fixes"', { stdio: 'inherit' });
  ...
  async commitAndPushFixes() {
    try {
      ...
      execSync('git add .', { stdio: 'inherit' });
      execSync('git commit -m "Auto-fix: Apply Netlify build fixes"', { stdio: 'inherit' });
      execSync('git push', { stdio: 'inherit' });
  ...
  async checkBuilds() {
    ...
    if (this.status.currentBuild && this.status.currentBuild.id === latestBuild.id) {
    ...
    if (latestBuild.state === 'error') {
      ...
      const successfulFixes = this.status.fixes.filter(fix => 
        fix.buildId === latestBuild.id && fix.success
      );
  ...
  async generateReport() {
    const report = {
      ...
      summary: {
        ...
        failedBuilds: this.status.buildHistory.filter(b => b.state === 'error').length,
        successfulFixes: this.status.fixes.filter(f => f.success).length,
        totalFixes: this.status.fixes.length
      }
    };

    fs.writeFileSync(path.join(__dirname, 'netlify-report.json'), JSON.stringify(report, null, 2));
    return report;
  }
