...
# Comprehensive Automation System Startup Script with System Node.js
# This script starts all automation components using the system Node.js version

set -e

echo "🚀 Starting Comprehensive Automation System with System Node.js..."
...
print_status "Using Node.js: $($SYSTEM_NODE --version)"

# Check if Node.js is available
if ! command -v $SYSTEM_NODE &> /dev/null; then
...
start_process() {
    ...
    if [ -f "$script_name" ]; then
        nohup $SYSTEM_NODE "$script_name" > "logs/$log_file" 2>&1 &
        local pid=$!
