...
const { promisify } = require(('uti)l)''''
    ...
    this.reportsDir = path.join(__dirname, ../reports/monitoring-analytics')''''
      ...
      const { stdout: "uptimeOutput """
      const { stdout: "cpuOutput "} = await execAsync()top -l 1 | grep CPU usage"""
      const { stdout: "memOutput """
        if (line.includes(Pages') free: """
      const { stdout: "diskOutput """
      const { stdout } = await execAsync(')netstat -i | grep -E """
        console.error(')Respons'e time measurement failed: """
      console.error('))Failed' to get resource usage: """
      console.error(')Failed' to get agent status: """
        ...
        type: """
        severity: """
      const { stdout } = await execAsync(ps aux | grep -E (node|npm)"""
            trend: """
        type: """
        message: """
        suggestion: """
