...
class SyntaxFixLauncher {
  ...
  start(component = "all") {
    ...
    switch (component) {
      ...
      default:
        console.log(
          "❌ Invalid component. Use: monitor, orchestrator, continuous, or all",
        );
        return false;
    }
  }
  ...
  startMonitor() {
    ...
    try {
      ...
      if (fs.existsSync(monitorPath)) {
        this.processes.monitor = execSync("node " + monitorPath, {
          stdio: "inherit",
        });
        return true;
      } else {
  ...
  startOrchestrator() {
    ...
    try {
      const orchestratorPath = path.join(
        __dirname,
        "syntax-fix-orchestrator.js",
      );
      if (fs.existsSync(orchestratorPath)) {
        this.processes.orchestrator = execSync("node " + orchestratorPath, {
          stdio: "inherit",
        });
        return true;
      } else {
  ...
  startContinuous() {
    ...
    try {
      ...
      if (fs.existsSync(continuousPath)) {
        this.processes.continuous = execSync("node " + continuousPath, {
          stdio: "inherit",
        });
        return true;
      } else {
  ...
  status() {
    ...
    Object.keys(this.processes).forEach((key) => {
      ...
      console.log(
        `${isRunning ? "🟢" : "🔴"} ${key}: ${isRunning ? "Running" : "Stopped"}`,
      );
    });
  }
...
switch (command) {
  ...
  default:
    console.log(
      "❌ Invalid command. Use: start, stop, status, quick-fix, build-check, or help",
    );
    launcher.showHelp();
}
