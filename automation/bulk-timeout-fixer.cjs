...
class BulkTimeoutFixer {
  ...
  async fixAllMissingTimeouts() {
    console.log("🔧 Bulk fixing missing timeouts in workflows...");
  ...
  generateReport() {
    ...
    const reportPath = path.join(this.reportsDir, 'bulk-timeout-fix-report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
    
    console.log(`\n📊 Bulk timeout fix report saved to: ${reportPath}`);
...
async function main() {
  try {
    const fixer = new BulkTimeoutFixer();
    const results = await fixer.fixAllMissingTimeouts();
    
    console.log("\n🎉 Bulk timeout fixing completed!");
    console.log(`Fixed: ${results.fixed.length}`);
    console.log(`Failed: ${results.failed.length}`);
    
  } catch (error) {
    console.error("❌ Error running bulk timeout fixer:", error);
    process.exit(1);
  }
}
...
module.exports = BulkTimeoutFixer;