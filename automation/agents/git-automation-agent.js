...
class Git-automationAutomationAgent {
  ...
  ensureDirectories() {
    const dirs = [
      ...
      path.join(this.reportsDir, 'branch-reports'),
      path.join(this.reportsDir, 'merge-reports'),
  ...
  async checkGitStatus() {
    try {
      ...
      try {
        ...
        console.error('Failed to get commits behind:', error);
      }
  ...
  async saveGitStatusReport(status) {
    ...
    const reportPath = path.join(this.reportsDir, 'commit-reports', `status-${timestamp}.json`);
  ...
  async saveCommitReport(changes, commitMessage) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const reportPath = path.join(this.reportsDir, 'commit-reports', `commit-${timestamp}.json`);
  ...
  async saveBranchReport(report) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const reportPath = path.join(this.reportsDir, 'branch-reports', `branch-${timestamp}.json`);
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  ...
  async saveSyncReport(report) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const reportPath = path.join(this.reportsDir, 'sync-reports', `sync-${timestamp}.json`);
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  ...
  async saveMergeReport(branchName) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const reportPath = path.join(this.reportsDir, 'merge-reports', `merge-${timestamp}.json`);
...
agent.start().catch(error => {
  console.error('Git Automation Agent failed to start:', error);
  process.exit(1);
}); 