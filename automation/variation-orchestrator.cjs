...
const LOG = path.join(__dirname, 'logs', 'variation-orchestrator.log');
...
function cycle() {
  run('node', [path.join(__dirname, 'variation-analyzer.cjs')]);
  run('node', [path.join(__dirname, 'variation-factory.cjs')]);
}

function start(mode = 'continuous') {
  log('🚀 Variation orchestrator started');
  if (mode === 'continuous') {
    const loop = () => { cycle(); setTimeout(loop, 20 * 60 * 1000); };
    loop();
  } else {
    cycle();
  }
