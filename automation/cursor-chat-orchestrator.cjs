...
const LOG = path.join(__dirname, 'logs', 'cursor-chat-orchestrator.log');
...
function cycle() {
  run('node', [path.join(__dirname, 'cursor-chat-analyzer.cjs')]);
  run('node', [path.join(__dirname, 'cursor-chat-factory.cjs')]);
}

function start(mode = 'continuous') {
  log('🚀 Cursor chat orchestrator started');
  if (mode === 'continuous') {
    const loop = () => { cycle(); setTimeout(loop, 60 * 60 * 1000); }; // hourly
    loop();
  } else {
    cycle();
  }
