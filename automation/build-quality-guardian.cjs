...
function findFiles(patterns, excludePatterns) {
  ...
  // Use a more reliable file finding approach
  try {
    // Find all TypeScript/JSX files first
    const allTsxFiles = execSync('find . -name "*.tsx" -o -name "*.jsx"', { encoding: 'utf8' })
      .split('\n')
      .filter(Boolean)
      .map(f => path.resolve(f));
    
    log(`Found ${allTsxFiles.length} total TypeScript/JSX files`);
    ...
    log(`After filtering: ${files.size} files to analyze`);
    
  } catch (error) {
    log(`Error finding files: ${error.message}`, 'WARNING');
  }
