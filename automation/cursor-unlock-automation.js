...
class CursorUnlockAutomation {
    ...
    async clearCursorCache() {
        ...
        for (const dir of directories) {
            try {
                ...
                this.log(`❌ Failed to clear cache in ${dir}: ${error.message}`, 'error');
            }
        }
    ...
    async backupWorkspace() {
        ...
        const backupDir = path.join(__dirname, 'backups', `cursor-backup-${Date.now()}`);
    ...
    async resetCursorExtensions() {
        ...
        if (platform === 'darwin') {
            extensionsDir = path.join(homeDir, 'Library', 'Application Support', 'Cursor', 'User', 'extensions');
        } else if (platform === 'win32') {
            extensionsDir = path.join(homeDir, 'AppData', 'Roaming', 'Cursor', 'User', 'extensions');
        } else {
            extensionsDir = path.join(homeDir, '.config', 'Cursor', 'User', 'extensions');
        }
    ...
    async optimizePerformance() {
        ...
        for (const command of optimizations) {
            try {
                ...
                this.log(`⚠️ Optimization failed: ${command} - ${error.message}`, 'warn');
            }
        }
    ...
    async unlockCursor(retryCount = 0) {
        ...
        this.log(`🚀 Starting Cursor unlock process (attempt ${retryCount + 1})...`);
        ...
        try {
            ...
            this.log(`📊 Summary: ${processesKilled} processes killed, ${filesUnlocked} files unlocked, ${optimizationsCompleted} optimizations completed`);
            ...
            if (retryCount < this.config.cursorUnlock.maxRetries) {
                ...
                await new Promise(resolve => setTimeout(resolve, this.config.cursorUnlock.retryDelay));
