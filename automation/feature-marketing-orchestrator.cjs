...
const LOG = path.join(__dirname, 'logs', 'feature-marketing-orchestrator.log');
...
function cycle() {
  run('node', [path.join(__dirname, 'feature-marketing-analyzer.cjs')]);
  run('node', [path.join(__dirname, 'feature-marketing-factory.cjs')]);
}

function start(mode = 'continuous') {
  log('🚀 Feature marketing orchestrator started');
  if (mode === 'continuous') {
