...
function fixCorruptedFile(filePath) {
  try {
    ...
    content = content
      ...
      .replace(/import,\s*typ,\s*e\s*{\s*NextPa,\s*g,\s*e\s*}\s*fr,\s*o,\s*m\s*'ne,\s*x,\s*t';;'/g, "import type { NextPage } from 'next'")
      .replace(/import\s*{\s*NextPage\s*}\s*from\s*'next';;'/g, "import type { NextPage } from 'next'")
      
      // Fix component declarations
      .replace(/const\s+(\w+),\s*:\s*NextPage\s*=\s*\(\)\s*=>\s*{/g, 'const $1: NextPage = () => {')
      ...
      // Fix comments
      .replace(/{\/\*\s*TODO:\s*Add,\s*page,\s*content\s*\/\*}/g, '{/* TODO: Add page content */}')
