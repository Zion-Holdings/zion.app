#!/usr/bin/env python3
"""
Targeted Structure Fixer
Fixes the specific corruption pattern where jobs are missing runs-on and steps sections
"""
...
def fix_targeted_structure(file_path):
    """Fix the targeted structural corruption in workflow files"""
    print(f"🔧 Targeted fixing: {file_path}")
    ...
    try:
        ...
        print(f"✅ Targeted fix successful")
...
def cleanup_content(content):
    ...
    for line in lines:
        ...
        # Skip stray content
        if stripped in ['contents: write', 'permissions: write', 'actions: read', 'pull-requests: write']:
            continue
...
def main():
    print("🚀 Starting targeted structure fixer...")
    ...
    for file_path in workflow_files:
        if fix_targeted_structure(file_path):
            fixed_count += 1
