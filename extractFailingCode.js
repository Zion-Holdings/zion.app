...
if (process.argv.length < 3) {
  console.error('Usage: node extractFailingCode.js <path_to_eslint_report.json>');'  process.exit(1);
}
;
const reportPath = process.argv[2];

try {
  const reportContent = fs.readFileSync(reportPath, 'utf8');'  const report = JSON.parse(reportContent);
