...
"""
import sys, json, time, datetime, importlib.util, collections, os
...
def send_current_batch(batch_path: Path, sender):
    ...
    try:
        for r in rows:
            ...
            subject = r.get('subject', '')
            body = r.get('body', '')
            # Try LLM tailoring for each email
            try:
                import os
                openrouter_key = os.environ.get('OPENROUTER_API_KEY', '')
                if openrouter_key:
                    contact_name = r.get('name', to.split('@')[0].replace('.', ' ').title())
                    contact_email = to
                    snippet = body[:500] if body else ''
                    tailored_body = _llm_tailor_reply_auth(
                        contact_name, contact_email, 'en', subject, snippet
                    )
                    if tailored_body:
                        body = tailored_body
                        append_log({'ts': now_iso(), 'event': 'llm_tailor_ok', 'to': to})
            except Exception:
                pass
            try:
                mid, tid = sender.send_mail(to, subject, body, r.get('html'))
