...
def main():
    config = load_config()
    send_allowed_env = os.environ.get('ZTG_SEND_ALLOWED', '1') == '1'
    send_allowed_config = bool(config.get('send_allowed', True))
    opt_in = bool(config.get('opt_in', True))
    if not send_allowed_env or not send_allowed_config or not opt_in:
        result = {
            'send_count': 0,
            'sent_to': None,
            'subject': None,
            ...
            'error': 'SEND_DISABLED: env/config/opt-in not enabled',
        }
    ...
    for r in rows:
        ...
        key = (to, subject, thread_id or '', message_id or '')
        if to in excluded:
            continue
        if key in sent:
            continue
