...
export default function RecommendationsPage() {
    ...
    if (loading || (error && !delayedError)) {
        return (<div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {Array.from({ length: 5 }).map((_, i) => (<div key={i} className="rounded-lg overflow-hidden border border-zion-blue-light">
    ...
    if (delayedError) {
        return (<div className="py-12 text-center text-red-400">Failed to load recommendations.</div>);
    }
    return (<div className="min-h-screen bg-zion-blue py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => (<ListingScoreCard key={item.id} title={item.title} description={item.description} image={item.images && item.images[0]} category={item.category} aiScore={item.aiScore ?? item.confidence}/>))}
      </div>
    </div>);
}
