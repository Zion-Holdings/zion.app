...
export default function TalentDetail() {
    ...
    if (loading) {
        return (<div className="p-4 space-y-2" data-testid="talent-loading">
    ...
    if (!talent) {
        return (<div className="p-4" data-testid="talent-not-found">
        <p>Talent not found</p>
      </div>);
    }
    return (<>
      <SEO title={talent.full_name} description={talent.bio || 'Talent profile'} ogImage={talent.profile_picture_url}/>
      <main className="min-h-screen bg-zion-blue py-8 text-white" data-testid="talent-details">
        <div className="container mx-auto px-4 space-y-4">
          ...
          {talent.skills && talent.skills.length > 0 && (<ul className="list-disc ml-4" data-testid="talent-skills">
              {talent.skills.map(skill => (<li key={skill}>{skill}</li>))}
            </ul>)}
