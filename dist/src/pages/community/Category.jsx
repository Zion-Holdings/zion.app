...
export default function Category() {
    ...
    const fetchPosts = async () => {
        const res = await fetch(`/api/community?category=${slug}&limit=${LIMIT}&offset=${offset}`);
        if (!res.ok)
    ...
    if (error)
        return <Alert variant="destructive">Failed to load posts</Alert>;
    const content = posts.length ? (<div className="space-y-4">
    ...
    </div>) : (<Empty message="No posts yet"/>);
