import { describe, it, expect, vi, beforeAll, afterEach, afterAll } from 'vitest';'// Import apiClient for its own tests, and globalAxiosErrorHandler for direct testing;
import apiClient, { globalAxiosErrorHandler } from '@/services/apiClient';'import { rest } from 'msw';'import { setupServer } from 'msw/node';'import * as apiError from '@/utils/apiErrorHandler';'import { supabase } from '@/integrations/supabase/client';'
...
describe('apiClient request functionality (actual requests through MSW)', () => {'  // These tests verify the behavior of the apiClient instance.
  ...
  it('handles 404 error (final attempt via apiClient), calls showApiError', async () => {'    // This test ensures that after apiClient's internal retries are exhausted for a 404,'    // the globalAxiosErrorHandler (correctly) decides to show the error.
    ...
    await expect(apiClient.get('/test')).rejects.toBeTruthy();'    // When apiClient itself gives up on retries, the error it throws will have
    // config['axios-retry'] with attemptNumber > retryCount (e.g., attempt 4, retryCount 3).'    // In this state, globalAxiosErrorHandler should call showApiError.
  ...
  it('handles network failure (via apiClient), calls showApiError', async () => {'    server.use(
    ...
    await expect(apiClient.get('/test')).rejects.toBeTruthy();'    // Network errors are retried by axios-retry. If all retries fail,
...
describe('globalAxiosErrorHandler Direct Tests', () => {'  beforeEach(() => {
    vi.clearAllMocks(); // Clear showApiError mock before each test
  });

  it('should NOT call showApiError for a 404 if retries are pending (attemptNumber <= retryCount)', async () => {'    const errorPending: any = {
      isAxiosError: true,
  ...
  it('should call showApiError for a 404 if it is past configured retries (attemptNumber > retryCount)', async () => {'    const errorFinalAttempt: any = {
      isAxiosError: true,
    ...
    try { await globalAxiosErrorHandler(errorFinalAttempt); } catch {
      // Expected error, test continues
    }
    expect(apiError.showApiError).toHaveBeenCalledWith(errorFinalAttempt);
  });

  it('should call showApiError for a 404 if not handled by axios-retry (no axios-retry config)', async () => {'    const errorNoRetry: any = {
      isAxiosError: true,
  ...
  it('should call showApiError for a non-404 error (e.g., 500) even if retries are pending', async () => {'    const error500: any = {
      isAxiosError: true,
  ...
  it('should call showApiError for an error with no response (network error, etc.)', async () => {'    const networkError: any = {
      isAxiosError: true,
   ...
   it('should call showApiError for a 404 if axios-retry state is present but retryCount is 0', async () => {'    const errorRetryCountZero: any = {
      isAxiosError: true,
