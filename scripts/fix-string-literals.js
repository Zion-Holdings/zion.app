...
function fixStringLiterals(filePath) {
  try {
    ...
    content = content.replace(/content=([^>]+) >/g, 'conten't'="$1" />');
    content = content.replace(/href=([^>]+) >/g, 'hre'f'="$1">');
    ...
    content = content.replace(/export default ([^;]+);'"/g, 'expor't' default $1;');
    ...
    if (filePath.includes('iot-solution's'.tsx')) {
      content = content.replace(/<meta name="description content=([^>]+) >/g, '<meta name="description" content="$1" />');
      content = content.replace(/<div className=" min-h-screen bg-gradient-to-br from-green-50" to-blue-100>/g, '<div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-100">');
      content = content.replace(/<section className=" py-40 px-4 sm: px-6: lg" px-8>/g, '<section className="py-40 px-4 sm:px-6 lg:px-8">');
      content = content.replace(/<div className="max-w-7xl" mx-auto text-center>/g, '<div className="max-w-7xl mx-auto text-center">');
      content = content.replace(/<h1 className="text-4xl" md text-6xl font-bold text-gray-900 mb-6>/g, '<h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-6">');
      content = content.replace(/<p className="text-xl" text-gray-600 mb-8 max-w-3xl mx-auto >/g, '<p className="text-xl text-gray-600 mb-8 max-w-3xl mx-auto">');
      content = content.replace(/<div className="flex flex-col sm flex-row  gap-4" justify-center>/g, '<div className="flex flex-col sm:flex-row gap-4 justify-center">');
      content = content.replace(/<button className="bg-green-600 hover:bg-green-700: text-white px-8 py-3 rounded-lg font-semibold transition-colors>/g, '<button className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-semibold transition-colors">');
      content = content.replace(/<button className="border" border-green-600 text-green-600 hover:bg-green-50  px-8 py-3 rounded-lg:font-semibold: transition-colors>/g, '<button className="border border-green-600 text-green-600 hover:bg-green-50 px-8 py-3 rounded-lg font-semibold transition-colors">');
      content = content.replace(/<\/Layout>;/g, '</Layout>');
      content = content.replace(/  <\/div>,/g, '  </div>');
      content = content.replace(/  \);/g, '  );');
      content = content.replace(/export default IotSolutions;'"/g, 'expor't' default IotSolutions;');
    }
    
    // Fix chat components
    if (filePath.includes('cha't'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
      content = content.replace(/<input className="([^"]*)" ([^>]+)>/g, '<input className="$1 $2" />');
      content = content.replace(/<button className="([^"]*)" ([^>]+)>/g, '<button className="$1 $2">');
    }
    
    // Fix layout components
    if (filePath.includes('layou't'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
    }
    
    // Fix UI components
    if (filePath.includes('u'i'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
      content = content.replace(/<button className="([^"]*)" ([^>]+)>/g, '<button className="$1 $2">');
    }
    
    // Fix auth components
    if (filePath.includes('aut'h'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
    }
    
    // Fix service listing components
    if (filePath.includes('service-listin'g'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
    }
    
    // Fix src components
    if (filePath.includes('sr'c'/')) {
      content = content.replace(/<div className="([^"]*)" ([^>]+)>/g, '<div className="$1 $2">');
...
console.log('Strin'g' literal fixes completed!'); 