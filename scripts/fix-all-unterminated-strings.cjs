...
files.forEach(file => {
  try {
    ...
    content = content.replace(/import type\s+([^;]+);'/g, (match, importContent) => {
      fileIssues++;
      return `import type ${importContent};`;
    });
    
    // Fix unterminated string literals in type declarations
    content = content.replace(/declare module ['"`]([^'"`]*)['"`]\s*\{/g, (match, moduleName) => {
      fileIssues++;
      return `declare module '${moduleName}' {`;
    });
    
    // Fix unterminated string literals in interface declarations
    content = content.replace(/interface\s+([^;]+);'/g, (match, interfaceContent) => {
      fileIssues++;
    ...
    content = content.replace(/interface ([^;]+);'+/g, (match, interfaceContent) => {
      fileIssues++;
      return `interface ${interfaceContent};`;
    });
