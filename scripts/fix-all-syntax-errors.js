  ...
  function traverse(currentDir) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (!['node_modules', '.git', 'coverage', 'dist', 'build', '.next'].includes(item)) {
          traverse(fullPath);
        }
...
function fixSyntaxErrors(content) {
  ...
  const unterminatedStringMatches = fixed.match(unterminatedStringPattern);
  if (unterminatedStringMatches) {
    fixed = fixed.replace(unterminatedStringPattern, "'$1';");
    changes += unterminatedStringMatches.length;
  }
  ...
  if (arrayBacktickMatches) {
    fixed = fixed.replace(arrayBacktickPattern, '];');
    changes += arrayBacktickMatches.length;
  }
  ...
  if (objectBacktickMatches) {
    fixed = fixed.replace(objectBacktickPattern, '};');
    changes += objectBacktickMatches.length;
  }
  ...
  if (functionCallBacktickMatches) {
    fixed = fixed.replace(functionCallBacktickPattern, ');');
    changes += functionCallBacktickMatches.length;
  }
  ...
  if (jsxBacktickMatches) {
    fixed = fixed.replace(jsxBacktickPattern, '>;');
    changes += jsxBacktickMatches.length;
  }
  
  // Fix import statements with backticks
  const importBacktickPattern = /from\s+['"]([^'"]*?)['"]\s*`\s*$/gm;
  const importBacktickMatches = fixed.match(importBacktickPattern);
  if (importBacktickMatches) {
    fixed = fixed.replace(importBacktickPattern, "from '$1';");
    changes += importBacktickMatches.length;
  }
  ...
  if (consoleBacktickMatches) {
    fixed = fixed.replace(consoleBacktickPattern, (match) => match.replace(/`\s*$/, ');'));
    changes += consoleBacktickMatches.length;
  }
...
function processFile(filePath) {
  try {
    ...
    if (changes > 0) {
      ...
      console.log(`✅ Fixed ${changes} syntax errors in ${path.relative(process.cwd(), filePath)}`);
      return changes;
    }
    return 0;
  } catch (error) {
