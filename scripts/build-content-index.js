...
const slug = path.basename(filePath).replace(/\.md$/, );  return { ...meta, id: slug, slug, content };
}

function build(dir) {
  const files = fs.readdirSync(dir).filter(f => f.endsWith('.md'));  return files.map(f => parseFile(path.join(dir, f))).filter(Boolean);
}
