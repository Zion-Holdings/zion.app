...
class PostBuildHealer {
  constructor() {
    this.logFile = 'logs/post-build-healer.log';
  ...
  async runLinting() {
    try {
      ...
      this.log('ESLint failed, running auto-fix...', 'WARN');
      try {
        execSync('npm run lint:fix', { stdio: 'inherit' });
        this.log('ESLint auto-fix applied');
        this.fixesApplied.push('ESLint fixes');
        return true;
      } catch (fixError) {
  ...
  async runTypeCheck() {
    try {
      ...
      this.log('TypeScript type check failed', 'WARN');
      return false;
    }
  }
  ...
  async fixMissingDependencies() {
    try {
      ...
      this.log(`Failed to fix missing dependencies: ${error.message}`, 'ERROR');
      return false;
    }
  }
  ...
  async fixImportIssues() {
    try {
      ...
      this.log(`Failed to fix import issues: ${error.message}`, 'ERROR');
      return false;
    }
  }
  ...
  async fixTypeIssues() {
    try {
      ...
      for (const file of tsFiles.slice(0, 10)) { // Limit to first 10 files
        ...
        // Fix null checks
        if (content.includes('Object is possibly')) {
          const newContent = content.replace(/\.(\w+)/g, '?.$1');
          if (newContent !== content) {
            fs.writeFileSync(file, newContent);
            modified = true;
          }
      ...
      this.log(`Failed to fix type issues: ${error.message}`, 'ERROR');
      return false;
    }
  }
  ...
  async fixBuildConfigIssues() {
    try {
      ...
      this.log(`Failed to fix build config issues: ${error.message}`, 'ERROR');
      return false;
    }
  }
...
`;
        ...
        this.log('Created .env.local file with placeholder values');
        return true;
      }
  ...
  findTypeScriptFiles() {
    ...
    return glob.sync('src/**/*.{ts,tsx}', { cwd: process.cwd() });
  }

  fixRelativeImport(importStatement, filePath) {
    // Simple relative import fix - in a real implementation, this would be more sophisticated
  ...
  async triggerNetlifyBuild() {
    try {
      ...
      try {
        ...
        this.log('Netlify build triggered via CLI');
        return true;
      } catch (netlifyError) {
        // Fallback to git push
        this.log('Netlify CLI not available, using git push fallback');
        execSync('git push', { stdio: 'inherit' });
  ...
  async run() {
    ...
    while (this.currentRetry < this.maxRetries) {
      ...
      // Apply fixes
      this.log('Build failed, applying fixes...');
    ...
    this.log('Post-build healing process finished.');
...
if (require.main === module) {
  ...
  healer.run().catch(error => {
    console.error('Post-build healer failed:', error);
    process.exit(1);
  });
}
