...
// 2. Check for duplicate workflow_dispatch entries in trigger section
for (const file of workflowFiles) {
  const content = fs.readFileSync(path.join(wfDir, file), 'utf8');
  // Only check the top-level trigger section (before the first 'jobs:' block)
  const jobsIdx = content.indexOf('\njobs:');
  const triggerSection = jobsIdx >= 0 ? content.substring(0, jobsIdx) : content;
  const dispatchMatches = triggerSection.match(/^\s*workflow_dispatch:/gm);
