...
class HealAndDeploy {
  constructor() {
    this.logFile = 'logs/heal-and-deploy.log';
  ...
  async run() {
    ...
    try {
      ...
      this.log(`Heal and deploy process failed: ${error.message}`, 'ERROR');
      throw error;
    }
  }
  ...
  async preDeploymentCheck() {
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        this.log(`Pre-deployment check ${index + 1} failed: ${result.reason}`, 'ERROR');
      }
    });
  }
...
NEXT_PUBLIC_REOWN_PROJECT_ID=${process.env.NEXT_PUBLIC_REOWN_PROJECT_ID || 'your_reown_project_id_here'}
`;
      fs.writeFileSync('.env.local', envContent);
      this.log('Created .env.local file with available values');
    } else {
  ...
  async checkDependencies() {
    ...
    try {
      ...
      if (Object.keys(outdated).length > 0) {
        this.log('Outdated dependencies found', 'WARN');
        this.log(`Outdated packages: ${Object.keys(outdated).join(', ')}`);
      } else {
  ...
  async runQualityChecks() {
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected' || result.value === false) {
        this.log(`Quality check ${index + 1} failed`, 'ERROR');
        hasFailures = true;
      }
    });
  ...
  async triggerDeployment() {
    try {
      ...
      try {
        ...
        this.log('Deployment triggered via Netlify CLI');
        this.deploymentStatus = 'netlify-cli';
      } catch (netlifyError) {
        // Fallback to git push for Netlify auto-deploy
        this.log('Netlify CLI not available, using git push for auto-deploy');
        execSync('git push', { stdio: 'inherit' });
        this.log('Git push completed, Netlify auto-deploy should trigger');
        this.deploymentStatus = 'git-push';
      }
    } catch (error) {
...
if (require.main === module) {
  ...
  healer.run().catch(error => {
    console.error('Heal and deploy failed:', error);
    process.exit(1);
  });
}
