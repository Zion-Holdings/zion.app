...
function isExcluded(text) {
  ...
  if (
    EXCEPTIONS.some((exc) => text.toLowerCase().includes(exc.toLowerCase()))
  ) {
    return true;
  }
...
function shouldBeTranslated(text) {
  ...
  return (
    TRANSLATION_INDICATORS.some((indicator) =>
...
const mediumPriority = filesWithHardcodedStrings.filter(
  ...
  const allStrings = [
    ...new Set(filesWithHardcodedStrings.flatMap((f) => f.hardcodedStrings)),
  ];
