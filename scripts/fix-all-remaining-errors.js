...
function fixAllRemainingErrors(filePath) {
  try {
    ...
    if (filePath.includes('cloud-services.tsx')) {
      ...
      content = content.replace(/import Head from "next\/head;}/g, "import Head from \next/head\");
      content = content.replace(/import Link from next\/link"/g, import Link from \next/link\"");
      content = content.replace(/;/g, );
      ...
      content = content.replace(/<Link href="""\/marketplace className="([^"]+)">/g, '<Link href=""/marketplace className=""$1>');
      content = content.replace(/<Link href="""\/about className="([^"]+)">/g, '<Link href=""/about className=""$1>');
      content = content.replace(/<Link href="""\/auth\/login className="([^"]+)">/g, '<Link href=""/auth/login className=""$1>');
      content = content.replace(/<Link href="""\/auth\/signup className="([^"]+)">/g, '<Link href=""/auth/signup className=""$1>');
      ...
      content = content.replace(/<li><Link href=([^]+) className=""([^]+)">/g, '<li><Link href="""$1 className=""$2>');
