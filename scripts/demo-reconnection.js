...
console.log('This demonstration will show how the automatic reconnection system');
...
class LiveReconnectionDemo {
  ...
  async step3_SimulateInterruption() {
    ...
    console.log('⚠️  Network interruption detected! All connections will be lost.\n');
    ...
    console.log('🔌 All connections lost. Automatic reconnection will begin...\n');
  }
  ...
  async step4_ShowReconnection() {
    ...
    for (const [sessionName, session] of this.sessions) {
      const status = session.socket.connected ? '✅ Connected' : '🔄 Reconnecting...';
  ...
  async step5_SuccessfulReconnection() {
    ...
    for (const [sessionName, session] of this.sessions) {
      if (session.socket.connected) {
        console.log(`${session.color} ${sessionName}: ✅ Successfully reconnected!`);
  ...
  async step6_FinalStatus() {
    ...
    for (const [sessionName, session] of this.sessions) {
      const status = session.socket.connected ? '✅ Connected' : '❌ Disconnected';
    ...
    console.log(`📊 Overall: ${connectedCount}/${this.sessions.size} sessions connected`);
    ...
    console.log('💡 This ensures seamless chat experience even during network interruptions.\n');
  }
  ...
  async createSession(sessionConfig) {
    return new Promise((resolve) => {
      ...
      socket.on('connect', () => {
        console.log(`${sessionConfig.color} ✅ ${sessionConfig.name} connected`);
        
        // Join room
        socket.emit('join-room', sessionConfig.roomId, (response) => {
          if (response && response.success) {
            console.log(`${sessionConfig.color} 🎯 ${sessionConfig.name} joined room: ${sessionConfig.roomId}`);
          }
        });
      ...
      socket.on('reconnect', (attemptNumber) => {
        ...
        console.log(`${sessionConfig.color} 🔄 ${sessionConfig.name} reconnected after ${attemptNumber} attempts`);
        
        // Rejoin room
        socket.emit('join-room', sessionConfig.roomId, (response) => {
          if (response && response.success) {
            console.log(`${sessionConfig.color} 🎯 ${sessionConfig.name} rejoined room: ${sessionConfig.roomId}`);
          }
        });
      });
      ...
      socket.on('reconnect_attempt', (attemptNumber) => {
        session.reconnectionAttempts = attemptNumber;
        console.log(`${sessionConfig.color} 🔄 ${sessionConfig.name} reconnection attempt ${attemptNumber}`);
      });

      socket.on('disconnect', (reason) => {
        console.log(`${sessionConfig.color} 🔌 ${sessionConfig.name} disconnected: ${reason}`);
      });
  ...
  async sendMessage(session, message) {
    return new Promise((resolve) => {
      session.socket.emit('send-message', {
        ...
        if (response && response.success) {
          console.log(`${session.color} 💬 ${session.name}: Message sent successfully`);
        }
        resolve();
      });
    });
