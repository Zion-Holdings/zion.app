...
ts += `  return allServices.find(s => s.id === id);\n`;
ts += `}\n\n`;
ts += `export function getServicesByCategory(category: string): Service[] {\n`;
ts += `  return allServices.filter(s => s.category.toLowerCase() === category.toLowerCase());\n`;
ts += `}\n\n`;
ts += `export function getPopularServices(limit = 20): Service[] {\n`;
ts += `  return allServices.slice(0, limit);\n`;
ts += `}\n`;
