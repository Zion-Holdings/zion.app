...
class AIDelegationCleanup {
    ...
    for (const processName of CONFIG.PROCESSES) {
      try {
        ...
        try {
          ...
          this.logCleanup('WARN', `${processName} is still running, force killing...`);
    ...
    for (const dir of CONFIG.DIRECTORIES) {
      ...
      if (fs.existsSync(dirPath)) {
        try {
          ...
          this.logCleanup('FAIL', `Failed to remove directory ${dir}: ${error.message}`);
        }
      } else {
    ...
    for (const file of CONFIG.FILES) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          this.logCleanup('FAIL', `Failed to remove file ${file}: ${error.message}`);
        }
      } else {
          ...
          for (const file of files) {
            ...
            if (isTempFile) {
              try {
                ...
                this.logCleanup('WARN', `Failed to remove temp file ${dir}/${file}: ${error.message}`);
              }
            }
          }
        } catch (error) {
          this.logCleanup('WARN', `Failed to scan directory ${dir}: ${error.message}`);
        }
      }
    }
  }
  ...
  async generateCleanupReport() {
    ...
    const report = {
      ...
      actions: {
        processes: CONFIG.PROCESSES.map(process => ({
          name: process,
          stopped: this.cleanupLog.some(r => r.message.includes(process) && r.status === 'PASS')
        })),
        directories: CONFIG.DIRECTORIES.map(dir => ({
          name: dir,
          cleaned: this.cleanupLog.some(r => r.message.includes(dir) && r.status === 'PASS')
        })),
        files: CONFIG.FILES.map(file => ({
          name: file,
          cleaned: this.cleanupLog.some(r => r.message.includes(file) && r.status === 'PASS')
        }))
      },
    ...
    this.logCleanup('INFO', `Cleanup report saved to: logs/cleanup-report.json`);
