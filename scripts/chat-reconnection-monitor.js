...
class ChatReconnectionMonitor {
  ...
  async start() {
    ...
    console.log('📊 Monitoring interval:', CONFIG.MONITOR_INTERVAL / 1000, 'seconds');
  ...
  displayStatus(status, responseTime) {
    ...
    console.log(`📊 Status Check #${this.metrics.checks} - ${new Date().toLocaleTimeString()}`);
    ...
    console.log(`   📈 Errors: ${this.metrics.errors}/${this.metrics.checks} (${((this.metrics.errors / this.metrics.checks) * 100).toFixed(1)}%)`);
    console.log('');
  }
  ...
  async analyzeLogs() {
    try {
      ...
      console.log(`📋 Log Analysis: ${errorCount} errors, ${chatCount} chats, ${cursorCount} Cursor interactions`);
  ...
  generateReport() {
    const report = {
      ...
      performance: {
        avgResponseTime: this.calculateAverage(this.metrics.performance.responseTimes),
  ...
  displayReport() {
    ...
    console.log(`⚡ Avg Response: ${report.performance.avgResponseTime.toFixed(0)}ms`);
