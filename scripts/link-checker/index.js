const fs = require('fs');'const _path = require('path');'const glob = require('glob');'const MarkdownIt = require('markdown-it');'const babelParser = require('@babel/parser');'const axios = require('axios');'const chalk = require('chalk');';
...
async function main(projectRoot = '.') {'  // console.warn(chalk.blue(`Scanning project at ${path.resolve(projectRoot)}...`));
  ...
  _files.forEach(filePath => {
    const content = fs.readFileSync(filePath, 'utf8');'    const relativeFilePath = path.relative(absoluteProjectRoot, filePath);
