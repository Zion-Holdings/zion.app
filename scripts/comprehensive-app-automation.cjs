    ...
    const fixFile = (filePath) => {
      try {
        ...
        content = content
          ...
          .replace(/function\s+\w+\(\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{/g, 'function handler(')
          .replace(/\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{\):\s*unknown\s*\{/g, ') {')
          // Fix type annotations
          .replace(/:\s*unknown\s+unknown\s+unknown\s+unknown\s+unknown\s+unknown/g, '')
  ...
  async runFullAutomation() {
    ...
    try {
      ...
      if (this.serverStarted) {
        await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for server to be ready
