...
function gitCommitAndPush(filesToAdd) {
  ...
  try {
    execSync(`git commit -m "${msg}" --no-verify`, { stdio: 'inherit', env: { ...process.env, HUSKY: '0' } });
  } catch (err) {
    log(`Git commit failed: ${err && err.message ? err.message : err}. Continuing without failing.`);
    return { committed: false, pushed: false };
  }
