...
async function testReconnectionScenario(scenario) {
  ...
  return new Promise((resolve) => {
    ...
    reconnectionManager = new TestReconnectionManager(TEST_CONFIG);
    ...
    socket.on('connect', () => {
      connectionAttempts++;
      console.log(`✅ Connected to ${scenario.name} (attempt ${connectionAttempts})`);
    ...
    socket.on('reconnect', (attemptNumber) => {
      ...
      console.log(`🔄 Reconnected to ${scenario.name} after ${attemptNumber} attempts`);
      ...
      socket.emit('join-room', scenario.roomId, (response) => {
        if (response && response.success) {
          ...
          socket.emit('send-message', {
            roomId: scenario.roomId,
            message: `Reconnection test message from ${scenario.name} - ${new Date().toISOString()}`,
            sender: 'test-user',
            type: 'text'
          }, (response) => {
            if (response && response.success) {
    ...
    socket.on('reconnect_attempt', (attemptNumber) => {
      reconnectionAttempts++;
      console.log(`🔄 Reconnection attempt ${attemptNumber} for ${scenario.name}`);
      reconnectionManager.handleReconnectionAttempt();
    });
    
    socket.on('reconnect_error', (error) => {
      console.log(`❌ Reconnection error for ${scenario.name}: ${error.message}`);
    ...
    reconnectionManager.on('reconnection_attempt', (event) => {
      console.log(`📊 Reconnection manager: Attempt ${event.attempt} for ${scenario.name}`);
    });
    
    reconnectionManager.on('reconnection_success', (event) => {
      console.log(`✅ Reconnection manager: Success for ${scenario.name} after ${event.attempt} attempts`);
    });
    
    reconnectionManager.on('reconnection_failure', (event) => {
      console.log(`❌ Reconnection manager: Failure for ${scenario.name} - ${event.error}`);
    });
    
    reconnectionManager.on('health_check', (event) => {
      console.log(`💓 Health check for ${scenario.name}: ${event.health.connectionQuality} (${event.health.latency}ms)`);
    });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (!testCompleted) {
          ...
          console.log(`   Successful reconnections: ${successfulReconnections}`);
          console.log(`   Connection quality: ${reconnectionManager.getConnectionHealth().connectionQuality}`);
          console.log(`   Average latency: ${reconnectionManager.getAverageLatency()}ms`);
          ...
          resolve({
            ...
            reconnectionAttempts,
            successfulReconnections,
            connectionQuality: reconnectionManager.getConnectionHealth().connectionQuality,
            averageLatency: reconnectionManager.getAverageLatency()
          });
        }
      }, 20000); // Wait 20 seconds for reconnection attempts
    }, 3000);
  });
}
...
async function runAllTests() {
  ...
  results.forEach(result => {
    if (result.error) {
      ...
      console.log(`   Success Rate: ${result.successfulReconnections}/${result.reconnectionAttempts}`);
  ...
  if (successfulTests === totalTests) {
    console.log('🎉 All reconnection tests passed! The automatic reconnection system is working correctly.');
  } else {
