...
TOKENS_FILE = None
for candidate in [
    _WORKSPACE / 'gog_tokens.json',
    LEAD_CRM_FALLBACK / 'gog_tokens.json',
]:
    if candidate.exists():
        TOKENS_FILE = candidate
        break
if TOKENS_FILE is None:
    TOKENS_FILE = _WORKSPACE / 'gog_tokens.json'
...
EXCLUSION_CANDIDATES = (
    _WORKSPACE / 'zion-clone-test2' / 'lead-crm' / 'exclusion-list.json',
    _WORKSPACE / 'zion-clone-test' / 'lead-crm' / 'exclusion-list.json',
...
def drive_list(query=None, folder_id=None, limit=20):
    ...
    url = ('https://www.googleapis.com/drive/v3/files'
           f'?q={",".join(q_parts)}&pageSize={limit}&fields=files(id,name,modifiedTime,mimeType)')
    req = urllib.request.Request(url, headers=gog_headers())
    resp = json.loads(urllib.request.urlopen(req).read())
