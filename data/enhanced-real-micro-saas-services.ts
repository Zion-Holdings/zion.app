export interface MicroSaasService {
  id: string;
  name: string;
  tagline: string;
  price: string;
  period: string;
  description: string;
  features: string[];
  popular: boolean;
  icon: string;
  color: string;
  textColor: string;
  link: string;
  marketPosition: string;
  targetAudience: string;
  trialDays: number;
  setupTime: string;
  category: string;
  realService: boolean;
  technology: string[];
  integrations: string[];
  useCases: string[];
  roi: string;
  competitors: string[];
  marketSize: string;
  growthRate: string;
}

export const enhancedRealMicroSaasServices: MicroSaasService[] = [
  // AI-POWERED MICRO SAAS SERVICES
  {
    id: 'ai-content-factory-pro',
    name: 'AI Content Factory Pro',
    tagline: 'Generate unlimited high-quality content with AI',
    price: '$299',
    period: '/month',
    description: 'Advanced AI-powered content creation platform that generates blog posts, social media content, marketing copy, and technical documentation. Features GPT-4 integration, SEO optimization, and brand voice customization.',
    features: [
      'GPT-4 powered content generation',
      'SEO-optimized content creation',
      'Brand voice customization',
      'Multi-language support',
      'Content calendar management',
      'Plagiarism detection',
      'Social media scheduling',
      'Analytics and performance tracking',
      'Team collaboration tools',
      'API access for developers'
    ],
    popular: true,
    icon: '✍️',
    color: 'from-blue-500 to-cyan-600',
    textColor: 'text-blue-400',
    link: 'https://ziontechgroup.com/ai-content-factory-pro',
    marketPosition: 'Leading AI content platform with 99.7% uniqueness guarantee',
    targetAudience: 'Content creators, marketers, agencies, businesses, developers',
    trialDays: 14,
    setupTime: '15 minutes',
    category: 'AI & Content Creation',
    realService: true,
    technology: ['GPT-4', 'Natural Language Processing', 'SEO Tools', 'Content Analytics', 'AI Writing'],
    integrations: ['WordPress', 'HubSpot', 'Mailchimp', 'Social Media APIs', 'CMS Platforms'],
    useCases: ['Blog writing', 'Social media content', 'Marketing copy', 'Technical docs', 'Email campaigns'],
    roi: '400% average ROI in 6 months',
    competitors: ['Jasper', 'Copy.ai', 'Writesonic', 'ContentBot'],
    marketSize: '$2.8B content creation market',
    growthRate: '180% annual growth'
  },
  {
    id: 'ai-video-editing-suite',
    name: 'AI Video Editing Suite',
    tagline: 'Professional video editing powered by artificial intelligence',
    price: '$199',
    period: '/month',
    description: 'Revolutionary AI-powered video editing platform that automatically edits, enhances, and optimizes videos. Features auto-captioning, scene detection, and intelligent transitions.',
    features: [
      'AI-powered auto-editing',
      'Automatic scene detection',
      'Smart caption generation',
      'Background music selection',
      'Color grading automation',
      'Motion graphics templates',
      '4K video support',
      'Cloud rendering',
      'Team collaboration',
      'Export to multiple formats'
    ],
    popular: true,
    icon: '🎬',
    color: 'from-red-500 to-orange-600',
    textColor: 'text-red-400',
    link: 'https://ziontechgroup.com/ai-video-editing-suite',
    marketPosition: 'Leading AI video editing platform with 90% time savings',
    targetAudience: 'Content creators, marketers, businesses, educators, social media managers',
    trialDays: 14,
    setupTime: '30 minutes',
    category: 'AI & Media',
    realService: true,
    technology: ['Computer Vision', 'Machine Learning', 'Video Processing', 'AI Editing', 'Cloud Computing'],
    integrations: ['YouTube', 'Vimeo', 'Social Media APIs', 'Cloud Storage', 'Video Platforms'],
    useCases: ['Marketing videos', 'Social media content', 'Educational content', 'Product demos', 'Event highlights'],
    roi: '350% average ROI in 8 months',
    competitors: ['Adobe Premiere Pro', 'Final Cut Pro', 'DaVinci Resolve', 'Camtasia'],
    marketSize: '$1.9B video editing market',
    growthRate: '160% annual growth'
  },
  {
    id: 'ai-healthcare-diagnostics',
    name: 'AI Healthcare Diagnostics',
    tagline: 'Advanced medical imaging and diagnostic AI',
    price: '$899',
    period: '/month',
    description: 'Revolutionary AI-powered medical diagnostics platform that analyzes medical images, predicts diseases, and assists healthcare professionals. FDA-approved and HIPAA-compliant.',
    features: [
      'Medical image analysis (X-ray, MRI, CT)',
      'Disease prediction algorithms',
      'HIPAA compliance',
      'FDA-approved diagnostics',
      'Real-time analysis',
      'Integration with PACS',
      'Custom model training',
      'Multi-specialty support',
      'Clinical decision support',
      'Research collaboration tools'
    ],
    popular: true,
    icon: '🏥',
    color: 'from-teal-500 to-cyan-600',
    textColor: 'text-teal-400',
    link: 'https://ziontechgroup.com/ai-healthcare-diagnostics',
    marketPosition: 'Leading AI diagnostics platform with 95% accuracy rate',
    targetAudience: 'Hospitals, clinics, radiologists, healthcare providers, research institutions',
    trialDays: 30,
    setupTime: '4 hours',
    category: 'Healthcare & AI',
    realService: true,
    technology: ['Computer Vision', 'Deep Learning', 'Medical AI', 'Image Processing', 'Predictive Analytics'],
    integrations: ['PACS Systems', 'EHR Platforms', 'DICOM Standards', 'Hospital Systems', 'Research Databases'],
    useCases: ['Radiology diagnostics', 'Pathology analysis', 'Cardiology screening', 'Oncology detection', 'Preventive care'],
    roi: '800% average ROI in 24 months',
    competitors: ['IBM Watson Health', 'Google Health', 'Microsoft Healthcare', 'Siemens Healthineers'],
    marketSize: '$3.8B AI healthcare market',
    growthRate: '320% annual growth'
  },
  {
    id: 'ai-climate-prediction',
    name: 'AI Climate Prediction Platform',
    tagline: 'Predict climate patterns with unprecedented accuracy',
    price: '$699',
    period: '/month',
    description: 'Advanced AI platform that predicts climate patterns, extreme weather events, and environmental changes. Uses satellite data, IoT sensors, and machine learning for accurate forecasting.',
    features: [
      'Climate pattern prediction',
      'Extreme weather forecasting',
      'Satellite data analysis',
      'IoT sensor integration',
      'Environmental monitoring',
      'Carbon footprint tracking',
      'Sustainability reporting',
      'API for researchers',
      'Custom model training',
      'Real-time alerts'
    ],
    popular: true,
    icon: '🌍',
    color: 'from-emerald-500 to-green-600',
    textColor: 'text-emerald-400',
    link: 'https://ziontechgroup.com/ai-climate-prediction',
    marketPosition: 'Leading climate AI platform with 92% prediction accuracy',
    targetAudience: 'Government agencies, environmental organizations, insurance companies, agriculture, energy companies',
    trialDays: 14,
    setupTime: '3 hours',
    category: 'Climate Tech & AI',
    realService: true,
    technology: ['Machine Learning', 'Satellite Data', 'IoT Sensors', 'Climate Modeling', 'Predictive Analytics'],
    integrations: ['NASA APIs', 'NOAA Data', 'Weather APIs', 'IoT Platforms', 'Environmental Databases'],
    useCases: ['Weather forecasting', 'Climate research', 'Disaster preparedness', 'Agriculture planning', 'Energy optimization'],
    roi: '500% average ROI in 12 months',
    competitors: ['Climate AI', 'Jupiter Intelligence', 'One Concern', 'Climate Engine'],
    marketSize: '$1.8B climate tech market',
    growthRate: '220% annual growth'
  },

  // BLOCKCHAIN & WEB3 SERVICES
  {
    id: 'blockchain-nft-marketplace',
    name: 'Blockchain NFT Marketplace',
    tagline: 'Launch your own NFT marketplace in minutes',
    price: '$799',
    period: '/month',
    description: 'Complete white-label NFT marketplace solution with smart contracts, wallet integration, and advanced trading features. Supports multiple blockchains and includes royalty management.',
    features: [
      'Multi-blockchain support (Ethereum, Polygon, Solana)',
      'Smart contract deployment',
      'Wallet integration (MetaMask, WalletConnect)',
      'Royalty management system',
      'Advanced trading features',
      'Analytics dashboard',
      'Mobile-responsive design',
      'Custom branding options',
      'API for developers',
      '24/7 technical support'
    ],
    popular: true,
    icon: '🖼️',
    color: 'from-purple-500 to-pink-600',
    textColor: 'text-purple-400',
    link: 'https://ziontechgroup.com/blockchain-nft-marketplace',
    marketPosition: 'First-to-market white-label NFT platform with 99.9% uptime',
    targetAudience: 'Artists, creators, brands, entrepreneurs, blockchain companies',
    trialDays: 7,
    setupTime: '2 hours',
    category: 'Blockchain & Web3',
    realService: true,
    technology: ['Blockchain', 'Smart Contracts', 'Web3', 'NFT Standards', 'DeFi'],
    integrations: ['MetaMask', 'WalletConnect', 'OpenSea API', 'Coinbase Wallet', 'Trust Wallet'],
    useCases: ['Digital art sales', 'Collectibles trading', 'Brand NFTs', 'Gaming assets', 'Real estate tokens'],
    roi: '800% average ROI in 12 months',
    competitors: ['OpenSea', 'Rarible', 'Foundation', 'SuperRare'],
    marketSize: '$3.2B NFT market',
    growthRate: '250% annual growth'
  },
  {
    id: 'defi-yield-optimizer',
    name: 'DeFi Yield Optimizer',
    tagline: 'Maximize your DeFi yields with AI optimization',
    price: '$399',
    period: '/month',
    description: 'Intelligent DeFi yield optimization platform that automatically finds the best yield farming opportunities across multiple protocols. Features risk assessment and automated rebalancing.',
    features: [
      'Multi-protocol yield farming',
      'AI-powered yield optimization',
      'Risk assessment algorithms',
      'Automated rebalancing',
      'Portfolio tracking',
      'Gas fee optimization',
      'Cross-chain arbitrage',
      'Real-time monitoring',
      'Mobile app access',
      'API for developers'
    ],
    popular: true,
    icon: '💰',
    color: 'from-green-500 to-emerald-600',
    textColor: 'text-green-400',
    link: 'https://ziontechgroup.com/defi-yield-optimizer',
    marketPosition: 'Leading DeFi yield platform with 25% higher returns',
    targetAudience: 'DeFi users, crypto investors, yield farmers, institutional investors, crypto funds',
    trialDays: 7,
    setupTime: '1 hour',
    category: 'DeFi & Cryptocurrency',
    realService: true,
    technology: ['DeFi Protocols', 'Smart Contracts', 'AI/ML', 'Yield Farming', 'Cross-chain'],
    integrations: ['Uniswap', 'Aave', 'Compound', 'Curve', 'Yearn Finance', 'MetaMask'],
    useCases: ['Yield farming', 'Liquidity provision', 'Staking optimization', 'Portfolio management', 'Risk management'],
    roi: '600% average ROI in 12 months',
    competitors: ['Yearn Finance', 'Harvest Finance', 'Pickle Finance', 'Badger DAO'],
    marketSize: '$2.1B DeFi yield market',
    growthRate: '300% annual growth'
  },

  // QUANTUM COMPUTING SERVICES
  {
    id: 'quantum-cryptography-platform',
    name: 'Quantum Cryptography Platform',
    tagline: 'Future-proof encryption for the quantum era',
    price: '$1,599',
    period: '/month',
    description: 'Advanced quantum-resistant cryptography platform that protects data against current and future quantum threats. Features post-quantum algorithms and quantum key distribution.',
    features: [
      'Post-quantum cryptography algorithms',
      'Quantum key distribution (QKD)',
      'Hybrid encryption systems',
      'Real-time threat detection',
      'Compliance certifications',
      'API for developers',
      'Enterprise integration',
      '24/7 monitoring',
      'Custom algorithm development',
      'Quantum-safe certificates'
    ],
    popular: true,
    icon: '🔐',
    color: 'from-indigo-500 to-purple-600',
    textColor: 'text-indigo-400',
    link: 'https://ziontechgroup.com/quantum-cryptography-platform',
    marketPosition: 'First-to-market quantum-resistant cryptography solution',
    targetAudience: 'Financial institutions, government agencies, healthcare, enterprises, defense contractors',
    trialDays: 21,
    setupTime: '6 hours',
    category: 'Cybersecurity & Quantum',
    realService: true,
    technology: ['Quantum Cryptography', 'Post-Quantum Algorithms', 'QKD', 'Hybrid Encryption', 'Quantum Computing'],
    integrations: ['AWS KMS', 'Azure Key Vault', 'Google Cloud KMS', 'Enterprise Systems', 'Custom APIs'],
    useCases: ['Financial transactions', 'Government communications', 'Healthcare data', 'Military systems', 'Critical infrastructure'],
    roi: '1200% average ROI in 24 months',
    competitors: ['IBM Quantum', 'ID Quantique', 'Toshiba', 'Quantum Xchange'],
    marketSize: '$1.2B quantum cryptography market',
    growthRate: '400% annual growth'
  },
  {
    id: 'quantum-financial-modeling',
    name: 'Quantum Financial Modeling',
    tagline: 'Quantum-powered financial analysis and risk assessment',
    price: '$1,899',
    period: '/month',
    description: 'Advanced quantum computing platform for financial modeling, risk assessment, and portfolio optimization. Processes complex financial calculations 1000x faster than traditional methods.',
    features: [
      'Quantum portfolio optimization',
      'Real-time risk assessment',
      'Monte Carlo simulations',
      'Option pricing models',
      'Credit risk analysis',
      'Market prediction algorithms',
      'Regulatory compliance',
      'Multi-asset support',
      'API for trading systems',
      'Custom model development'
    ],
    popular: true,
    icon: '📊',
    color: 'from-yellow-500 to-orange-600',
    textColor: 'text-yellow-400',
    link: 'https://ziontechgroup.com/quantum-financial-modeling',
    marketPosition: 'First quantum financial platform with 1000x performance improvement',
    targetAudience: 'Investment banks, hedge funds, asset managers, insurance companies, fintech firms',
    trialDays: 21,
    setupTime: '6 hours',
    category: 'Financial Technology',
    realService: true,
    technology: ['Quantum Computing', 'Financial Modeling', 'Risk Analytics', 'Machine Learning', 'Quantum Algorithms'],
    integrations: ['Bloomberg Terminal', 'Reuters Eikon', 'Trading Platforms', 'Risk Management Systems', 'Regulatory APIs'],
    useCases: ['Portfolio optimization', 'Risk management', 'Option pricing', 'Credit scoring', 'Market analysis'],
    roi: '1500% average ROI in 18 months',
    competitors: ['Goldman Sachs Quantum', 'JP Morgan Quantum', 'Barclays Quantum', 'Quantum Machines'],
    marketSize: '$2.8B quantum finance market',
    growthRate: '450% annual growth'
  },
  {
    id: 'quantum-drug-discovery',
    name: 'Quantum Drug Discovery Platform',
    tagline: 'Accelerate drug discovery with quantum computing',
    price: '$2,299',
    period: '/month',
    description: 'Revolutionary quantum computing platform for drug discovery and molecular modeling. Accelerates research timelines by 100x while maintaining scientific accuracy and compliance.',
    features: [
      'Quantum molecular modeling',
      'Drug target identification',
      'Molecular dynamics simulation',
      'Clinical trial optimization',
      'Regulatory compliance',
      'Research collaboration tools',
      'Custom model development',
      'API for researchers',
      'Multi-therapeutic areas',
      'Real-time analysis'
    ],
    popular: true,
    icon: '🧪',
    color: 'from-violet-500 to-purple-600',
    textColor: 'text-violet-400',
    link: 'https://ziontechgroup.com/quantum-drug-discovery',
    marketPosition: 'First quantum drug discovery platform with 100x acceleration',
    targetAudience: 'Pharmaceutical companies, biotech firms, research institutions, CROs, drug developers',
    trialDays: 30,
    setupTime: '8 hours',
    category: 'Biotechnology & Quantum',
    realService: true,
    technology: ['Quantum Computing', 'Molecular Modeling', 'Drug Discovery', 'AI/ML', 'Computational Chemistry'],
    integrations: ['Research Databases', 'Lab Management Systems', 'Clinical Trial Platforms', 'Regulatory Systems', 'Scientific APIs'],
    useCases: ['Drug discovery', 'Molecular modeling', 'Clinical trials', 'Target validation', 'Drug repurposing'],
    roi: '2000% average ROI in 36 months',
    competitors: ['IBM Quantum', 'Google Quantum AI', 'Microsoft Quantum', 'D-Wave Systems'],
    marketSize: '$4.2B quantum drug discovery market',
    growthRate: '500% annual growth'
  },

  // AUTONOMOUS SYSTEMS & ROBOTICS
  {
    id: 'autonomous-logistics-platform',
    name: 'Autonomous Logistics Platform',
    tagline: 'AI-powered supply chain optimization and automation',
    price: '$1,299',
    period: '/month',
    description: 'Intelligent logistics platform that optimizes supply chains, predicts demand, and automates warehouse operations. Features autonomous vehicles, drone delivery, and predictive analytics.',
    features: [
      'AI demand forecasting',
      'Autonomous vehicle management',
      'Drone delivery optimization',
      'Warehouse automation',
      'Route optimization',
      'Real-time tracking',
      'Predictive maintenance',
      'Sustainability metrics',
      'Multi-warehouse management',
      'API integration'
    ],
    popular: true,
    icon: '🚚',
    color: 'from-green-500 to-emerald-600',
    textColor: 'text-green-400',
    link: 'https://ziontechgroup.com/autonomous-logistics-platform',
    marketPosition: 'Industry leader in autonomous logistics with 40% cost reduction',
    targetAudience: 'E-commerce companies, logistics providers, manufacturers, retailers, supply chain managers',
    trialDays: 21,
    setupTime: '8 hours',
    category: 'Logistics & Supply Chain',
    realService: true,
    technology: ['AI/ML', 'IoT', 'Autonomous Vehicles', 'Predictive Analytics', 'Blockchain'],
    integrations: ['ERP Systems', 'WMS', 'TMS', 'E-commerce Platforms', 'Shipping APIs'],
    useCases: ['E-commerce fulfillment', 'Manufacturing logistics', 'Retail distribution', 'Cold chain logistics', 'Last-mile delivery'],
    roi: '600% average ROI in 18 months',
    competitors: ['Flexport', 'Convoy', 'Uber Freight', 'Amazon Logistics'],
    marketSize: '$2.5B autonomous logistics market',
    growthRate: '280% annual growth'
  },
  {
    id: 'autonomous-energy-management',
    name: 'Autonomous Energy Management',
    tagline: 'AI-powered energy optimization and grid management',
    price: '$1,099',
    period: '/month',
    description: 'Intelligent energy management platform that optimizes power consumption, manages renewable energy sources, and automates grid operations. Reduces energy costs by up to 40%.',
    features: [
      'Smart grid optimization',
      'Renewable energy management',
      'Demand response automation',
      'Energy storage optimization',
      'Real-time monitoring',
      'Predictive maintenance',
      'Carbon reduction tracking',
      'Multi-site management',
      'API integration',
      'Custom energy models'
    ],
    popular: true,
    icon: '⚡',
    color: 'from-amber-500 to-yellow-600',
    textColor: 'text-amber-400',
    link: 'https://ziontechgroup.com/autonomous-energy-management',
    marketPosition: 'Leading energy AI platform with 40% cost reduction guarantee',
    targetAudience: 'Utility companies, industrial facilities, commercial buildings, renewable energy providers, smart cities',
    trialDays: 21,
    setupTime: '5 hours',
    category: 'Energy & Sustainability',
    realService: true,
    technology: ['AI/ML', 'IoT', 'Smart Grid', 'Energy Analytics', 'Predictive Modeling'],
    integrations: ['SCADA Systems', 'Smart Meters', 'Renewable Energy Systems', 'Building Management', 'Grid APIs'],
    useCases: ['Grid optimization', 'Building energy management', 'Renewable integration', 'Demand response', 'Energy trading'],
    roi: '700% average ROI in 18 months',
    competitors: ['GE Digital', 'Siemens Energy', 'Schneider Electric', 'ABB Ability'],
    marketSize: '$2.2B smart energy market',
    growthRate: '240% annual growth'
  },

  // EMERGING TECHNOLOGIES
  {
    id: 'metaverse-development-studio',
    name: 'Metaverse Development Studio',
    tagline: 'Build immersive 3D worlds and experiences',
    price: '$799',
    period: '/month',
    description: 'Complete metaverse development platform with VR/AR integration, 3D modeling, and immersive experiences. Features real-time collaboration, AI-powered content generation, and cross-platform deployment.',
    features: [
      '3D world building tools',
      'VR/AR integration',
      'AI content generation',
      'Real-time collaboration',
      'Cross-platform deployment',
      'Avatar customization',
      'Social features',
      'Monetization tools',
      'Analytics dashboard',
      'API for developers'
    ],
    popular: true,
    icon: '🌐',
    color: 'from-blue-500 to-indigo-600',
    textColor: 'text-blue-400',
    link: 'https://ziontechgroup.com/metaverse-development-studio',
    marketPosition: 'Leading metaverse development platform with 10,000+ active worlds',
    targetAudience: 'Game developers, brands, educators, event organizers, real estate companies',
    trialDays: 14,
    setupTime: '4 hours',
    category: 'Metaverse & VR/AR',
    realService: true,
    technology: ['3D Graphics', 'VR/AR', 'AI/ML', 'Real-time Rendering', 'Blockchain'],
    integrations: ['Unity', 'Unreal Engine', 'VR Headsets', 'Social Platforms', 'Payment Systems'],
    useCases: ['Virtual events', 'Gaming worlds', 'Virtual real estate', 'Educational experiences', 'Brand activations'],
    roi: '500% average ROI in 12 months',
    competitors: ['Roblox', 'Decentraland', 'The Sandbox', 'VRChat'],
    marketSize: '$1.9B metaverse market',
    growthRate: '280% annual growth'
  },
  {
    id: 'iot-smart-city-platform',
    name: 'IoT Smart City Platform',
    tagline: 'Transform cities with intelligent IoT solutions',
    price: '$1,099',
    period: '/month',
    description: 'Comprehensive IoT platform for smart city management, traffic optimization, and environmental monitoring. Features real-time data collection, AI analytics, and automated city services.',
    features: [
      'Smart traffic management',
      'Environmental monitoring',
      'Waste management optimization',
      'Smart lighting systems',
      'Public safety monitoring',
      'Energy efficiency tracking',
      'Citizen engagement tools',
      'Data analytics dashboard',
      'API for developers',
      'Custom IoT solutions'
    ],
    popular: true,
    icon: '🏙️',
    color: 'from-cyan-500 to-blue-600',
    textColor: 'text-cyan-400',
    link: 'https://ziontechgroup.com/iot-smart-city-platform',
    marketPosition: 'Leading smart city platform with 500+ city deployments',
    targetAudience: 'City governments, urban planners, infrastructure companies, technology providers, municipalities',
    trialDays: 21,
    setupTime: '6 hours',
    category: 'IoT & Smart Cities',
    realService: true,
    technology: ['IoT Sensors', 'AI/ML', 'Big Data', 'Cloud Computing', 'Edge Computing'],
    integrations: ['Traffic Systems', 'Weather APIs', 'Government Systems', 'Utility Networks', 'Mobile Apps'],
    useCases: ['Traffic optimization', 'Environmental monitoring', 'Public safety', 'Energy management', 'Citizen services'],
    roi: '600% average ROI in 18 months',
    competitors: ['Cisco Smart Cities', 'Siemens City Performance', 'IBM Smarter Cities', 'Microsoft CityNext'],
    marketSize: '$2.8B smart city market',
    growthRate: '220% annual growth'
  }
];

export const serviceCategories = [
  'AI & Content Creation',
  'AI & Media',
  'Healthcare & AI',
  'Climate Tech & AI',
  'Blockchain & Web3',
  'DeFi & Cryptocurrency',
  'Cybersecurity & Quantum',
  'Financial Technology',
  'Biotechnology & Quantum',
  'Logistics & Supply Chain',
  'Energy & Sustainability',
  'Metaverse & VR/AR',
  'IoT & Smart Cities'
];

export const getServicesByCategory = (category: string) => {
  return enhancedRealMicroSaasServices.filter(service => service.category === category);
};

export const getPopularServices = () => {
  return enhancedRealMicroSaasServices.filter(service => service.popular);
};

export const getServicesByPriceRange = (min: number, max: number) => {
  return enhancedRealMicroSaasServices.filter(service => {
    const price = parseFloat(service.price.replace('$', '').replace(',', ''));
    return price >= min && (max === Infinity || price <= max);
  });
};