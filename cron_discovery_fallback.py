...
LOG_OUT = os.path.join(BASE, "logs", "lead_gen_cron_fallback.log")
...
results = []
seen_domains = set()
emails_blacklist = ['example.com','noreply','mailer-daemon','.png','.jpg','.svg','sentry','webpack','no-reply','donotreply']
        ...
        try:
            ...
            for e in found:
                ...
                if domain_part in seen_domains:
                    continue
                bad = any(x in e for x in emails_blacklist)
                if bad:
                    continue
                emails.add(e)
            time.sleep(2)
...
def startpage_search(query):
    r = requests.get("https://www.startpage.com/do/dsearch", params={"query": query}, timeout=25, headers={"User-Agent": UA})
    return r.status_code, r.text

start = time.time()
log = []
log.append(f"=== Fallback Lead discovery start {datetime.now().isoformat()} ===")
blocked = False
for q in queries:
    if blocked:
        log.append(f"Blocked provider skip for query: {q}")
        continue
    status, html = startpage_search(q)
    ...
    for a in soup.find_all('a', href=True):
        ...
        if not href.startswith('http'):
            continue
        keywords = ['msp','it support','managed it','it services','email deliverability','migration','backup','wi-fi','wireless','tech','computer','network']
        if any(t in txt.lower() for t in keywords):
...
log.append(f"Fallback discovery finished in {round(time.time()-start,1)}s | unique leads={len(results)}")
