...
function parseMarkdown(filePath: string): { meta: PageMeta; content: string } {
  const raw = fs.readFileSync(filePath, utf8');  const match = raw.match(/---\n([\s\S]+?)\n---\n([\s\S]*)/);
...
const StaticPage: React.FC<PageProps> = ({ content, meta }) => {
  ...
  return (
    <>
      ...
      <main className="prose dark:prose-invert max-w-3xl mx-auto py-8">"        <ReactMarkdown>{content}</ReactMarkdown>"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      </main>
    </>
  );
};
const _getStaticPaths: GetStaticPaths = async () => {
  const dir = path.join(process.cwd(), content', pages');  const files = fs.readdirSync(dir).filter((f) => f.endsWith('.md'));
  ...
  const paths = files
    .map((f) => ({ params: { slug: f.replace(/\.md$/, ) } }))    .filter(p => !reservedSlugs.includes(p.params.slug));

  return { paths, _fallback: false };
};
const getStaticProps: GetStaticProps<PageProps> = async ({ params }: { params?: { slug?: string } }) => {
  const slug = params?.slug as string;
  const filePath = path.join(process.cwd(), content', pages', `${slug}.md`);  if (!fs.existsSync(filePath)) {
