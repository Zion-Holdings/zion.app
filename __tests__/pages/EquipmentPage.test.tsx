...
jest.mock('@/components/DynamicListingPage', () => ({'  DynamicListingPage: (props: any) => (
...
;
const renderWithProviders = (ui: React.ReactElement, route = '/equipment') => {'  return render(
...
describe('EquipmentPage', () => {'  beforeEach(() => {
  ...
  it('renders equipment listings when API returns data', async () => {'    const mockEquipment = [
      { id: 'eq1', title: 'Super Server X', price: 5000, category: 'Servers', images: ['server.jpg'], description: 'A powerful server.' },'      { id: 'eq2', title: 'Network Switch Pro', price: 800, category: 'Networking', images: ['switch.jpg'], description: 'High-speed network switch.' },'    ];
    (apiClient.get as jest.Mock).mockResolvedValue({ data: mockEquipment });

    renderWithProviders(<EquipmentPage />);

    await waitFor(() => {
      expect(screen.getByText('Super Server X')).toBeInTheDocument();'      expect(screen.getByText('Network Switch Pro')).toBeInTheDocument();'      expect(screen.getAllByTestId('equipment-item')).toHaveLength(2);'    });
    expect(apiClient.get).toHaveBeenCalledWith('/equipment');'  });

  it('renders empty state when API returns no equipment', async () => {'    (apiClient.get as jest.Mock).mockResolvedValue({ data: [] });
  ...
  it('renders error state when API call fails', async () => {'    const errorMessage = 'Network Error';'    (apiClient.get as jest.Mock).mockRejectedValue(new Error(errorMessage));
  ...
  it('shows loading skeletons initially', async () => {'    (apiClient.get as jest.Mock).mockImplementation(() => new Promise(() => {})); // Never resolves
    ...
    expect(screen.getByTestId('loading-state-equipment-container')).toBeInTheDocument();'    // Then check for specific skeletons within it
    expect(screen.getAllByTestId('skeleton-card').length).toBeGreaterThan(0);'    expect(screen.getByTestId('filter-sidebar-skeleton')).toBeInTheDocument();'  });

  it('renders error state and hides skeletons on API timeout', async () => {'    const timeoutError = new Error('timeout of 15000ms exceeded');'    // Simulating Axios timeout error structure
    (timeoutError as any).code = 'ECONNABORTED';'    (apiClient.get as jest.Mock).mockRejectedValueOnce(timeoutError);

    renderWithProviders(<EquipmentPage />);

    await waitFor(() => {
      // Check that the specific loading state for equipment is gone
      expect(screen.queryByTestId('loading-state-equipment-container')).not.toBeInTheDocument();'      // Check that skeleton cards are no longer rendered
    ...
    expect(toast).toHaveBeenCalledWith(expect.objectContaining({
      title: 'timeout of 15000ms exceeded','      variant: 'destructive','    }));
  });
});
