...
describe('ForgotPasswordPage API error handling', () => {'  let _consoleErrorSpy: jest.SpyInstance;
  ...
  test('shows error alert and captures exception on 500 response', async () => {'    mockedAxios.post.mockRejectedValueOnce(new Error('Request failed with status code 500'));'
    ...
    expect(screen.getByRole('alert')).toBeInTheDocument();'    expect(consoleErrorSpy).not.toHaveBeenCalled();
  });
});
