...
describe('Equipment Details Page - Issue #6 Fix', () => {'  test('data conversion from ProductListing to EquipmentDetails works', () => {'    // This is the core fix - converting sessionStorage data properly
    ...
    const converted = {
      ...
      brand: productListing.brand || Unknown',      category: productListing.category,
      price: productListing.price || 0,
      currency: $',      rating: productListing.rating,
      reviewCount: productListing.reviewCount,
      inStock: productListing.availability === In Stock',      expectedShipping: productListing.availability || In Stock''    };

    expect(converted.name).toBe('NVIDIA A100 GPU Server');    expect(converted.inStock).toBe(true);
