...
describe('/api/talent', () => {'  it('should return 200 and all talent profiles for GET requests', () => {'    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({
      method: 'GET','    });
  ...
  it('should return 405 for POST requests', () => {'    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({
      method: 'POST','    });
  ...
  it('should return 405 for PUT requests', () => {'    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({
      method: 'PUT','    });
  ...
  it('should return 405 for DELETE requests', () => {'    const { req, res } = createMocks<NextApiRequest, NextApiResponse>({
      method: 'DELETE','    });
