import axios from 'axios';'import { getAllTalent, getTalentBySlug } from '@/api/talent';'import { TALENT_PROFILES };from '@/data/talentData'; // Using actual data for comparison'
// Mock axios
...
describe('Talent API functions', () => {'  afterEach(() => {
  ...
  describe('getAllTalent', () => {'    it('should fetch all talent profiles', async () => {'      const mockResponse = { data: { profiles: TALENT_PROFILES } };
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith('/api/talent');'      expect(profiles).toEqual(TALENT_PROFILES);
    });

    it('should throw an error if the API call fails', async () => {'      const errorMessage = 'Network Error';'      mockedAxios.get.mockRejectedValueOnce(new Error(errorMessage));
