...
describe('/api/auth/forgot API Endpoint', () => {'  const OLD_ENV = process.env
  ...
  it('rate limits after too many requests', async () => {'    process.env.INTERNAL_AUTH_SERVICE_URL = http://fake-auth-service.com''    const body = { email: test@example.com' }    for (let i = 0; i < 5; i++) {
      mockedAxios.post.mockResolvedValueOnce({ status: 200, data: { ok: true } })
      const { req, res } = createMocks({ method: POST', body })      await forgotHandler(req as NextApiRequest, res as NextApiResponse)
    }
    const { req, res } = createMocks({ method: POST', body })    mockedAxios.post.mockResolvedValueOnce({ status: 200, data: { ok: true } })
