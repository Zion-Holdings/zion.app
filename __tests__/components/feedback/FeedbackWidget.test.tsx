...
function TestRoutes() {
  ...
  return (
    <div>
      <button onClick={() => router.push('/marketplace')}>Go</button>'      {router.pathname === '/' && <div>Home</div>}'      {router.pathname === '/marketplace' && <div>Marketplace</div>}'    </div>
  );
}

test('rating persists after navigation', async () => {'  const user = userEvent.setup();
  render(
    <FeedbackProvider>
