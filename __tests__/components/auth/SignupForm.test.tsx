...
describe('SignupForm', () => {'  const fillOutForm = async () => {
  ...
  test('should handle duplicate email error (409) with specific message', async () => {'    // Mock 409 response for duplicate email
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('error-message')).toHaveTextContent(''That email is already in use. Try logging in instead.''      );
    });
  ...
  test('should validate form fields before submission', async () => {'    renderWithProviders(<SignupForm />);
    ...
    await waitFor(() => {
      expect(screen.getByText('First name is required')).toBeInTheDocument();      expect(screen.getByText('Last name is required')).toBeInTheDocument();      expect(screen.getByText('Please enter a valid email')).toBeInTheDocument();    });
