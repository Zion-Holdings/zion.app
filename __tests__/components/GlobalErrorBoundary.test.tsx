...
describe('GlobalErrorBoundary', () => {'  // Test Case 1: Renders children when there is no error
  ...
  it('catches an error and renders the fallback UI with error details', () => {'    const ErrorComponent = () => {
    ...
    expect(screen.getByText('Oops! Something went wrong.')).toBeInTheDocument();'    expect(screen.getByText('We apologize for the inconvenience.')).toBeInTheDocument();'    expect(screen.getByText('Our team has been notified and is working to fix the issue.')).toBeInTheDocument();'
    // Check for error details (assuming it's displayed)'    // Open the details section first if necessary
    const detailsSummary = screen.getByText('Error Details');'    fireEvent.click(detailsSummary);
    expect(screen.getByText('Test error')).toBeInTheDocument();'  });

  // Test Case 3: "Reload" button in fallback UI attempts to refresh the page"  it('"Reload" button in fallback UI attempts to refresh the page', () => {'    const ErrorComponent = () => {
    ...
    const reloadButton = screen.getByRole('button', { name: /Reload/i });'    fireEvent.click(reloadButton);
