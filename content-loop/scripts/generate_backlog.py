...
def seed_backlog() -> list[dict]:
    ...
    return [{"title": s["title"], "slug": slugify(s["title"]), "cluster": s["cluster"], "intent": s["intent"], "differentiation_hook": s["differentiation_hook"], "rationale": s["rationale"]} for s in seeds]


def merge_backlog() -> list[dict]:
    current_topics = load_json(TOPICS_FILE)
