...
**2026-09-03 apex + money pages:** Pages API `cname=ziontechgroup.com` (cert approved). **Source must stay `legacy` / `gh-pages` / `/`.** GitHub Pages serves the **`public/`** subtree when it exists. Recurrence: live `/` became “Measurable Growth” again at `2026-09-03T01:36:37Z` (5318 bytes) while workspace Discovery is 5944 bytes. Always upsert honest pages into **both** `/` and `public/`. Also overwrite leftover Next.js aliases that still 200: `/managed-it-services/`, `/partner-program/`, `/affiliate-program/`, `/free-tools-hub/` (fake 4.8/200 + free consult), `/composio-automation-stack/` (15-app fiction + HubSpot), `/services/ai-automation/` (old `ZG04` Stripe + $149/mo), `/integrations/composio/`, `/about/` (stale `calendly.com/ziontechgroup/30min`).
