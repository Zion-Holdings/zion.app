...
**2026-09-03T11:21Z:** Merged `origin/main` `02d219e88bc4` on PR 71127. Kept Discovery `$99` homepage and no-op `mirror-to-gh-pages.yml`. Disabled incoming `composio-account-factory.yml` (`if: false` — it mints Connect Links for already-ACTIVE Stripe/GitHub/Notion/etc). 1Password 6h cron is dispatch-only (0 vaults). kleber@ still 0 ACTIVE; HubSpot INITIALIZING (competing remints ~every 2 min — do not remint). Jira/noCRM/Perplexity INITIATED. Stripe $0; Calendly 0 upcoming; 1Password 0 vaults; Cloudflare 0 zones. Competing `gh-pages` commit `5a68f9f0` (“restore 3 broken doc routes”) overwrote `/industries/financial-services/` and added `/public-roadmap/` + `/status-page/` stubs linking `/contact/` — convert those to honest closers. Workflow **348274365** still `disabled_manually`. Live apex still Discovery `$99`.
