...
- **GitHub** authenticated user has 3 repos: public `zion-support.github.io`, private `telegram-ai-reply-worker` (Wrangler `telegram-ai-reply-worker`, OpenRouter `openai/gpt-4o-mini`, Durable Object `STATE`, Telegram webhook only), private `telegram-agent-listener` (Python webhook). Workers Builds on the Pages repo is the wrong hook. OpenRouter overage means the worker cannot generate replies. **Ruleset 22178533** (`non_fast_forward` + `deletion`, bypass never) protects `gh-pages` — never force-push that branch. Restore leftover Next.js with `restore-discovery-on-gh-pages.mjs` (fast-forward upsert of `PIN_PATHS`). `pages.yml` must stay `force: false` / `keep_files: true` or the deploy fights the ruleset.
