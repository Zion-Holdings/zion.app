...
- **GitHub** authenticated user has 3 repos: public `zion-support.github.io`, private `telegram-ai-reply-worker` (Wrangler `telegram-ai-reply-worker`, OpenRouter `openai/gpt-4o-mini`, Durable Object `STATE`, Telegram webhook only), private `telegram-agent-listener` (Python webhook). Workers Builds on the Pages repo is the wrong hook. OpenRouter overage means the worker cannot generate replies.
