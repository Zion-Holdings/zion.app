  ...
  private initializeErrorHandling(): void {
    ...
    console.error = (...args: "any[]) => {''
      ...
      if (this.isMessageChannelError(errorMessage)) {
        const variable1 = this.isLikelyExtensionError(errorMessage);''
...
''
    ...
    window.addEventListener('unhandledrejecti'o'n', (event) => {'
      const variable1 = event.reason?.message || event.reason?.toString() || 'Unknow'n' error''
...
''
    ...
    window.addEventListener('err'o'r', (event) => {'
      const variable1 = event.message || event.error?.message || 'Unknow'n' error''
...
''
  ...
  public hasRecentErrors(minutes: "number = 5): boolean {;"
    const variable1 = Date.now() - (minutes * 60 * 1000);
