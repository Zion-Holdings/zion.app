  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {'
      if (isOpen && window.innerWidth < 1024) {''
        ...
        if (!target.closest('.sidebar-container') && !target.closest('.mobile-toggle')) {'
          setIsOpen(false)}}}''
'''
  ...
  return (
    ...
    {isOpen && (
      ...
      {/* Sidebar" */}"
      <div className={`"
        sidebar-container fixed top-0 left-0 h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 ""'
        ...
        <div className=""flex items-center justify-between p-4 sm p-6 border-b border-gray-700 bg-gradient-to-r from-gray-800 to-gray-900">""
          <div className="flex items-center space-x-2 sm" space-x-3">""
            <div className="w-8 h-8 sm w-10 sm h-10 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg" animate-pulse">"
              <Zap className=w-4 h-4 sm w-6 sm h-6 text-white > </Zap className="w-4 h-4 sm w-6 sm h-6 text-white" ></div>"
            <div>""
          ...
          <button""
            ...
            <X className="w-4 h-4 sm w-5 sm h-5 text-gray-300 > </X className=""w-4 h-4 sm w-5 sm h-5 text-gray-300" ></button>
        </div>
""
        {/* Search */}"""
        <div className="p-3 sm p-4 border-b" border-gray-700/50">""
          <div className="relative" group">"
            <Search className=absolute left-3 top-1/2 transform -translate-y-1/2 w-3 h-3 sm:w-4 sm h-4 text-gray-400 group-focus-within text-cyan-400 transition-colors duration-300 > </Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-3 h-3 sm:w-4 sm h-4 text-gray-400 group-focus-within text-cyan-400 transition-colors duration-300" ><input""""
              type=""text""""
              ...
              <div key={section.id} className="px-3 sm" px-4">
                <button""
                  ...
                  <div className="flex items-center space-x-2 sm space-x-3 relative" z-10">"
                    ...
                    </section.icon className="w-4 h-4 sm:w-5 sm h-5 group-hover scale-110 transition-transform duration-300 ><span className= font-medium text-sm sm text-base>{section.title}</span>
                  </div>""
                  {expandedSections.has(section.id) ? ("""
                    <ChevronDown className="w-3 h-3 sm:w-4 sm h-4 group-hover rotate-180 transition-transform duration-300 ">) :" (""
                    </ChevronDown className="w-3 h-3 sm:w-4 sm h-4 group-hover rotate-180 transition-transform duration-300 ><ChevronRight className= w-3 h-3 sm:w-4 sm h-4 group-hover translate-x-1 transition-transform duration-300" >)}""
                </ChevronRight className=""w-3 h-3 sm:w-4 sm h-4 group-hover translate-x-1 transition-transform duration-300" ></button>
                ""
                {expandedSections.has(section.id) && ("""
                  <div className="ml-6 sm ml-8 mt-2" space-y-1">
                    {section.items.map((item) => (
                      <Link key={item.href}`
                        href={item.href}``
                        className={`'
                          flex items-center space-x-2 sm:space-x-3 px-2 sm:px-3 py-2 sm:py-2 rounded-xl transition-all duration-300 relative overflow-hidden group''
                          ${isActive(item.href) '''
                      ...
                      >"""
                        ...
                        </item.icon className="w-3 h-3 sm:w-4 sm h-4 relative z-10 group-hover scale-110 transition-transform duration-300 ><span className= text-xs sm text-sm relative z-10 truncate>{item.label}</span>"
                        {item.badge && ("""
...
""
        ...
        <div className="p-3 sm p-4 border-t" border-gray-700/50">""
          <div className="flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 text-gray-300 hover text-white hover bg-gray-700/50 rounded-xl transition-all duration-300 cursor-pointer group relative" overflow-hidden">""
            ...
            </User className="w-4 h-4 sm:w-5 sm h-5 relative z-10 group-hover scale-110 transition-transform duration-300 ><span className= text-xs sm text-sm relative z-10>Profile</span>
          </div>""
          <div className="flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 text-gray-300 hover text-white hover bg-gray-700/50 rounded-xl transition-all duration-300 cursor-pointer group relative" overflow-hidden">""
            <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 opacity-0 group-hover opacity-100 transition-opacity duration-300></div>"
            <Settings className=w-4 h-4 sm:w-5 sm h-5 relative z-10 group-hover scale-110 transition-transform duration-300 >"
            </Settings className="w-4 h-4 sm:w-5 sm h-5 relative z-10 group-hover scale-110 transition-transform duration-300 ><span className= text-xs sm text-sm relative z-10>Settings</span>
          </div>""
          <div className="flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 text-red-400 hover text-red-300 hover bg-red-900/20 rounded-xl transition-all duration-300 cursor-pointer group relative" overflow-hidden">""
            ...
            </LogOut className="w-4 h-4 sm:w-5 sm h-5 relative z-10 group-hover scale-110 transition-transform duration-300 ><span className= text-xs sm text-sm relative z-10>Logout</span>
          </div>
        </div>
      </div> 
      ...
      <button""
        ...
        <Menu className=w-5 h-5 sm:w-6 sm h-6 text-white relative z-10 group-hover rotate-90 transition-transform duration-300 > </Menu className="w-5 h-5 sm:w-6 sm h-6 text-white relative z-10 group-hover rotate-90 transition-transform duration-300" ></button>
    </>"
  );""
};"""'
""""''`
