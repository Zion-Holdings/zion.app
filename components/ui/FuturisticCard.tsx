...
interface FuturisticCardProps {
  ...
  link: string;
  category?: string;
  color?: 'bl'u'e | purp'l'e | p'i'nk' | 'green";
  featured?: boolean;}
const FuturisticCard: React.FC<FuturisticCardProps> = ({
  ...
  return (
    ...
    {category && (``
        ...
        {/* Icon */},
    {icon && ("
          <div className= text-2xl sm:text-3xl lg text-4xl mb-3 sm mb-4 animate-cyber-float> {icon}
          </div>
        )}
        {/* Title */}
        <h3 className=" text-sm sm:text-base lg:text-xl font-bold text-white mb-2 sm:mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover from-neon-blue group-hover to-neon-purple transition-all duration-300> {title}
        </h3>

        {/* Description */}"
        <p className= text-gray-400 text-xs sm text-sm mb-3 sm mb-4 line-clamp-3> {description}
        </p>

        {/* Price and Rating */}
        <div className=" flex items-center justify-between>`
          {price && (``
            <div className="{`${currentColor.text} font-bold text-sm sm:text-base lg:text-lg`}> {price}
            </div>
          )}
          {rating && (
