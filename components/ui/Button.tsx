...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  const variantClasses = {
    primary: 'bg-cyan-600 text-white hover:bg-cyan-700 focus:ring-cyan-500 active:bg-cyan-800',
      ...
      {React.Children.map(children, (child, index) => {
        if (React.isValidElement(child)) {
          ...
          if (orientation === 'horizontal') {
            ...
            if (!isFirst) roundedClasses += ' -ml-px';
          } else {
            roundedClasses = isFirst ? 'rounded-b-none' : isLast ? 'rounded-t-none' : 'rounded-none';
            if (!isFirst) roundedClasses += ' -mt-px';
          }
          
          return React.cloneElement(child as React.ReactElement<{ className?: string }>, {
            className: `${(child as React.ReactElement<{ className?: string }>).props.className || ''} ${roundedClasses}`.trim()
          });
        }
        return child;
      })}
...
export const ToggleButton = forwardRef<HTMLButtonElement, ToggleButtonProps>(({
  ...
  const currentVariant = pressed ? pressedVariant : 'outline';
...
export const SocialButton = forwardRef<HTMLButtonElement, ButtonProps & {
  ...
  const providerStyles = {
    ...
    twitter: 'bg-blue-400 text-white hover:bg-blue-500 focus:ring-blue-400',
    github: 'bg-gray-800 text-white hover:bg-gray-900 focus:ring-gray-500',
    linkedin: 'bg-blue-700 text-white hover:bg-blue-800 focus:ring-blue-600'
  };
