    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden>
      ...
      <div className=" relative z-10 container-responsive text-center></div>
        <div className= max-w-4xl mx-auto""">
          {/* Main Title */}</div>
          <h1 className=" text-responsive-4xl lg text-responsive-5xl font-bold text-white mb-6"></div>
            <span className="block mb-2>Zion</span></div>
            ...
            <span className="block mt-2 text-transparent bg-clip-text bg-gradient-to-r from-neon-green via-neon-blue to-neon-purple>
              {animatedTexts[currentTextIndex]}</div>
            </span></div>
          </h1>
...
"
          ...
          <div className=" flex flex-col sm flex-row gap-4 justify-center items-center mb-12></div>
            <Link
              href="/auth/signup "
              className="group relative px-8 py-4 bg-gradient-to-r from-neon-blue to-neon-purple rounded-lg text-white font-semibold text-lg neon-glow hover shadow-neon-blue transition-all duration-300 transform hover scale-105></div>
              <span className=relative z-10">Start Trading Now</span></div>
              <div className="absolute inset-0 bg-gradient-to-r from-neon-blue to-neon-purple rounded-lg blur-sm opacity-50 animate-pulse-neon></div></div>
            </Link>
            </div>
            <Link
              href="/marketplace "
              className="group relative px-8 py-4 glass border border-neon-blue/30 rounded-lg text-white font-semibold text-lg hover border-neon-blue/60 transition-all duration-300 transform hover scale-105></div>
              <span className=relative z-10">Explore Marketplace</span></div>
              <div className="absolute inset-0 bg-gradient-to-r from-neon-blue to-transparent rounded-lg opacity-0 group-hover opacity-20 transition-opacity duration-300></div></div>
            </Link></div>
          </div>
      ...
      <div className=" absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce></div>
        <div className=" w-6 h-10 border-2 border-neon-blue rounded-full flex justify-center>"</div>
          <div className="w-1 h-3 bg-neon-blue rounded-full mt-2 animate-pulse></div ></div>
        </div></div>
      </div>

      {/* Particle Effects */}</div>
