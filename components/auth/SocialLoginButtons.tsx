...
interface $1 {
  onSocialLogin: (provider: 'goog'l'e | gith'u'b | linke'd'in' | 'twitter') => Promise<void>
  ...
  const $1 = ['
    {
      ...
      name: Googl'e',
      icon: (</div>
        <svg className=" w-5 h-5 mr-3 viewBox= 0 0 24 24"></div>
        ...
        <button key={provider.id}
          ...
          {socialLoading === provider.id ? (</div>
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
              ...
              {provider.icon}
              Continue with {provider.name}</div>
            </>
          )}</div>
        </button>
      ))}</div>
...
';}
export default SocialLoginButtons "'"</div>