...
import 'react-datepicke'r/dist/react-datepicker.css";
...
'
    ...
    if (!formData.budgetRange) {
      newErrors.budgetRange = Budge't' range is required;}
    if (!formData.contactEmail?.trim()) {
      newErrors.contactEmail = Emai'l' is required;
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.contactEmail)) {
      newErrors.contactEmail = Pleas'e' enter a valid email address;}
    setErrors(newErrors);
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className= block text-sm font-medium text-gray-700 mb-2">
                      <FileText className="w-4 h-4 inline mr-2 >Project Description *
                    </FileText className=w-4 h-4 inline mr-2" "></label>
                    <textarea'
                      value={formData.projectDescription}
                      onChange={(e) => handleInputChange(projectDescriptio'n', e.target.value)}
                      rows={4}
                  ...
                  <div className=" grid grid-cols-1 md grid-cols-2 gap-4">
                    <div>
                      ...
                      </Calendar className="w-4 h-4 inline mr-2 ></label>
                      <DatePicker
                        selected={formData.timeline?.startDate}
                        onChange={(date) => handleInputChange(timeli'n'e, {
                          ...formData.timeline, 
                          startDate: date 
                        })}`
                        minDate={new Date()}"'``
                  ...
                  <div>
                    <label className= block text-sm font-medium text-gray-700 mb-2"">
                      <DollarSign className=w-4 h-4 inline mr-2 ">Budget Range *
                  ...
                  <div>
                    ...
                    </Mail className=w-4 h-4 inline mr-2 "></label>
                    <input
                      type=email"
