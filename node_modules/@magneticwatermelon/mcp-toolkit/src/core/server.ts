...
import { ToolLoader, PromptLoader, ResourceLoader } from "./loaders";
import {
...
import { getProjectName, getProjectVersion, resolveBasePath } from "../utils";
import { loadMCPEnvironment } from "./envLoader";

const require = createRequire(import.meta.url);
export type TransportType = "stdio" | "http-stream";
...
export class MCPServer {
  ...
  constructor(config: MCPServerConfig = {}) {
    this.basePath = resolveBasePath(config.basePath);
    this.serverName = config.name ?? getProjectName(this.basePath);
    this.serverVersion = config.version ?? getProjectVersion(this.basePath);
    this.transportConfig = config.transport ?? { type: "http-stream" };
  ...
  private createTransport(): BaseTransport {
    ...
    // Configure safe logging based on transport type
    const logFile = process.env.MCP_LOG_FILE;
    import("../logger.js").then(({ configureSafeLogging }) => {
      configureSafeLogging(this.transportConfig.type, logFile);
    });
    ...
    switch (this.transportConfig.type) {
      ...
      case "http-stream":
      default:
        if (this.transportConfig.type !== "http-stream") {
          logger.warn(
            `Unsupported type '${this.transportConfig.type}', defaulting to http-stream.`
          );
        }
        ...
        logger.debug(
          `Creating HttpStreamTransport. response mode: ${httpConfig.responseMode}`
        );
    ...
    transport.onerror = (error: Error) => {
      logger.error(
        `Transport (${transport.type}) error: ${error.message}\n${error.stack}`
      );
    };
    return transport;
  }
  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(
      ...
      async (request: any) => {
        ...
        logger.debug(
          `Tool call arguments: ${JSON.stringify(request.params.arguments)}`
        );
    ...
    if (this.capabilities.resources) {
        ...
        async (request: any) => {
          ...
          if (!resource) {
            throw new Error(
              `Unknown resource: ${
                request.params.uri
              }. Available resources: ${Array.from(
      ...
      this.server.setRequestHandler(
        ...
        async () => {
          ...
          logger.debug(
            `Sending ListResourceTemplates response: ${JSON.stringify(
              response
            )}`
          );
          return response;
  ...
  private getSdkVersion(): string {
    try {
      const sdkSpecificFile = require.resolve(
  ...
  async start() {
    try {
      ...
      logger.info(
        `Starting MCP server: (Framework: ${frameworkVersion}, SDK: ${sdkVersion})...`
      );

      // Load environment variables first
      const envResult = await loadMCPEnvironment(this.basePath);
      if (!envResult.success) {
        throw new Error(
          `Environment validation failed: ${envResult.error || "Unknown error"}`
        );
      }

      if (envResult.hasConfig) {
        logger.info(
          `Environment loaded: ${envResult.appliedCount} variables applied`
        );
      }

      // Load all tools, prompts, and resources
      ...
      for (const tool of tools) {
        if ("validate" in tool && typeof tool.validate === "function") {
          try {
            ...
            logger.error(
              `Tool validation failed for '${tool.name}': ${error.message}`
            );
            throw new Error(
              `Tool validation failed for '${tool.name}': ${error.message}`
            );
      ...
      logger.info(
        `Capabilities detected: ${JSON.stringify(this.capabilities)}`
      );
      ...
      logger.debug(
        `SDK Server instance created with capabilities: ${JSON.stringify(
          this.capabilities
        )}`
      );
      ...
      logger.info(
        `Connecting transport (${this.transport.type}) to SDK Server...`
      );
      ...
      if (this.capabilities.prompts) {
        logger.info(
          `Prompts (${prompts.length}): ${
            prompts.map((p) => p.name).join(", ") || "None"
          }`
        );
      }
      if (this.capabilities.resources) {
