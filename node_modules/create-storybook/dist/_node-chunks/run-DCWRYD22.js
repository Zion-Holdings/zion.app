...
import {
  initiate
} from "./chunk-74Q7I2WQ.js";
import "./chunk-WFMAWW5S.js";
import {
  __commonJS,
  __require,
  __toESM
} from "./chunk-XQNML7MH.js";
  ...
  "../../../node_modules/commander/lib/help.js"(exports) {
    var { humanReadableArgName } = require_argument(), Help2 = class {
      ...
      visibleCommands(cmd) {
        ...
        return helpCommand && !helpCommand._hidden && visibleCommands.push(helpCommand), this.sortSubcommands && visibleCommands.sort((a2, b2) => a2.name().localeCompare(b2.name())), visibleCommands;
      }
      /**
...
var require_suggestSimilar = __commonJS({
  "../../../node_modules/commander/lib/suggestSimilar.js"(exports) {
    function editDistance(a2, b2) {
      ...
      for (let j2 = 1; j2 <= b2.length; j2++)
        for (let i2 = 1; i2 <= a2.length; i2++) {
          let cost = 1;
          a2[i2 - 1] === b2[j2 - 1] ? cost = 0 : cost = 1, d2[i2][j2] = Math.min(
            d2[i2 - 1][j2] + 1,
            // deletion
            d2[i2][j2 - 1] + 1,
            // insertion
            d2[i2 - 1][j2 - 1] + cost
            // substitution
          ), i2 > 1 && j2 > 1 && a2[i2 - 1] === b2[j2 - 2] && a2[i2 - 2] === b2[j2 - 1] && (d2[i2][j2] = Math.min(d2[i2][j2], d2[i2 - 2][j2 - 2] + 1));
        }
    ...
    function suggestSimilar(word, candidates) {
      ...
      }), similar.sort((a2, b2) => a2.localeCompare(b2)), searchingOptions && (similar = similar.map((candidate) => `--${candidate}`)), similar.length > 1 ? `
...
-  already used by option '${matchingOption.flags}'`);
      ...
      _registerCommand(command) {
        ...
        this._initCommandGroup(command), this.commands.push(command);
      }
      /**
       * Add an option.
       *
 ...
 - ${executableDirMessage}`;
      ...
      _checkNumberOfArguments() {
        this.registeredArguments.forEach((arg, i2) => {
          arg.required && this.args[i2] == null && this.missingArgument(arg.name());
      ...
      _processArguments() {
        ...
        this.registeredArguments.forEach((declaredArg, index) => {
          ...
          declaredArg.variadic ? index < this.args.length ? (value = this.args.slice(index), declaredArg.parseArg && (value = value.reduce((processed, v2) => myParseArg(declaredArg, v2, processed), declaredArg.defaultValue))) : value === void 0 && (value = []) : index < this.args.length && (value = this.args[index], declaredArg.parseArg && (value = myParseArg(declaredArg, value, declaredArg.defaultValue))), processedArgs[index] = value;
      ...
      parseOptions(args) {
        ...
        ) : !1, activeVariadicOption = null, activeGroup = null, i2 = 0;
        for (; i2 < args.length || activeGroup; ) {
          let arg = activeGroup ?? args[i2++];
          if (activeGroup = null, arg === "--") {
            dest === unknown && dest.push(arg), dest.push(...args.slice(i2));
            break;
          }
          ...
          if (activeVariadicOption = null, maybeOption(arg)) {
            ...
            if (option) {
              if (option.required) {
                ...
                let value = null;
                i2 < args.length && (!maybeOption(args[i2]) || negativeNumberArg(args[i2])) && (value = args[i2++]), this.emit(`option:${option.name()}`, value);
              } else
          ...
          if (dest === operands && maybeOption(arg) && !(this.commands.length === 0 && negativeNumberArg(arg)) && (dest = unknown), (this._enablePositionalOptions || this._passThroughOptions) && operands.length === 0 && unknown.length === 0) {
            if (this._findCommand(arg)) {
              operands.push(arg), unknown.push(...args.slice(i2));
              break;
      ...
      opts() {
        if (this._storeOptionsAsProperties) {
          ...
          for (let i2 = 0; i2 < len; i2++) {
            let key = this.options[i2].attributeName();
...
`), this._exit(0, "commander.version", str);
      ...
      outputHelp(contextOptions) {
        ...
        this._getCommandAndAncestors().reverse().forEach((command) => command.emit("beforeAllHelp", eventContext)), this.emit("beforeHelp", eventContext);
...
var createStorybookProgram = program.name("Initialize Storybook into your project.").option(
  ...
  new Option("--features <list...>", "Storybook features").choices(Object.values(Feature)).default(void 0)
).option("--no-features", "Disable all features (overrides --features)").option("--debug", "Get more logs in debug mode").option("--enable-crash-reports", "Enable sending crash reports to telemetry data").option("-f --force", "Force add Storybook").option("-s --skip-install", "Skip installing deps").addOption(
  new Option("--package-manager <type>", "Force package manager for installing deps").choices(
    Object.values(PackageManagerName)
  )
).option("--use-pnp", "Enable pnp mode for Yarn 2+").addOption(
...
).option("-y --yes", "Answer yes to all prompts").addOption(
  new Option("--builder <type>", "Builder library").choices(Object.values(SupportedBuilder))
).option("-l --linkable", "Prepare installation for link (contributor helper)").option(
  "--dev",
...
).option("--agent", "Force agent mode (non-interactive, logs AI setup instructions)").option("--no-agent", "Force disable agent mode even when an AI agent is detected").option(
  "--logfile [path]",
  ...
  let options = self.opts(), resolvedAgent = options.agent ?? c;
  options.debug && logger.setLogLevel("debug"), options.loglevel && logger.setLogLevel(options.loglevel), (options.logfile || resolvedAgent) && logTracker.enableLogWriting();
...
createStorybookProgram.action(async (options) => {
  options.debug = options.debug ?? !1, options.dev = options.dev ?? !1, options.features === !1 && (options.features = []);
  let resolvedAgent = options.agent ?? c;
  if (options.agent = resolvedAgent, resolvedAgent) {
    let agent = a(), agentName = agent ? agent.name : "unknown";
    logger.log(
