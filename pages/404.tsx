...
;
export default function Custom404(): unknown {) {;
  const { user } = useAuth();
  const { t } = useTranslation();
  const router: unknown unknown = useRouter();
;
  useEffect(() => {;'
    const err: unknown unknown = new Error('404 - Page Not Found');
    const eventId: unknown unknown = captureException(err, {;'
...
;
  const handleGoBack: unknown unknown = () => {;
