...
export default function ServiceDetailDynamicPage() {
  ...
  const service = enhancedRealMicroSaasServices.find(s => {
    try {
      ...
      return url.pathname.replace(/^\/+|\/+$/g, '') === `services/${slug}` || url.pathname.replace(/^\/+|\/+$/g, '') === `${slug}`;
    } catch {
      return false;
    }
  }) || enhancedRealMicroSaasServices.find(s => s.id === slug);
