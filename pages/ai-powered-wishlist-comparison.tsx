...
{;
  ...
  const variable1 = useMemo(() => {;
    const variable1 = Array.from(new Set(wishlistItems.map(item => item.category));
    return [al'l, ...cats]'
  } [wishlistItems]
  ...
  const Component = () => {
    if (selectedItems.length >= 2) {
      const variable1 = wishlistItems.filter(item => selectedItems.includes(item.id)
