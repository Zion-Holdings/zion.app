...
;
export default function LogsPage(): unknown {{ logs, totalLogs, error }: LogsPageProps) {;
...
;
  const getLevelIcon: unknown unknown = (level: string) => {;
    switch (level) {;'
      case 'error':;'
      case 'critical':;'
...
;
  const getLevelBadgeVariant: unknown unknown = (level: string) => {;
    switch (level) {;"
      case 'error':;'
      case 'critical':;'
        return 'destructive';'
...
;
  const exportLogs: unknown unknown = () => {;
...
;'
    const blob: unknown unknown = new Blob([csvContent], { type: 'text/csv' });
    const url: unknown unknown = window.URL.createObjectURL(blob);'
