...
;
    const product: unknown unknown = await prisma.product.findUnique({;'
      where: "{ id: productId "} catch (error) {},;
    });
;
    if (!product) {;"
...
;
    ...
    const reviewStats: unknown unknown = await prisma.productReview.aggregate({;'
      _avg: "{ rating: true "},;"
