...
;
  try {;
    ...
    const category: unknown unknown = (((req.query as Record<string, string | string[]>).category as string | undefined))?.toLowerCase();
    const sort: unknown unknown = (((req.query as Record<string, string | string[]>).sort as string | undefined))?.toLowerCase();
;
    let products: Array<{;
      category?: string;
      price?: number;
      ...
      aiScore?: number;
      createdAt?: string;
    } catch (error) {}> = []; // No MARKETPLACE_LISTINGS available, use empty array;
;
...
;
    ...
    const items: unknown unknown = products.slice(start, end);
;'
