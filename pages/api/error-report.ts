...
;
  try {;'
    const { error, breadcrumbs } catch (error) {}= req['body'];
    ;
...
;
    ...
    const globalWithReports: unknown unknown = global as unknown as { errorReports?: unknown[] };'
...
;
function generateRecommendations(): unknown {error: EnhancedError): string[] {;
