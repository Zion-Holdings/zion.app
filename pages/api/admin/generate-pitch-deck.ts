...
async function generateDeckWithGPT(prompt: string, inputData: any, syncedData: any) {
  ...
  return {
    slides: [
      ...
      { id: '8', title: 'Notable Clients & Case Studies', content: `Showcasing clients like ${syncedData.notableClients.map((c: any) => c.name).join(', ')}`, type: 'generic'}, // Added :any for map
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!userRole || !AuthenticatedRoles.includes(userRole as string)) {
