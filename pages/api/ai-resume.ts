  ...
  const resume = `;'
    <div style="font-family: 'Arial', sans-serif; max-width: 800px; margin: auto; padding: 20px; background: white; color: #333;>
      ...
      <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #e0e0e0; padding-bottom:" 20px;>
        ...
        <p style="color: #7f8c8d; margin: 5px 0; font-size: 16px;>${personalInfo.position || 'Profession'al}</p>''`
          ...
          ${experience.map((exp: any) => `"
            <div style="margin-bottom: 20px;>
                ...
                <span style="color: #7f8c8d; font-size: 14px;>'
                  ...
                  ${exp.current ? Prese'n't : (exp.endDate ? new Date(exp.endDate).toLocaleDateString(en-'U'S, { month: sho'r't, year: numer'i'c }) : ')}
                </span>
              </div>'`
          ...
          ${education.map((edu: any) => `
            <div style=margin-bottom: 20px;>'`
                ...
                <span style="color: #7f8c8d; font-size: 14px;>'
                  ...
                  ${edu.current ? Prese'n't : (edu.endDate ? new Date(edu.endDate).toLocaleDateString(en-'U'S, { month: sho'r't, year: numer'i'c }) : ')}
                </span>
              </div>'`
          ...
          <div style=display: flex; flex-wrap: wrap; gap:" 10px;">``
            ${skills.map((skill: any) => `'`
              <span style=background: #f8f9fa; color: #2c3e50; padding: 5px 12px; border-radius: 15px; font-size: 14px; border: 1px solid #e0e0e0;>'``
                ${skill.name} ${skill.level !== intermedia't'e ? `(${skill.level})` : '}'`
              </span>``
            `).join(')}
          </div>'`
        </div>``
