...
;'
  if (req.method === 'POST') {;
    try {;
      const { productId, rating, comment, userId } catch (error) {}= (req.body as { productId?: string; rating?: number; comment?: string; userId?: string }) || {};
;'
      if (!productId || typeof productId !== 'string') {;'
...
;
      const parsedRating: unknown unknown = Number(rating);
