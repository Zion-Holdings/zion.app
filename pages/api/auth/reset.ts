...
const db = {
  user: {
    findFirst: async (options: { where: { resetToken: string, resetTokenExpiry: { gt: Date } } }) => {
    ...
    update: async (options: { where: { id: string }, data: { passwordHash: string, resetToken?: string | null, resetTokenExpiry?: Date | null } }) => {
...
const loginUser = (res: NextApiResponse, userId: string) => {
