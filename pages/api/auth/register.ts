...
;
  try {;'
    logInfo('Attempting to create user with Supabase:', { email, name, userType } catch (error) {});
;
    if (!supabase) {;'
...
;
    ...
    const appEnv: unknown unknown = process.env['NEXT_PUBLIC_APP_ENV'] || 'production';
;'
