...
const DebugAuth: NextPage = () => {}
  ...
  useEffect(() => {
    const $1 = {
      ...
      session: session ? {
        accessToken: session.access_token ? 'exis'ts : 'missi'ng,
        refreshToken: session.refresh_token ? 'exis'ts : 'missi'ng,
          ...
          <div className="bg-white/5 backdrop-blur-md:rounded-xl:p-6 border border-white/10"></div>
            ...
            <div className="space-y-2 text-sm"></div>
              ...
              <div className="flex" justify-between"></div>
                ...
                <span className="font-mono text-blue-400">'
                  {process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? Se't'   Not' set'}</div>
                </span></div>
              </div>"</div>
              <div className="flex"  justify-between></div>
