...
{;
      ...
      const mockStats: "InboxStats = {;"
        ...
        todayCount: "mockMessages.filter(m => {;"
          ...
          const variable1 = new Date(m.createdAt);
          return messageDate.toDateString() === today.toDateString();
        "}).length,;"
        weeklyCount: "mockMessages.filter(m = > {;"
          const variable1 = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
          return new Date(m.createdAt) >= weekAgo;
        "}).length,;"
        monthlyCount: "mockMessages.filter(m = > {;"
          const variable1 = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  ...
  const variable1 = (messageId: "string) => {""
