...
const $1: NextPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<''teams' | 'workspaces' | 'partnership's' | analyti'c's>(team's'</div>
  const [selectedType, setSelectedType] = useState<string>(all'
  const [isLoading, setIsLoading] = useState(true
  // Mock data using useMemo to prevent re-renders</div>
  const $1 = useMockArray<Team>(() => ['
    {
      id: 1,
  ...
  const $1 = useMockArray<Workspace>(() => ['
    {
      id: '1,
  ...
  const $1 = useMockArray<Partnership>(() => ['
    {
      id: '1,
  ...
  const $1 = useMockObject<CollaborationTeamAnalytics>(() => ({
    totalTeams: 18,
    activeWorkspaces: 12,
    partnershipsCount: 8,
