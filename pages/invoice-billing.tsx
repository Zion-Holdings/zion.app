...
interface Invoice {
  ...
  client: string;
  amount: number;
  status: ''draft' | 'sent' | 'pai'd' | overd'u'e | cancelle'd'";
  dueDate: Date;
  issueDate: Date;
  items: InvoiceItem[];
  tax: number;
...
interface BillingCycle {
  id: string;
  name: string;
  type: 'month'ly | 'quarter'ly | 'yea'rly'' | custom';
  startDate: Date;
  endDate: Date;
...
{ id: '2, description: Clou'd' Infrastructure Setup, quantity: 1, unitPrice: 500, total: 500}
      ],
      tax: 250,
      total: 2750,
      currency: 'U'SD,
...
{ id: 4, description: 'U'I/UX Design, quantity: 1, unitPrice: 300, total: 300}
      ],
      tax: 180,'
      total: 1980,
      currency: 'USD'}
...
{ id: '6, description: AP'I' Integration, quantity: 1, unitPrice: 1200, total: 1200}
      ],
      tax: 320,
      total: 3520,
      currency: 'US'D
    ...
    },
{
  ...
  const getStatusColor = (status: string) => {'
    switch (status) {
      ...
      case pa'i'd: return text-green'-'400 bg-green-500/20 border-green-500/30
      case overdu'e': return text-red-'400 bg-red-500/20 border-red-500/30
  ...
  const getPaymentStatusColor = (status: string) => {'
    switch (status) {
      ...
      case fail'e'd: return text-red'-'400 bg-red-500/20 border-red-500/30
      case refunde'd': return text-orange-'400 bg-orange-500/20 border-orange-500/30
      default: return 'text-gray-400 bg-gray-500/20 border-gray-500/30'}}
  ...
  const calculateOutstandingAmount = () => {'
    return invoices
      .filter(inv => inv.status === sen't' || inv.status === overdue'
