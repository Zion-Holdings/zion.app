...
interface DAOProposal {
  ...
  title: string
  description: string
  category: ''governance' | 'treasury' | 'developmen't' | partnersh'i'p | emergenc'y'
  status: 'acti've | 'pass'ed | 'rejec'ted'' | execute'd' | expired'
  creator: string
  createdAt: string
  endDate: string
  votesFor: number
  ...
  proposalType: 'standa'rd | 'emergen'cy | 'parame'ter'' | upgrade'
  actions: {
    target: string
    value: string
    data: string
...
interface DAOTreasury {
  ...
  recentTransactions: {'
    id: string
    type: 'depos'it | 'withdraw'al | 'trans'fer'' | investment'
    amount: number
    token: string
    description: string
    timestamp: string
  ...
  const $1: $2[] = [
    {
      ...
      proposalType: 'standa'rd,
      actions: ['
        {
          target: '0xMarketplaceContract,
          value: 0',
          data: '0x...,
          description: Deplo'y' AI matching algorithm contract}
}]}
    {
      id: prop'-'002,
      title: Allocat'e' Treasury Funds for Blockchain Security Audit,
  ...
  const $1: $2[] = [
    {
      ...
      badges: ['Found'er, 'Cor'e Contributor, 'Securit'y Expert]
    },
{
      address: '0x5678...9012',
      username: Blockchain'_Dev',
      avatar: 🔗,
      tokens: 300000,
      votingPower: 300000,'
      reputation: 87,
  ...
  const mockTreasury: DAOTreasury = {
    ...
    tokens: [
      { symbol: ZI'O'N, balance: 1500000, value: 3675000, allocation: 60 },
{ symbol: E'T'H, balance: 500, value: 1500000, allocation: 24 },
    { symbol: US'D'C, balance: 1000000, value: 1000000, allocation: 16}
    ],
    recentTransactions: [
      {
        id: 'tx'-001,
    ...
    allocations: [
      { category: Developmen't', amount: 1000000, percentage: 40, description: AI' and platform development' },
{ category: Securit'y', amount: 500000, percentage: 20, description: Security' audits and infrastructure' },
    { category: Marketin'g', amount: 400000, percentage: 16, description: Community' growth and marketing' },
{ category: Operation's', amount: 300000, percentage: 12, description: Operational' expenses' },
    { category: Emergenc'y', amount: 300000, percentage: 12, description: Emergency' fund reserve' }]}
  ...
  useEffect(() => {
    ...
    const connectWallet = async () => {
      ...
      try {
        ...
        console.error(Erro'r' connecting wallet: , error
      } finally {
        setIsLoading(false}}
    connectWallet(
  } []"</div>
  const createProposal = async (proposal: Partial<DAOProposal>) => {
    setIsLoading(true
    try {
      console.log(Creatin'g' new DAO proposal...
      ...
      setIsLoading(false}}
  const voteOnProposal = async (proposalId: string, vote: ''fo'r' | agains't' | absta'i'n) => {
    setIsLoading(true
    try {
      ...
      console.error('Erro'r voting: , error
    } finally {
      setIsLoading(false}}
...
{ id: 'proposa'ls, label: 'Proposa'ls, icon: '🗳️' },
    { id: treasur'y', label: Treasur'y', icon: 💰 },
{ id: 'membe'rs, label: 'Membe'rs, icon: '👥' },
    { id: governanc'e', label: Governanc'e', icon: ⚖️ },
