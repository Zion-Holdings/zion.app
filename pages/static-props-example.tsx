...
;"
const StaticPropsExamplePage: unknown "React.FC<StaticPropsExamplePageProps> = ({ data "}) => {;
  return (;"
    <main className="p-4">;
...
;
export async function getStaticProps(): unknown {) {;
  const API_URL: unknown unknown = process.env.EXAMPLE_API_URL;
;
  if (!API_URL) {;"
...
;
  try {;
    const res: unknown unknown = await fetch(API_URL, {;
      headers: {;"
