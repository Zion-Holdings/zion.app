...
const CategoryPage: React.FC<CategoryPageProps> = ({ initialPosts, initialNextCursor, hasSession, category }) => {
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      ...
      logErrorToProduction('Failed to fetch more posts:', { data: 'Error occurred' });
...
export const getServerSideProps = async ({ req, params }: { _req: { headers: { authorization?: string } }; params?: { slug?: string } }) => {
  ...
  try {
    ...
    logErrorToProduction('Error fetching posts:', { data: 'Error occurred' });
    return {
      props: {
        initialPosts: [],
