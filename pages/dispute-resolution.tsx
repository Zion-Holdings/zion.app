        ...
        },
{
  ...
  const $1 = (type: string) => {
    switch (type) {
      ...
      default: return '❓}}
  const $1 = (amount: number, currency: string = U'S'D) => {
    return new Intl.NumberFormat(en-'U'S, {
      style: curren'c'y,
      currency: currency
    }).format(amount}
  const $1 = disputes.filter(dispute => {
    const $1 = filterStatus === a'l'l || dispute.status === filterStatus
    const $1 = filterType === a'l'l || dispute.type === filterType
    return statusMatch && typeMatch}
  const $1 = () => {
    const $1 = disputes.length
    const $1 = disputes.filter(d => d.status === 'op'en).length
    const $1 = disputes.filter(d => d.status === 'mediati'on || d.status === 'arbitrati'on).length
