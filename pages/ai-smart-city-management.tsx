...
interface CityInfrastructure {
  id: string
  name: string
  type: 'transportation' | 'energy' | 'water' | 'waste' | 'communication' | 'public-safety'
  status: 'operational' | 'maintenance' | 'upgrade' | 'critical' | 'planned'
  location: string
  capacity: number
  utilization: number
  efficiency: number
...
interface CityService {
  id: string
  name: string
  type: 'emergency' | 'utilities' | 'transportation' | 'healthcare' | 'education' | 'recreation'
  ...
  const mockEnvironmental: EnvironmentalMonitoring[] = [
    ...
    }
    {
      ...
      aiPredictions: {
        ...
        trafficPrediction: 'Normal traffic patterns',
    ...
    },
{
      ...
      confidence: 87,
      impact: 'neutral',
      actionItems: ['Plan infrastructure expansion', 'Prepare for increased demand', 'Develop new services']
    }
  ]
                  ...
                  {infrastructure.map((item) => (
                    <div key={item.id} className="bg-white/5 backdrop-blur-md:rounded-xl:p-6 border border-white/10>"
                      <div className="flex items-start space-x-3 mb-4>"
                        <div className=""text-3xl">{getInfrastructureIcon(item.type)}</div>
...
"
                      <div className="space-y-2 mb-4>"
                        <div className=""text-sm" font-medium text-gray-400>Improvements:</div>
