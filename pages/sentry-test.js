...
const SentryTestPage = () => {
    ...
    return (_jsxs("div", { style: { padding: '20px', fontFamily: 'Arial, sans-serif' }, children: [_jsx("h1", { children: "Sentry Integration Test Page" }), _jsx("p", { children: "This page is for testing the Sentry error reporting setup. If Sentry is configured correctly (including valid DSNs in environment variables), clicking these buttons should result in errors being sent to your Sentry dashboard." }), _jsx("button", { onClick: handleClientError, style: { padding: '10px', marginRight: '10px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px' }, children: "Throw Client-Side Error" }), _jsx("button", { onClick: handleServerError, style: { padding: '10px', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '5px' }, children: "Trigger Server-Side API Error" }), _jsxs("p", { style: { marginTop: '20px', fontSize: '0.9em', color: '#555' }, children: [_jsx("strong", { children: "Note:" }), " For this test to be fully effective, ensure that `NEXT_PUBLIC_SENTRY_DSN` (for client-side) and `SENTRY_DSN` (for server-side/API) are set in your environment variables (e.g., in an `.env.local` file for local development). Without valid DSNs, Sentry will initialize but will not be able to send events. Check your browser console and server logs for any Sentry-related messages."] })] }));
};
