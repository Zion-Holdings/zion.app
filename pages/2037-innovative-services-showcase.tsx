...
const getServiceColor = (category: string) => {
  const colors = {
    'AI & Consciousness': 'from-purple-500 to-pink-600',
    'Quantum Technology': 'from-blue-500 to-cyan-600',
    'Autonomous Systems': 'from-green-500 to-emerald-600',
    'Micro SAAS': 'from-yellow-500 to-orange-600',
    ...
    >
      ...
      <div className={`relative bg-gradient-to-r ${serviceColor} bg-opacity-10 border border-opacity-30 rounded-2xl p-8 hover:border-opacity-50 transition-all duration-300 h-full`}>
        <div className="flex items-start justify-between mb-6">
          <div className="text-4xl">{service.icon || <Star className="w-8 h-8" />}</div>
          {service.popular && (
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <span className="text-3xl font-bold text-white">{service.pricing?.starter || service.marketPrice || '$99/month'}</span>
          ...
          <div className="text-right">
            ...
            <span className="text-gray-400 text-xs">{service.rating || 4}/5 ({service.reviews || 100} reviews)</span>
          </div>
        </div>
