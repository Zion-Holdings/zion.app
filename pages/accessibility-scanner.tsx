...
import { Phone, Mail, MapPin, Check, ArrowRight, Eye, Star } from 'lucide-react';
...
export default function AccessibilityScannerPage() {
	const service = additional2025ServiceExpansions.find(s => s.link.endsWith('/accessibility-scanner'));
			...
			<div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8">
				<div className="max-w-6xl mx-auto">
					<div className="text-center mb-10">
						<h1 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-400 bg-clip-text text-transparent flex items-center justify-center gap-3"><Eye className="w-10 h-10" />{service.name}</h1>
						...
						<div className="bg-black/30 rounded-2xl border border-cyan-500/30 p-6 h-fit">
							<div className="flex items-end justify-between mb-3">
								...
								<div className="flex items-center text-yellow-400"><Star className="w-4 h-4 mr-1" />{service.rating.toFixed(1)}</div>
							                                                  ...
							                                                  <Button href="/contact" variant="primary" size="lg" className="w-full">Run a Scan<ArrowRight className="w-5 h-5 ml-2" /></Button>
