        ...
        >
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[ 
              ...
              { label: 'Front Surface Maximizer', desc: 'High‑frequency front directory, visionary, and promo refresh.', href: '/.netlify/functions/front-surface-maximizer' },
              { label: 'Dependency Upgrader', desc: 'Smart dependency upgrades with safe commits to main.', href: '/.netlify/functions/dependency-upgrader' },
              { label: 'Docs Knowledge Sync', desc: 'Docs index + repo knowledge graph + radar metrics.', href: '/.netlify/functions/docs-knowledge-sync' },
            ].map((f) => (
...
+          className="mx-auto max-w-7xl px-6 pb-14"
+        >
+          <h2 className="text-center text-2xl font-bold tracking-wide text-white/90">Autonomous Cloud Services</h2>
+          <p className="mx-auto mt-2 max-w-3xl text-center text-sm text-white/70">Always-on cloud automations with direct links. Runs without human intervention and syncs changes to the repo.</p>
+          <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
+            {[
+              { title: 'Edge Cache Warmer', desc: 'Warms key pages for instant loads; logs results.', href: '/.netlify/functions/edge-cache-warmer' },
+              { title: 'Accessibility Auditor', desc: 'Checks a11y basics and publishes a markdown report.', href: '/.netlify/functions/a11y-auditor' },
+              { title: 'Search Engine Pinger', desc: 'Pings Google and Bing with the sitemap after updates.', href: '/.netlify/functions/search-engine-pinger' },
+            ].map((card) => (
+              <a key={card.title} href={card.href} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-6 backdrop-blur-xl hover:border-cyan-400/30 tilt-on-hover holo">
+                <div className="pointer-events-none absolute -inset-px -z-10 bg-gradient-to-r from-fuchsia-500/0 via-cyan-400/10 to-fuchsia-500/0 opacity-0 blur-2xl transition-opacity group-hover:opacity-100" />
+                <h3 className="text-lg font-semibold">{card.title}</h3>
