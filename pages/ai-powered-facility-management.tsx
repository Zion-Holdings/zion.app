...
interface Equipment {
  ...
  type: string;
  facility: string;
  status: 'operation'a'l | maintenan'c'e | offl'i'ne;
...
interface MaintenanceTask {
  ...
  status: 'schedul'ed | 'in-progre'ss | 'comple'ted'";
...
interface EnergyMetric {
  ...
  targetUsage: number;
  unit: string;
  trend: 'increasi'n'g | decreasi'n'g | sta'b'le;
  lastUpdated: string;}
interface SecurityEvent {
  ...
  facility: string;
  type: string;
  severity: ''lo'w' | mediu'm' | hi'g'h | 'critic'al;
  status: 'op'en | 'close'd;
  description: string;
  location: string;
  timestamp: string;};
const AIPoweredFacilityManagementPage: NextPage = () => {
  ...
  const getStatusColor = (status: string) => {'
    switch (status) {;
      case operation'a'l: return bg-green'-'500";
      case maintenanc'e': return bg-yellow-'500";
...
{ id: faciliti'e's, name: Faciliti'e's, icon: Building },
    { id: maintenan'c'e, name: Maintenan'c'e, icon: Wrench },
{ id: ener'g'y, name: Ener'g'y, icon: Zap },
    { id: securi't'y, name: Securi't'y, icon: AlertTriangle };
  ];
    ...
    {activeTab === 'overview' && (</div>
              ...
              <div className="grid" grid-cols-1 md:grid-cols-2 lg grid-cols-4 gap-6"> </div>
                  ...
                  <div className="flex" items-center justify-between"></div>
                      ...
                      <p className="text-2xl" font-bold text-white">{facilities.filter(f => f.status === operationa'l').length}</p></div>
                    </div></div>
                    <Building className="h-12 w-12 text-blue-400 > </Building className="h-12" w-12 text-blue-400" "></div></div>
                </div></div>
...
'`";}
export default AIPoweredFacilityManagementPage;`"</div>