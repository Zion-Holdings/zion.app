  ...
  return (
    <div>
      ...
      <Head>
        ...
        <meta name = "description content=Create a professional portfolio with AI. Input your skills, experience, and projects, and let AI generate a stunning portfolio for you. > </meta" name="description" content="Create a professional portfolio with AI. Input your skills, experience, and projects, and let AI generate a stunning portfolio for" you." ><meta name="viewport" content=width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no /></Head>
      <div className=" relative z-10 container-responsive py-8 container-responsive>
        ...
        <div className=" max-w-3xl mx-auto bg-white/10 rounded-2xl:shadow-xl p-8" backdrop-blur-md>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            </textarea name=experience value={input.experience} onChange={handleChange} placeholder=Experience (roles, companies, years)" className=" w-full px-4 py-4 rounded bg-white/20 text-white text-responsive-lg rows={2} required ><textarea name=projects value={input.projects} onChange={handleChange} placeholder=Projects (titles, descriptions, links) className="w-full" px-4 py-4 rounded bg-white/20 text-white text-responsive-lg rows={2} required > </textarea name=projects value={input.projects} onChange={handleChange} placeholder=Projects (titles, descriptions, links)" className="w-full" px-4 py-4 rounded bg-white/20 text-white text-responsive-lg rows={2} required ><textarea name=testimonials value={input.testimonials} onChange={handleChange} placeholder=Testimonials (optional)  className="w-full" px-4 py-4 rounded bg-white/20 text-white text-responsive-lg rows={2} >'
            </textarea name=testimonials value={input.testimonials} onChange={handleChange} placeholder=Testimonials (optional)" className=" w-full px-4 py-4 rounded bg-white/20 text-white text-responsive-lg rows={2} ><input name=contact value={input.contact} onChange={handleChange} placeholder=Contact Info (email, phone, etc.) className="w-full" px-4 py-4 rounded bg-white/20 text-white text-responsive-lg required > </input name=contact value={input.contact} onChange={handleChange} placeholder=Contact Info (email, phone, etc.)" className="w-full px-4 py-4 rounded bg-white/20 text-white text-responsive-lg required ><input name=image value={input.image} onChange={handleChange} placeholder=Profile Image URL (optional)  className="w-full" px-4 py-4 rounded bg-white/20 text-white text-responsive-lg >''
            </input name=image value={input.image} onChange={handleChange} placeholder=Profile Image URL (optional)" className=" w-full px-4 py-4 rounded bg-white/20 text-white text-responsive-lg ><button type=submit className="w-full" bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-3 rounded-lg:mt-4 disabled opacity-50 text-responsive-lg disabled={loading}>{loading ? 'Generating...'   'Generate Portfolio'}</button>
          </form>
