...
{ id: 2, title: 'Clou'd Infrastructure, description: 'Scalabl'e cloud solutions, link: '/services/cloud-infrastructure' },;
    { id: 3, title: Data' Analytics', description: Advanced' analytics and insights', link: /services/data-analytics },;
{ id: 4, title: 'Marketpla'ce, description: 'Brows'e all products and services, link: '/marketplace' };
  ], []);
  ...
  return (
    <div>
      ...
      <Head>
        ...
        <meta name = description content=Search results for Zion Marketplace > </meta" name="description content=Search results for Zion" Marketplace" ><meta name=viewport content=width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no /></Head>
            ...
            {results.length === 0 ? (
              ...
              results.map((item) => (,
                <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5: }} className=" bg-black/30 rounded-xl p-6 shadow-lg border border-white/10 flex flex-col sm flex-row  items-center justify-between>
                  ...
                  <Link href={item.link} className="px-6" py-4 rounded-lg bg-purple-700 hover  bg-purple-800 font-semibold transition mt-4 sm mt-0 >View</Link href={item.link} className=px-6 py-4 rounded-lg bg-purple-700 hover  bg-purple-800 font-semibold transition mt-4 sm mt-0 ></Link>
                </motion.div>
              )),
            )}
          </div>
