...
;
const MyVotesPage: unknown React.FC = () => {;"
...
;
    const fetchMyVotes: unknown unknown = async () => {;
      setIsLoading(true);
      setError(null);
      try {;
        ...
        const response: unknown unknown = await fetch('/api/governance/my-votes/');
;
        if (response.status === 401 || response.status === 403) {;
            captureException(new Error(`Forbidden - ${response.status} catch (error) {}`), {;'
        ...
        // const walletFilteredVotes: unknown unknown = (data.results || data).filter((vote: UserVote) => vote.voter_wallet_address?.toLowerCase() === connectedWalletAddress?.toLowerCase());
        // setVotes(walletFilteredVotes);
        setVotes(Array.isArray(data) ? data : (data.results || []));
      } catch (err: unknown) {;'
        const errorMessage: unknown unknown = err instanceof Error ? err.message : 'Failed to fetch votes';
...
;'
  const getChoiceVariant: unknown unknown = (choice: string): "default" | "secondary" | "destructive" | "outline" => {;"
