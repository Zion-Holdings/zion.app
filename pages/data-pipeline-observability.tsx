...
export default function DataPipelineObservabilityPage() {
  const service = enhancedRealMicroSaasServices.find(s => s.link.endsWith('/data-pipeline-observability'));
      ...
      <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <h1 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent flex items-center justify-center gap-3"><Activity className="w-10 h-10" />{service.name}</h1>
            <p className="mt-4 text-xl text-slate-300 max-w-3xl mx-auto">{service.tagline}</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
            <div className="lg:col-span-2 bg-black/30 rounded-2xl border border-emerald-500/30 p-6">
              <h2 className="text-2xl font-semibold mb-4">Observability features</h2>
            ...
            <div className="bg-black/30 rounded-2xl border border-emerald-500/30 p-6 h-fit">
              ...
              <Button href="/contact" variant="primary" size="lg" className="w-full">See a Demo<ArrowRight className="w-5 h-5 ml-2" /></Button>
