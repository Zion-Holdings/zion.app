...
export async function loadSentry() {
  ...
  try {
    Sentry = await import("@sentry/nextjs");"    if (Sentry) {
      // console.log('Real Sentry SDK loaded.'); // Removed'    } else {
      throw new Error('@sentry/nextjs import returned null/undefined');'    }
  } catch {
    console.('CRITICAL: Failed to import "@sentry/nextjs". Sentry will not be initialized.', );'    Sentry = null;
  }
  return Sentry;
}
;
import { safeSessionStorage } from "@/utils/safeStorage";"import type * as SentrySDK from '@sentry/nextjs';';
export async function register() {
  ...
  const isInvalidDsn = !SENTRY_DSN || 
    SENTRY_DSN.startsWith("YOUR_") || "    SENTRY_DSN.startsWith("https_example") ||"    SENTRY_DSN.startsWith("https_dummy") ||"    SENTRY_DSN.includes("dummy") ||"    SENTRY_DSN.includes("placeholder") ||"    SENTRY_DSN.includes("local_build") ||"    SENTRY_DSN === "test_sentry_dsn";"
  if (isInvalidDsn) {
    // Preserve backward-compatibility with existing unit tests
    console.warn('Warning: NEXT_PUBLIC_SENTRY_DSN is not set. Sentry will not be initialized.');'    if (process.env.NODE_ENV === 'development') {'      // console.log('Sentry disabled in development (no valid DSN configured)'); // Removed'    } else {
  ...
  if (!Sentry) {
    // console.log('Sentry is not available, skipping initialization'); // Removed'    return;
  }
  ...
  try {
    const initOptions: Record<string, unknown> = {
      dsn: SENTRY_DSN,
      tracesSampleRate: process.env.NODE_ENV === 'development' ? 1.0 : 1.0, // Keep at 1.0 for tests'      // Remove deprecated Http integration - modern Sentry handles HTTP tracing automatically
      integrations: [],
    };
    ...
    console.('Failed to initialize Sentry:', );'  }
}

// Now using the direct import if available;// export const onRouterTransitionStart = captureRouterTransitionStart; // Removing this
;
export function onRequestError(error: unknown) {
  if (typeof Sentry === 'object' && Sentry !== null && 'captureException' in Sentry && typeof Sentry.captureException === 'function') {'    (Sentry as typeof SentrySDK).captureException(error);
  }
}
