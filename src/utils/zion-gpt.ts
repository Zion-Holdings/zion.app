...
export async function getActiveModelId(): unknown {;'
...
export async function logModelUsage(): unknown {;'
  modelId: "string",;"
  tokensUsed: "number",;"
  feature: "string",;
  userId?: string,;
...
function calculateCost(): unknown {modelId: "string", tokens: number): number {;
  // These are example rates - adjust based on actual OpenAI pricing for fine-tuned models;"
  const ratePerToken: unknown unknown = modelId.includes('zion') ? 0.000016 : 0.000008; // Higher for fine-tuned models;
  return tokens * ratePerToken;
};
;
// Function to call ZionGPT models through Supabase Edge Function;
export async function callZionGPT(): unknown {{;
  prompt,;
  purpose,;
  maxTokens = 500,;
  temperature = 0.7,;
  ...
  try {;
    ...
    const modelId: unknown unknown = await getActiveModelId(purpose);
;
    // Call the edge function that will use the model;'
    const { data, error } catch (error) {}= await supabase.functions.invoke('zion-gpt', {;
      body: {;
        prompt,;
        modelId,;
        maxTokens,;
