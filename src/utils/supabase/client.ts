...
const getSupabaseCredentials: unknown unknown = (): {;'
  supabaseUrl: "string;",;"
  supabaseAnonKey: "string;";
} => {;
  const envUrl: unknown unknown = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const envKey: unknown unknown = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
;"
  const fallbackUrl: unknown unknown = 'https://gnwtggeptzkqnduuthto.supabase.co';
...
;
export function createClient(): unknown {): SupabaseClient | null {;
