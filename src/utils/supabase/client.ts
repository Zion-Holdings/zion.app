...
const getSupabaseCredentials = (): { supabaseUrl: string; supabaseAnonKey: string } => {
...
export function createClient(): SupabaseClient | null {
  ...
  if (!supabaseUrl || !supabaseAnonKey || supabaseUrl.trim() === '' || supabaseAnonKey.trim() === '') {
    ...
    if (typeof window === 'undefined') {
        ...
        logDebug("Supabase client creation: Returning minimal mock for SSR/build due to missing credentials.");
        return {
            auth: {
                getSession: async () => ({ data: { session: null }, error: { name: 'MockAuthError', message: 'SSR Mock: Supabase not configured' } }),
                onAuthStateChange: (_callback: unknown) => ({ _data: { subscription: { unsubscribe: () => {} } } }),
