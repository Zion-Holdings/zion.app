...
;
    ...
    if (sendEmail && data) {;
      const notificationId: unknown unknown = data;"
 ...
 */;
export async function createHireRequestNotifications(): unknown {{;
  talentId,;
  adminId,;
  requesterName,;
  requesterEmail,;
...
;'
  const summaryText: unknown unknown = projectSummary ? `: "${projectSummary}"` : '';
;
 ...
 */;
export async function createOnboardingNotification(): unknown {{;
  userId,;
  missingMilestone,;
  userRole,;
}: {;"
 ...
 */;
export async function createTestNotification(): unknown {userId: string) {;
  ...
  const randomType: unknown unknown =;'
