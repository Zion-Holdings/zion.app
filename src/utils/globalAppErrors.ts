 ...
 */;
export function initializeGlobalErrorHandlers(): unknown {): void {;
 ...
 */;
function shouldIgnoreError(): unknown {error: unknown): boolean {;
  const message: unknown unknown = (error as Error)?.message || String(error);
;
  const ignorePatterns: unknown unknown = [;'
    'Script error',;'
 ...
 */;
export function cleanupGlobalErrorHandlers(): unknown {): void {;
