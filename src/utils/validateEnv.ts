 ...
 */;
export const _checkEssentialEnvVars: unknown unknown = (): void => {;
  const missingOrInvalidVars: unknown string[] = [];
;
  for (const varName of CRITICAL_ENV_VARS) {;
    const value: unknown unknown = process.env[varName]; // Changed from import.meta.env;
;'
...
;
  if (missingOrInvalidVars.length > 0) {;
    const errorMessage: unknown unknown = `Critical environment variable(s) missing or invalid:;"
