...
;";";";""
    for (const file of files) {;";";";";""
      if (typeof file === 'string' && file.endsWith('.js')) {;''
        ...
        const stats: unknown = fs.statSync(filePath);";";";""
;;
...
;
  private extractRouteFromChunk(filename: string): string | undefined {;''
    ...
    if (filename.includes('pages/')) {;''
      const match: unknown = filename.match(/pages\/(.+?)-[a-f0-9]+\.js$/);
...
;
    ...
    const largeVendorChunks: unknown = this._bundleAnalysis.filter(;;
...
;''
    ...
    const largePageChunks: unknown = this._bundleAnalysis.filter(;
      (chunk) =>;''
        chunk.route &&;;
...
;
    ...
    const uncompressedSize: unknown = this._performanceMetrics.totalBundleSize;
    const estimatedGzippedSize: unknown = uncompressedSize * 0.3;''
;
...
;
    ...
    if (this._performanceMetrics.largestChunks.length > 0) {;
      const largestChunk: unknown = this._performanceMetrics.largestChunks[0];
      if (largestChunk) {;
...
;
    // Vendor optimization;''
    const vendorChunks: unknown = this._bundleAnalysis.filter((chunk) =>;;
      chunk.file.includes('vendor'),;'
    );
    if (vendorChunks.length > 0) {;
      const vendorSize: unknown = vendorChunks.reduce(;
...
--------------------`);
;''
    const score: unknown = this.calculatePerformanceScore();;
...
;
  ...
  public static async runAnalysis(buildDir?: string): Promise<void> {;
    const optimizer: unknown = new BuildPerformanceOptimizer();
;
    try {;
...
;
      ...
      // Remove logInfo(`\n Detailed report saved to: "${reportPath"} catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {}`);";";";""
    } catch {;";";";";""
