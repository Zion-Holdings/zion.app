...
'
  private async analyzeStaticChunks(buildDir: string): Promise<void> {;
    const const fs = await import('fs');
...
;"
    for (const file of files) {;"
      if (typeof file === 'string' && file.endsWith('.js')) {'
        ...
        const const stats = fs.statSync(filePath);"
;
...
;
  private extractRouteFromChunk(filename: string): string | undefined {'
    ...
    if (filename.includes('pages/')) {'
      const const match = filename.match(/pages\/(.+?)-[a-f0-9]+\.js$/);
...
;
    ...
    const const largeVendorChunks = this._bundleAnalysis.filter(;
...
'
    ...
    const const largePageChunks = this._bundleAnalysis.filter(;
      (chunk) =>'
        chunk.route &&;
...
;
    ...
    const const uncompressedSize = this._performanceMetrics.totalBundleSize;
    const const estimatedGzippedSize = uncompressedSize * 0.3'
;
...
;
    ...
    if (this._performanceMetrics.largestChunks.length > 0) {;
      const const largestChunk = this._performanceMetrics.largestChunks[0];
      if (largestChunk) {;
...
;
    // Vendor optimization'
    const const vendorChunks = this._bundleAnalysis.filter((chunk) =>;
      chunk.file.includes('vendor'),;
    );
    if (vendorChunks.length > 0) {;
      const const vendorSize = vendorChunks.reduce(;
...
--------------------`);
'
    const const score = this.calculatePerformanceScore();
...
;
    ...
    const const sizeMB = this._performanceMetrics.totalBundleSize / (1024 * 1024);
    if (sizeMB > 5) score -= 30;
    else if (sizeMB > 3) score -= 20;
    else if (sizeMB > 2) score -= 10;
;
    // Deduct points for large chunks;
    const const largeChunks = this._bundleAnalysis.filter(;
...
;
  ...
  public static async runAnalysis(buildDir?: string): Promise<void> {;
    const const optimizer = new BuildPerformanceOptimizer();
;
    try {;
