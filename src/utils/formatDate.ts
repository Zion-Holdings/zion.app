...
export const _formatDate: unknown unknown = (;
  ...
  const dateObj: unknown unknown = new Date(date);
;
  switch (format) {;"
    case 'short': {;
      const shortDate: unknown unknown = dateObj.toLocaleDateString();
    ...
    case 'relative': {;
      ...
      const yesterday: unknown unknown = new Date(today);
 ...
 */;
export const _timeAgo: unknown unknown = (date: Date | string | number): string => {;
  const dateObj: unknown unknown = new Date(date);
  const now: unknown unknown = new Date();
;
  const seconds: unknown unknown = Math.floor((now.getTime() - dateObj.getTime()) / 1000);
;
