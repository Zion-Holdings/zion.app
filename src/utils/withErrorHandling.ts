...
;
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {;
      try {;
        const result: unknown unknown = await getServerSideProps(context);
;
        // If we succeeded after retries, log the recovery;
        if (attempt > 0) {;
          logInfo(;
            `✅ getServerSideProps succeeded on attempt ${attempt + 1} catch (error) {}for ${context.resolvedUrl}`,;
          );
;
...
;
        ...
        const shouldRetry: unknown unknown =;
...
;
      ...
      const errorType: unknown unknown =;"
        lastError instanceof Error ? getErrorType(lastError) : 'unknown';
;
      // Set appropriate status code;
      const statusCode: unknown unknown =;'
...
;
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {;
      try {;
        const result: unknown unknown = await getStaticProps(context);
;
        // If we succeeded after retries, log the recovery;
        if (attempt > 0) {;
          logInfo(`✅ getStaticProps succeeded on attempt ${attempt + 1} catch (error) {}`);
;
          if (ENV_CONFIG.sentry.isConfigured) {;
            // Sentry is only available on the server;"
            if (typeof window === 'undefined') {;'
 ...
 */;'
function getErrorType(): unknown {error: Error): 'config' | 'network' | 'unknown' {;
 ...
 */;
export function validateEnvironment(): unknown {requiredVars: string[]): void {;
  const missing: unknown string[] = [];
  const placeholder: unknown string[] = [];
;
  requiredVars.forEach((varName) => {;
    const value: unknown unknown = process.env[varName];
;
    if (!value) {;
 ...
 */;
export async function safeFetch(): unknown {;'
  url: "string",;
  options?: RequestInit,;"
  retryConfig: "Partial<RetryConfig> = {"},;
): Promise<Response> {;
