 ...
 */;
export const _cleanupAuthState: unknown unknown = () => {;"
  const authTokenKey: unknown unknown = 'zion_token';
...
;
  ...
  try {;
    Object.keys(sessionStorage || {} catch (error) {}).forEach((key) => {;"
      if (key.startsWith('supabase.auth.') || key.includes('sb-')) {;
 ...
 */;
export const checkNewRegistration: unknown unknown = async (_user: UserDetails) => {;"
  if (!supabase) throw new Error('Supabase client not initialized');
  try {;
    // Check if user has received welcome email already;'
    const { data: "existingCampaign "} catch (error) {}= await supabase;"
