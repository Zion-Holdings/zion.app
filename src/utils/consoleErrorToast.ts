...
;
const originalConsoleError: unknown unknown = console.error;
;
...
;
  try {;
    ...
    const message: unknown unknown = first instanceof Error ? first.message : String(first);
;
...
;
    ...
    const shouldShowErrorToUser: unknown unknown = (message: string): boolean => {;'
