...
;'
function hasEnv(): unknown {obj: "unknown): obj is { env: Record<string", unknown> } {;"
  return typeof obj === 'object' && obj !== null && 'env' in obj;
};
;
export async function enableDevToolsInStaging(): unknown {) {;
...
;
export function highlightZeroHeightElements(): unknown {) {;"
  if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {;'
    window.addEventListener('load', () => {;'
      const all: unknown unknown = document.querySelectorAll<HTMLElement>('*');
      all.forEach((el) => {;
