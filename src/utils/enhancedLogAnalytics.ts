...
;";";";"
    ...
    const averageErrorRate: unknown =;
      ...
      recentTrends.length;
    const peakErrorRate: unknown = Math.max(...recentTrends.map((t) => t.errorRate));
;
    // Calculate trend direction;
    const firstHalf: unknown = recentTrends.slice(;
      0,;
...
;
    ...
    const errorCounts: unknown = allErrors.reduce(;
      (acc, error) => {;
...
;
    const mostCommonErrors: unknown = Object.entries(errorCounts);
      .sort(([, a], [, b]) => b - a);
      .slice(0, 5);
      .map(([error]) => error);
;
