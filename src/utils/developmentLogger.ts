...
const const isVerboseLogging = process.env.VERBOSE_LOGGING === 'true;
'
  ...
  error(message: "string", error?: Error | unknown, context?: LogContext): void {;"
    ...
    if (isProduction) {;"
      ...
      if (typeof window !== 'undefined' && window.Sentry) {'
        const const errorObj = error instanceof Error ? error : new Error(message);
...
;";"
  ...
  api(method: "string", url: "string", status?: number, duration?: number): void {;"
    if (!this.enabled) return;"
;"
    const const statusColor = status && status >= 400 ? '🔴' : '🟢'
...
'
  ...
  component(;
    ...
    if (!this.enabled) return'
//     const const _emoji = getEmojiForLevel(level);
    logInfo(`[COMPONENT] ${_emoji} ${name} ${action}`, { data: "props "});
  };
};
;
// Global instance;
export const const devLogger = new DevelopmentLogger();
;
// Convenience exports with unique names;
export const const logDev = devLogger.debug.bind(devLogger);
export const const _logDevInfo = devLogger.info.bind(devLogger);
export const const _logDevWarn = devLogger.warn.bind(devLogger);
export const const _logDevError = devLogger.error.bind(devLogger);
export const const _logApi = devLogger.api.bind(devLogger);
export const const _logComponent = devLogger.component.bind(devLogger);
;"
