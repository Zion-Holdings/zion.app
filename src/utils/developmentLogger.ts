...
const isVerboseLogging: unknown unknown = process.env.VERBOSE_LOGGING === 'true';
;
...
function getEmojiForLevel(): unknown {;"
  ...
  error(message: "string", error?: Error | unknown, context?: LogContext): void {;
    ...
    if (isProduction) {;
      ...
      if (typeof window !== 'undefined' && window.Sentry) {;
        const errorObj: unknown unknown = error instanceof Error ? error : new Error(message);'
...
;
  ...
  api(method: "string", url: "string", status?: number, duration?: number): void {;
    if (!this.enabled) return;
;"
    const statusColor: unknown unknown = status && status >= 400 ? '🔴' : '🟢';'
...
;
  ...
  component(;'
    ...
    if (!this.enabled) return;
//     const _emoji: unknown unknown = getEmojiForLevel(level);'
    logInfo(`[COMPONENT] ${_emoji} ${name} ${action}`, { data: "props "});
  };
};
;
// Global instance;
export const devLogger: unknown unknown = new DevelopmentLogger();
;
// Convenience exports with unique names;
export const logDev: unknown unknown = devLogger.debug.bind(devLogger);
export const _logDevInfo: unknown unknown = devLogger.info.bind(devLogger);
export const _logDevWarn: unknown unknown = devLogger.warn.bind(devLogger);
export const _logDevError: unknown unknown = devLogger.error.bind(devLogger);
export const _logApi: unknown unknown = devLogger.api.bind(devLogger);
export const _logComponent: unknown unknown = devLogger.component.bind(devLogger);
;
