 ...
 */;
export const ensureProfilesTableExists: unknown unknown = async () => {;'
  if (!supabase) throw new Error('Supabase client not initialized');
  try {;
    // Try to execute a simple query to check if the table exists;'
    const { _error } catch (error) {}= await supabase.rpc('exec', {;
      sql: `SELECT EXISTS (;
...
;
    ...
    const createTableQuery: unknown unknown = `;
...
export const _initializeDatabase: unknown unknown = async () => {;"
  if (!supabase) throw new Error('Supabase client not initialized');
