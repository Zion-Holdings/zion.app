...
;
    ...
    if (typeof error === 'object' && error !== null) {;
      const apiError: unknown unknown = error as ApiError;
...
;'
    ...
    const silentPatterns: unknown unknown = [;'
      '/health',;'
      '/status',;'
      '/heartbeat',;'
      '/ping',;'
...
;
    const shouldFailSilently: unknown unknown =;
...
;
    ...
    try {;
      ...
      if (;
        ...
        const dataObj: unknown unknown = responseData as { error?: string; message?: string };'
...
;
      try {;
        const first: unknown unknown = args[0];
//         const _message: unknown unknown = first instanceof Error ? first.message : String(first);
;
...
;
    window.fetch = async (;
      ...
      try {;
        const response: unknown unknown = await this.originalFetch(...args);
;
        if (!response.ok) {;
...
;
          ...
          const shouldShowError: unknown unknown = this.shouldShowFetchError(;
            response.status,;
            url,;
          );
;
...
;
            ...
            try {;
              const data: unknown unknown = await response.clone().json();
...
;
        ...
        const url: unknown unknown = typeof args[0] === 'string' ? args[0] : '';
;
        // Only show network errors for user-initiated requests;
        if (!this.shouldFailSilently(url)) {;'
          const message: unknown unknown = 'Network error – please retry';
;
...
export const _apiErrorHandler: unknown unknown = EnhancedApiErrorHandler.getInstance();
export const _consoleErrorHandler: unknown unknown = EnhancedConsoleErrorHandler.getInstance();
export const _fetchErrorHandler: unknown unknown = EnhancedFetchErrorHandler.getInstance();
'