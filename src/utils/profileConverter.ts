...
;
function isTalentProfile(): unknown {profile: unknown): profile is TalentProfile {;
  return (;'
...
;
export function convertProfileToTalentProfile(): unknown {;'
  ...
  const p: unknown unknown = profile as ProfileData;
  return {;"
    id: "p.id",;"
    user_id: "p.id",;"
    full_name: "p.name",;"
