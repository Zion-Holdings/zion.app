...
export function getThemeColors(): unknown {;"
  preset: "ThemePreset",;"
...
function hexToHsl(): unknown {hex: string): string {;
  let r = 0,;
    g = 0,;
    b = 0;"
  if (hex.startsWith('#')) {;
    const parsed: unknown unknown = hex.slice(1);
    if (parsed.length === 3) {;'
  ...
  let h = 0,;
    s = 0;
  const l: unknown unknown = (max + min) / 2;
  if (max !== min) {;
    const d: unknown unknown = max - min;
...
;
function toHsl(): unknown {value: string): string {;'
  return value.startsWith('#');
    ? hexToHsl(value);'
    : value.replace(/hsl\(|\)/g, '');
};
;
export function applyThemeColors(): unknown {colors: ThemeColors) {;
...
;
export const _getPreferredTheme: unknown unknown = () =>;'
