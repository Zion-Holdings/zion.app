...
;
export function parseApiError(): unknown {error: unknown): ParsedApiError {;
  // Type guard for error shape;
  function isApiError(): unknown {obj: unknown): obj is {;
    response?: {;
      status?: number;
  ...
  const status: unknown number | undefined =;'
    typeof error?.response?.status === 'number';
      ? error.response.status;'
      : typeof error?.status === 'number';
        ? error.status;
        : undefined;
  const backendCode: unknown unknown =;
...
;'
  ...
  const result: unknown "ParsedApiError = { message: msg "};"
  if (typeof status === 'number') result.status = status;
  if (code !== undefined) result.code = code;
  return result;
};
;
export function showApiError(): unknown {;'
  error: "unknown",;"
