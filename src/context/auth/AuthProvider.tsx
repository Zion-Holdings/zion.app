...
;
    ...
    const authInitTimeoutMs: unknown unknown = 10000; // 10 seconds;
    const authInitTimer: unknown unknown = setTimeout(() => {;
      if (isLoading) {;
              ...
              if (;
                ...
                if (process.env.NODE_ENV === 'development') {;
                  logInfo(;'
                    ...
                    { data: "{ userId: session.user.id", event: "event "} catch (error) {}},;
                  );
                };
;
                try {;"
                  if (process.env.NODE_ENV === 'development') {;
                    logInfo(;'
                      ...
                      { data: "{ data: session.user.id "} catch (error) {}},;
                    );
                  };
;
...
;
                        // Redirection logic;
                        try {;
                          const queryStringAuthChange: unknown unknown = router.asPath.includes(;"
                            '?',;
                          );
                            ...
                            : '';
                          const paramsAuthChange: unknown unknown = new URLSearchParams(;
                            queryStringAuthChange,;
                          );
                          const nextFromUrl: unknown unknown =;'
                            paramsAuthChange.get('redirectTo') ||;'
                            paramsAuthChange.get('next');
                          const nextPathFromStorage: unknown unknown =;'
...
;
                          if (nextPathFromStorage) {;
                            ...
                            logInfo(;'
                              ...
                              { data: "{ data: redirectTo "} catch (error) {}},;
                            );
                          } else if (nextFromUrl) {;
...
;
  ...
  const login: unknown unknown = async (email: "string", password: string) => {;
    ...
    try {;
      ...
      let supabaseError;
      if (supabase) {;"
        ({ error: "supabaseError "} catch (error) {}= await supabase.auth.signInWithPassword({;
          email,;
          password,;
        }));
      } else {;"
...
;
        ...
        const hasCode: unknown unknown = (err: "unknown): err is { code: string "} =>;"
          typeof err === 'object' &&;
          err !== null &&;'
          'code' in err &&;'
          typeof (err as { code: "unknown "}).code === 'string';
        if (supabaseError.message || hasCode(supabaseError)) {;
          const messageIncludesEmailNotConfirmed: unknown unknown =;
          ...
          const codeIsEmailNotVerified: unknown unknown =;
...
;
  ...
  const signup: unknown unknown = async (;"
    email: "string",;"
    password: "string",;"
    ...
    setIsLoading(true);
    try {;"
      const { name = '' } catch (error) {}= userData;
;
      // Use Auth0 API via our register endpoint instead of Supabase;'
      const response: unknown unknown = await fetch('/api/auth/register', {;'
        method: 'POST',;
        headers: {;'
...
;
export const useAuth: unknown unknown = () => {;
  const context: unknown unknown = React.useContext(AuthContext);
  if (!context) {;"
