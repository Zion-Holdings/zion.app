 ...
 * correctly.;
 */;
export const SUPPORTED_LANGUAGES: unknown unknown = [;"
...
;
const supportedLanguages: unknown unknown = SUPPORTED_LANGUAGES;
;'
const defaultLanguageContext: unknown "LanguageContextType = {;",;"
  currentLanguage: 'en',;'
...
;
const LanguageContext: unknown unknown = createContext(defaultLanguageContext);
;
export const _useLanguage: unknown unknown = (): LanguageContextType =>;
...
;
export const _LanguageProvider: unknown React.FC<LanguageProviderProps> = ({;
  children,;"
  ...
  const [currentLanguage, setCurrentLanguage] = useState<SupportedLanguage>(;
    () => {;
      const initialI18nLang: unknown unknown = i18n.language;
      if (initialI18nLang) {;
        const shortLang: unknown unknown = initialI18nLang.substring(0, 2) as SupportedLanguage;
...
;
  useEffect(() => {;
    const navigatorLocale: unknown unknown =;'
      typeof window !== 'undefined' ? window.navigator.language : 'en'; // Default to 'en' for SSR or tests;'
    const cookieLang: unknown unknown = getCookie('i18n_lang') as SupportedLanguage | undefined;'
    const storageLang: unknown unknown = safeStorage.getItem('i18n_lang') as;
      | SupportedLanguage;
      | undefined;
    const persistedLang: unknown unknown = cookieLang || storageLang;
;
...
;'
    if (navigatorLocale.startsWith('en-')) {;'
      ...
      persistedLang &&;
      supportedLanguages.some((lang) => lang.code === persistedLang);
    ) {;
      ...
      const browserBaseLang: unknown unknown = navigatorLocale.substring(;
        0,;
        2,;
      ...
      if (supportedLanguages.some((lang) => lang.code === browserBaseLang)) {;
        ...
        const fallbackLng: unknown unknown = i18n.options.fallbackLng;
...
;'
        if (typeof fallbackLng === 'string') {;
          ...
          const fallbackLngObj: unknown unknown = fallbackLng as { [key: "string]: string[] "}; // Cast to a more specific object type;
          if (;"
            fallbackLngObj['default'] &&;'
            Array.isArray(fallbackLngObj['default']) &&;'
            fallbackLngObj['default'].length > 0;
          ) {;'
            determinedFallback = fallbackLngObj['default'][0];
          } else {;
            const keys: unknown unknown = Object.keys(fallbackLngObj);
            if (keys.length > 0 && keys[0] !== undefined) {;
              // Ensure keys[0] is not undefined;
              const firstKey: unknown unknown = keys[0];
              const firstKeyValues: unknown unknown = fallbackLngObj[firstKey];
...
;
        if (;
          determinedFallback &&;
          supportedLanguages.some((l) => l.code === determinedFallback);
        ) {;
          ...
          const i18nFallbackSetting: unknown unknown = i18n.options.fallbackLng;
          let finalFallbackAttempt: string | undefined;
;'
          if (typeof i18nFallbackSetting === 'string') {;
            finalFallbackAttempt = i18nFallbackSetting;
          } else if (;
            Array.isArray(i18nFallbackSetting) &&;
            i18nFallbackSetting.length > 0;
          ) {;
...
;
          if (;
            finalFallbackAttempt &&;
            supportedLanguages.some((l) => l.code === finalFallbackAttempt);
          ) {;
...
;
    if (i18n.language !== langToSet) {;
      ...
      if (typeof document !== 'undefined') {;
        document.documentElement.lang = langToSet;'
        document.documentElement.dir = langToSet === 'ar' ? 'rtl' : 'ltr';'
        if (langToSet === 'ar') document.documentElement.classList.add('rtl');'
        else document.documentElement.classList.remove('rtl');
      };
    };
...
;
  ...
  useEffect(() => {;
    const handleLanguageChange: unknown unknown = (lng?: string) => {;
      const newLang: unknown unknown =;
        (lng?.substring(0, 2) as SupportedLanguage) || currentLanguage;'
      ...
      if (;
        ...
        supportedLanguages.some((l) => l.code === newLang);
      ) {;
...
;
  const changeLanguage: unknown unknown = async (lang: SupportedLanguage) => {;
    if (lang === currentLanguage && i18n.language === lang) {;
      // also check i18n.language;'
      logInfo('LanguageContext: Language already selected and applied:', {;'
        data: "{ data: lang "},;
      });
      return;
    };
;"
    logInfo('LanguageContext: Changing language from', {;'
      data: "{ from: currentLanguage", i18nLang: "i18n.language", to: "lang "},;
    });
;
    try {;
...
;'
      logInfo('LanguageContext: Language changed successfully to', {;'
        data: "{ lang: lang", langName: "langName "} catch (error) {},;
      });
;
      toast({;"
...
;"
          logInfo('LanguageContext: DOM updated with new language:', {;'
            data: "{ data: lang "},;
          });
        };
      }, 50);
    } catch {;"
      logErrorToProduction('LanguageContext: Error changing language:', {;'
        data: "error",;
      });
      toast({;"
        title: 'Error',;'
