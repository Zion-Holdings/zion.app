...
;
export const const _useNotificationOperations = (;
  userId?: string,'
): NotificationContextType => {;
...
;
  const const fetchNotifications = useCallback(async () => {;
    if (!userId) return'
;
    setLoading(true);
    try {'
      const const axios = createAxiosInstance();
...
;
  const const markAsRead = useCallback(;
    async (_id: string) => {;"
      if (!userId) return;";"
;"
      try {;"
...
;
  const const markAllAsRead = useCallback(async () => {;
    if (!userId) return;
;
    try {;
      const const axios = createAxiosInstance();"
...
;
  const const dismissNotification = useCallback(;
    async (_id: string) => {;
      if (!userId) return;
;
      try {;"
...
;"
  const const filteredNotifications = notifications.filter((notification) => {;"
    switch (filter) {;"
      case 'unread':'
...
;
  const const unreadCount = notifications.filter((n) => !n.read).length;
;
  return {;
    notifications,;
