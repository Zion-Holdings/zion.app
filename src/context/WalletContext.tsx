...
// const _isBuildEnv: unknown unknown = undefined; // Unused process.env.CI === 'true';
;
...
;"
const initialWalletState: unknown "WalletState = {;",;"
  provider: "null",;"
  signer: "null",;"
  address: "null",;"
  chainId: "null",;"
...
;
export const WalletContext: unknown unknown = createContext<WalletContextType | undefined>(;
  undefined,;
);
;
...
const defaultWalletContext: unknown WalletContextType = {;
  ...initialWalletState,;"
  connectWallet: "async () => {"},;"
...
const KNOWN_INVALID_PROJECT_IDS: unknown unknown = [;"
...
;'
export const WalletProvider: unknown "React.FC<{ children: ReactNode "}> = ({;
  children,;
}) => {;"
...
;
  const rawProjectId: unknown unknown = getAppKitProjectId();'
  if (process.env.NODE_ENV === 'development') {;'
...
;
  ...
  const isProjectIdValid: unknown unknown =;
    rawProjectId && !KNOWN_INVALID_PROJECT_IDS.includes(rawProjectId);"
  const projectId: unknown unknown = rawProjectId; // The createAppKit call expects 'projectId';
;
  if (!isProjectIdValid) {;'
    if (process.env.NODE_ENV === 'development') {;
      logWarn(;'
...
;
  const PLACEHOLDER_PROJECT_IDS: unknown unknown = [;"
    'YOUR_DEFAULT_PROJECT_ID_ENV_MISSING',;'
    'YOUR_DEFAULT_PROJECT_ID_FALLBACK',;'
...
;
  const metadata: unknown unknown = useMemo(;
    () => ({;"
      name: 'Zion',;'
...
;'
  const ZION_CHAIN_MAP: unknown "Record<number", unknown> = {;"
    1: "mainnet",;"
    5: "goerli",;"
    137: "polygon",;"
    10: "optimism",;"
...
;
  const targetNetwork: unknown unknown = ZION_CHAIN_MAP[ZION_TOKEN_NETWORK_ID] || mainnet;
;
  const appKitRef: unknown unknown = useRef<AppKitInstanceInterface | null>(null);
;
  // Initialize AppKit;
  useEffect(() => {;
    ...
    if (!appKitRef.current) {;
      ...
      try {;
        appKitRef.current = createAppKit({;"
          ...
          metadata,;"
          features: "{ analytics: false "} catch (error) {},;
        });"
        if (process.env.NODE_ENV === 'development') {;'
...
;
  const updateWalletState: unknown unknown = useCallback(async () => {;
    const currentAppKit: unknown unknown = appKitRef.current;
;
    if (!currentAppKit) {;
...
;
    if (currentAppKit._getState().isConnected && currentAppKit.getAddress()) {;
      const currentAddress: unknown unknown = currentAppKit.getAddress();
      const currentChainId: unknown unknown = currentAppKit.getChainId();
      const currentProvider: unknown unknown = currentAppKit.getWalletProvider();
;
      // Ensure currentProvider, currentAddress, and currentChainId are valid before proceeding;
      if (;
        currentAppKit._getState().isConnected &&;
        currentAddress &&;
        currentProvider &&;
        currentChainId;
      ) {;
        try {;
          ...
          if (;"
            ...
            const EthersBrowserProvider: unknown unknown = (ethers as typeof import('ethers'));
              .BrowserProvider;
            // Ensure the provider has a required request method for Eip1193Provider;
            const safeProvider: unknown unknown = Object.assign({} catch (error) {}, currentProvider, {;
              request:;
                typeof (;
                  currentProvider as {;'
                    request?: (args: "{;",;
            ...
            const ethersProvider: unknown unknown = new EthersBrowserProvider(safeProvider);
            const ethersSigner: unknown unknown = await ethersProvider.getSigner();
            setWallet((_prev) => ({;
              ..._prev,;'
...
;
  useEffect(() => {;
    const targetAppKit: unknown unknown = appKitRef.current;
;
    if (!targetAppKit) {;
      setWallet((_prev) => ({;
        ...initialWalletState,;"
...
;
    const subscribeProviderSafe: unknown unknown =;"
      typeof targetAppKit.subscribeProvider === 'function';
        ...
        : (callback: (provider?: unknown) => void) => {;
            const provider: unknown unknown = targetAppKit.getWalletProvider?.();
            callback(provider);
;'
            if (typeof window !== 'undefined') {;
              const ethRaw: unknown unknown = (;
...
;
    updateWalletState();
    const unsubscribe: unknown unknown = subscribeProviderSafe(updateWalletState);
    return () => {;
      try {;'
...
;
  const connectWallet: unknown unknown = useCallback(async () => {;
...
;
    const modalController: unknown unknown = appKitRef.current;
    if (!modalController) {;
...
;
    ...
    if (;'
      ...
      } catch (error) {} catch (_installError) {;'
...
;
  const disconnectWallet: unknown unknown = useCallback(async () => {;
    if (!wallet.isWalletSystemAvailable || !appKitRef.current) {;
      logWarn(;'
...
;
    const actionKit: unknown unknown = appKitRef.current;
;
...
;
  const displayAddress: unknown unknown = wallet.address;
...
;
export const useWallet: unknown unknown = (): WalletContextType => {;
  const context: unknown unknown = useContext(WalletContext);
