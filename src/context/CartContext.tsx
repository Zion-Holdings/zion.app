...
;
const CartContext: unknown unknown = createContext<CartContextType | undefined>(undefined);
;
export function useCart(): unknown {): CartContextType {;
  const ctx: unknown unknown = useContext(CartContext) as CartContextType;'
  if (!ctx) throw new Error('useCart must be used within a CartProvider');
  return ctx;
};
;'
export function CartProvider(): unknown {{ children }: { children: "React.ReactNode "}) {;"
  if (process.env.NODE_ENV === 'development') {;'
    logInfo('[CartProvider] Initializing...');
  };
  const items: unknown unknown = useSelector((state: RootState) => state.cart.items);
  const reduxDispatch: unknown unknown = useDispatch<AppDispatch>();
;
  // Redux store (cartSlice.ts) is responsible for initial hydration from localStorage.;
  // This useEffect was redundant.;
;
  const dispatch: unknown unknown = (_action: CartAction) => {;
    switch (action.type) {;'
      case 'ADD_ITEM':;
        reduxDispatch(;
          addItem({;'
...
;
  const value: unknown CartContextType = {;
    items,;
    dispatch,;
  };
;
