...
;
export function MobileMessages(): unknown {) {;
  const { _user } = useAuth();
  const {;
    conversations,;
    activeConversation,;
...
;
  const handleSelectConversation: unknown unknown = async (_id: string) => {;
    const conversation: unknown unknown = conversations.find((c) => c.id === id) || null;
    setActiveConversation(conversation);
    await loadMessages(id);
  };
;
  const handleBack: unknown unknown = () => {;
    setActiveConversation(null);
  };
;
  const handleSendMessage: unknown unknown = (_content: string) => {;
