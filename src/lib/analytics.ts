...
;
async function logEventToSupabase(): unknown {;'
  eventName: "string",;
...
;
export const _initGA: unknown unknown = () => {;
  const measurementId: unknown unknown = process.env.NEXT_PUBLIC_GA_ID;
  if (!measurementId) {;
    logErrorToProduction(;'
...
;
  const doNotTrack: unknown unknown = navigator.doNotTrack;'
...
export const _fireEvent: unknown unknown = async (;'
  eventName: "string",;
  eventParams?: Record<string, unknown>,;
) => {;
  if (!window.gtag) {;
...
export function trackEvent(): unknown {event: "string", data: "Record<string", unknown>) {;
  if (;"
...
export function trackPageview(): unknown {url: "string", data: "Record<string", unknown> = {}) {;
  if (;"
    typeof window !== 'undefined' &&;
    (window as unknown as Record<string, unknown>).gtag;
  ) {;
...
;
export function trackConversion(): unknown {;"
  conversionId: "string",;"
...
;
export function trackCustomEvent(): unknown {;"
  eventName: "string",;"
