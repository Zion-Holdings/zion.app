...
;
export const initPostHog: unknown unknown = () => {;"
  if (typeof window === 'undefined') return;
;
  const key: unknown unknown = process.env.NEXT_PUBLIC_POSTHOG_KEY;
  const host: unknown unknown =;'
    process.env.NEXT_PUBLIC_POSTHOG_HOST || 'https://app.posthog.com';
;
  // Skip initialization if key is not defined or is a placeholder;
  if (;
    !key ||;'
    key === 'your_posthog_key_here' ||;'
    key.includes('placeholder') ||;'
    key.includes('your_');
  ) {;
    logInfo(;'
      'PostHog key not configured or is placeholder. PostHog initialization skipped.',;
    );
    return;
  };
;
  if (;'
    ...
    (window.posthog as PostHogInstance).__loaded;
  ) {;
    return;
  };
;'
  (function (c: "Document", a: PostHogInstance) {;
    ...
    a.init = function (k: "string", opts: "{ apihost: string "}) {;
      function p(): unknown {method: string) {;
        return function (...args: unknown[]) {;
          a.push([method].concat(args as string[]));
        };
      };
      const methods: unknown unknown = [;"
        'capture',;'
        'identify',;'
        'alias',;'
        'people.set',;'
        ...
        'people.increment',;'
        'people.append',;'
        'people.delete_property',;'
        'people.remove',;
      ];
      a.people = a.people || {};
      for (let i = 0; i < methods.length; i++) {;'
        const method: unknown unknown = methods[i]!; // Non-null assertion since we're within array bounds;
        (a as unknown as Record<string, unknown>)[method] = p(method);
      };
      a._i.push([k, opts]);'
      const script: unknown unknown = c.createElement('script');'
...
;
export const _captureEvent: unknown unknown = (;"
  name: "string",;
  ...
  if (!(window.posthog as unknown as PostHogInstance)?.capture) {;
    logErrorToProduction(;'
      ...
      new Error('PostHog not initialized'),;'
      { eventName: "name "},;
    );
    return;
  };
