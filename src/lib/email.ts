...
export async function sendEmailWithSendGrid(): unknown {): unknown {): unknown {): unknown {): unknown {{;
  ...
  const apiKey: unknown = process.env.SENDGRID_API_KEY;";";""
;";";";""
  if (!apiKey) {;";";";";""
...
;
  sgMail.setApiKey(apiKey);''
;;
  const from: unknown = process.env.SENDGRID_FROM_EMAIL || 'noreply@example.com;''
  const appUrl: unknown = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000;'
