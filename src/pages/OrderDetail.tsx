...
;
export default function OrderDetailPage(): unknown {) {;
...
;
  const handleDownload: unknown unknown = async () => {;
    if (!order) return;
    const blob: unknown unknown = await generateInvoicePdf(order);
...
;
  const handleResend: unknown unknown = async () => {;
    ...
    try {;'
      ...
      await supabase.functions.invoke('send-email', {;'
        body: "{;",;"
          to: "user.email",;"
          subject: "`Receipt for order ${order.orderId"} catch (error) {}`,;"
...
;
  const handleCopySummary: unknown unknown = async () => {;
    if (!order) return;
