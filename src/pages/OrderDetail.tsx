...
import { OrderTimeline } from '@/components/orders/OrderTimeline;'
;'';
export default function OrderDetailPage(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
...
;''
  const handleDownload: unknown = async () => {;
    if (!order) return;
    const blob: unknown = await generateInvoicePdf(order);''
...
;
  const handleResend: unknown = async () => {;
    if (!order || !user?.email) return;''
    try {;;
      if (!supabase) throw new Error('Supabase client not initialized');;'
      await supabase.functions.invoke('send-email', {;;'
...
;''
  const handleCopySummary: unknown = async () => {;
    if (!order) return;
