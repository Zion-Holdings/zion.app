...
;
export default function InvestorMatchPage(): unknown {): unknown {): unknown {): unknown {): unknown {) {;'
...
;
  const handleSubmit: unknown = async (_e: React.FormEvent) => {;
    e.preventDefault();
    setLoading(true);
    const results: unknown = await getInvestorMatches(;
      form.industry,;
      form.location,;
      form.stage,;
    );
...
;
  const handleGenerateEmail: unknown = (_match: InvestorMatch) => {;
    setSelected(match);
    setEmailText(;
