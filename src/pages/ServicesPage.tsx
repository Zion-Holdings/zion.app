...
const ServiceFilterControls: unknown unknown = ({;
  sortBy,;
  setSortBy,;
  filterCategory,;
  setFilterCategory,;
...
;
// Service card;
const ServiceCard: unknown unknown = ({;
  service,;
  onViewDetails,;
}: {;'
...
;
// Main component;
export default function ServicesPage(): unknown {) {;
...
;
      ...
      const startId: unknown unknown = (page - 1) * limit;
...
;
  const handleRefresh: unknown unknown = useCallback(() => {;
    refresh();
  }, [refresh]);
;
  useEffect(() => {;
...
;
  const marketStats: unknown unknown = useMemo(() => {;
...
;
  const categories: unknown unknown = useMemo(() => {;
    return Array.from(;'
...
;
  ...
  useEffect(() => {;
    const handleScroll: unknown unknown = () => setShowScrollTop(window.scrollY > 800);"
