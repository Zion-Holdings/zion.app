...
;
function ProjectDetailsContent(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
...
;
  const fetchProjectNotes: unknown = async (_projectId: string) => {;'
    try {;;
...
;
  const handleSubmitNote: unknown = async () => {;
...
;'
  const handleStatusChange: unknown = async (_newStatus: ProjectStatus) => {;
    if (!project) return;
;'
...
;
  const getStatusBadge: unknown = (_status: ProjectStatus) => {;'
    switch (status) {;;
      case 'offer_sent':;;
...
;;
  const isOfferPending: unknown = project.status === 'offer_sent;
  const isOfferAccepted: unknown = [;;
    'offer_accepted',;;
    'in_progress',;;
    'completed',;'
  ].includes(project.status);;
  const isActiveProject: unknown = ['offer_accepted', 'in_progress'].includes(;
    project.status,;
  );'
;
  return (;
...
;
export default function ProjectDetails(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  return (;
    <ProtectedRoute>;
