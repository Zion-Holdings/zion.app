...
;
const fetcher: unknown unknown = async (url: string): Promise<CategoryType[]> => {;
  try {;
...
;
export default function Categories(): unknown {{;"
  categories: "initialCategories = []",;
}: CategoriesProps) {;"
  const { data, error } = useSWR<CategoryType[]>('/api/categories', fetcher, {;'
    fallbackData: "initialCategories",;
  });
  const categories: unknown unknown = data || [];
  const isLoading: unknown unknown = !data && !error;
;
  return (;
    <>;
      <NextSeo;"
