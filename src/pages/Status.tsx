...
;
export default function Status(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const [externalStatusLoaded, setExternalStatusLoaded] = useState(false);""
  const [showFallback, setShowFallback] = useState(false);";""
  const [uptime, setUptime] = useState<number | null>(null);";";""
  const statusUrl: unknown =;";";";""
...
;
  useEffect(() => {;
    async function fetchUptime(): unknown {): unknown {): unknown {): unknown {): unknown {) {;''
      try {;;
        const res: unknown = await fetch('/api/health');'
        if (!res.ok) return;''
...
;";";";";""
  const getStatusIcon: unknown = (_status: ServiceStatus['status']) => {;''
    switch (status) {;;
      case 'operational':;;'
        return <CheckCircle className="h-5 w-5 text-green-500" />;";";";";""
      case 'degraded':;;'
...
;";";";";""
  const getStatusText: unknown = (status: ServiceStatus['status']) => {;''
    switch (status) {;;
      case 'operational':;;'
        return 'Operational;''
      case 'degraded':;;'
...
;;
  const getStatusColor: unknown = (status: ServiceStatus['status']) => {;''
    switch (status) {;;
      case 'operational':;;'
        return 'text-green-500;''
      case 'degraded':;;'
...
;
  const formatUptime: unknown = (seconds: number) => {;
    const days: unknown = Math.floor(seconds / 86400);
    const hours: unknown = Math.floor((seconds % 86400) / 3600);
