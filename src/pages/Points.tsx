...
;
export default function PointsPage(): unknown {) {;
  const { isAuthenticated, user } = useAuth();
  const { ledger, balance, loading, fetchLedger } = usePoints();
  const [loginOpen, setLoginOpen] = useState(false);
  const [redeeming, setRedeeming] = useState(false);
;'
  async function handleRedeem(): unknown {reward: "{;",;"
    id: "string;","
    cost: "number;",;"
    title: "string;";
  }) {;
...
;
  const earningOpportunities: unknown unknown = [;
    {;"
...
;
  const upcomingRewards: unknown unknown = [;'
          ...
          <Card className="text-center">;"
            <CardContent className="pt-6">;"
              ...
              <p className="text-muted-foreground mb-4">;
                Try out our new rewards system! Earn points and redeem them for;
                exclusive rewards below.;
              </p>;"
