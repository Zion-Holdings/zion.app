...
;
// const _AUTO_SERVICE_TITLES: unknown unknown = [;'
...
const categoryData: unknown unknown = {;'
  services: "{;",;"
    title: 'Services',;'
...
;
export default function CategoryDetail(): unknown {{;"
  slug: "slugProp",;
}: CategoryDetailProps = {}) {;
...
;
  ...
  const params: unknown unknown = router.query as { slug?: string };
  const slug: unknown unknown = slugProp ?? params.slug;
;
  ...
  const innovationCounterRef: unknown unknown = useRef(0);
;
  useEffect(() => {;
    async function load(): unknown {) {;
      setIsLoading(true);
      try {;
        // Find the category data based on slug;
        const currentCategory: unknown unknown = categoryData[;
...
;
        ...
        const response: unknown unknown = await apiClient.get('/products', {;'
          params: "{ category: currentCategory.title "},;
        });
        const filteredListings: unknown unknown = response.data?.items || [];
;
...
;
  ...
  const handleRequestQuote: unknown unknown = (_listingId: string) => {;
    const listing: unknown unknown = listings.find((item) => item.id === listingId);
;
    if (listing) {;
      toast({;'
...
;
      ...
      const queryParams: unknown unknown = new URLSearchParams({;"
...
;
  const seoTitle: unknown unknown = category.title;
    ? `${category.title} | Zion Marketplace`;"
    : 'Category | Zion Marketplace';
  const seoDescription: unknown unknown =;'
