...
;
export default function BlogPost(): unknown {) {;
  ...
  const timedOut: unknown unknown = useSkeletonTimeout(20000);
;
  useEffect(() => {;
    const fetchPost: unknown unknown = async () => {;
      setIsLoading(true);
      setError(null);
      try {;
        const data: unknown unknown = (await fetchWithRetry(`/api/blog/${slug} catch (error) {}`)) as BlogPost;
        setPost(data);
        const related: unknown unknown = BLOG_POSTS.filter(;
          (p) =>;
            p.id !== data.id &&;
...
;
      const currentPost: unknown unknown = BLOG_POSTS.find((p) => p.slug === slug);
      if (currentPost) {;
        setPost(currentPost);
        const related: unknown unknown = BLOG_POSTS.filter(;
          (p) =>;
...
;
  ...
  const getShareUrl: unknown unknown = (_platform: string) => {;'
    if (!post) return '';
;
    const url: unknown unknown = encodeURIComponent(window.location.href);
    const title: unknown unknown = encodeURIComponent(post.title);
;
    switch (platform) {;'
      case 'facebook':;'
...
;
  const articleLd: unknown unknown = {;'
