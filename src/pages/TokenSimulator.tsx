...
;
  const velocityChart: unknown = useRef<HTMLCanvasElement | null>(null);
  const supplyChart: unknown = useRef<HTMLCanvasElement | null>(null);
;
  const handleChange: unknown =;
    (field: keyof Inputs) => (_e: React.ChangeEvent<HTMLInputElement>) => {;
      const value: unknown = parseFloat(e.target.value);
