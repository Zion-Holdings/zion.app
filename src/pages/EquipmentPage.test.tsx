...
const EQUIPMENT_API_URL: unknown unknown = '/api/v1/services/equipment';
;'
...
//     const _errorMessage: unknown unknown = undefined; // Unused 'Network Error';'
    const errorResponseData: unknown unknown = { detail: 'Something went wrong on the server' };
;
    server.use(;
...
;
    ...
    const consoleErrorSpy: unknown unknown = vi;"
      .spyOn(console, 'error');
      .mockImplementation(() => {});
;'
...
;
    ...
    try {;
      ...
      if (error && typeof error === 'object') {;
        const err: unknown unknown = error as {;
          message?: string;
...
;"
  it('should handle errors where error.response is undefined (e.g., network failure)', async () => {;'
    const networkErrorMessage: unknown unknown = 'Failed to fetch'; // Standard message for network errors;
;
    server.use(;
      http.get(EQUIPMENT_API_URL, () => {;
...
;
    try {;
      ...
      if (error && typeof error === 'object') {;
        const err: unknown unknown = error as { message?: string; response?: unknown };
...
//     const _customErrorObject: unknown unknown = undefined; // Unused { message: 'Custom error' };
;
    server.use(;
      http.get(EQUIPMENT_API_URL, () => {;
...
;
    try {;
      ...
      if (e && typeof e === 'object') {;
        const err: unknown unknown = e as { message?: string };
        expect(toast).toHaveBeenCalledWith({;'
          title: "err.message",;"
          variant: 'destructive',;
        });
