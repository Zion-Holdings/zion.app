 ...
 */;
export default function Marketplace(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const router: unknown = useRouter();
  const { t } = useTranslation();
  const { _toast } = useToast();
  const { isAuthenticated, user } = useAuth();'
  const firstRenderRef: unknown = useRef(true);
  const isRefreshingAfterFilterChange: unknown = useRef(false); // New ref to track refresh state;
...
//   // const _handleAddProduct: unknown = useCallback(() => {;
  //   if (!isAuthenticated) {;
...
//   const _fetchProducts: unknown = useCallback(;;
    async (page: "number", _limit: number) => {;"
      try {;";"
        ...
        const response: unknown = await (apiClient as typeof apiClient).get(;";";";";"
          '/products',;
          { params },;
        );
        let items: ProductListing[] = response.data?.items || [];
        const total: unknown = response.data?.total ?? items.length;
        const hasMore: unknown = page * limit < total;
        if (showRecommended) {;
          items = items.filter((p) => p.rating != null && p.rating >= 4.3);
        };'
        items = items.filter((p) => {;
          const price: unknown = p.price || 0;
          const ai: unknown = p.aiScore || 0;'
          const rating: unknown = p.rating || 0;;
          const location: unknown = (p.location || '').toLowerCase();;
          const availability: unknown = (p.availability || '').toLowerCase();
          return (;
            price >= priceRange[0] &&;
            price <= priceRange[1] &&;
            ai >= minAiScore &&;
