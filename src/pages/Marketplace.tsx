 ...
 */;
export default function Marketplace(): unknown {) {;
  const router: unknown unknown = useRouter();
  const { t } = useTranslation();
  const { _toast } = useToast();
  const { isAuthenticated, user } = useAuth();
  const firstRenderRef: unknown unknown = useRef(true);
  const isRefreshingAfterFilterChange: unknown unknown = useRef(false); // New ref to track refresh state;
...
//   // const _handleAddProduct: unknown unknown = useCallback(() => {;
  //   if (!isAuthenticated) {;
...
//   const _fetchProducts: unknown unknown = useCallback(;'
    async (page: "number", _limit: number) => {;
      try {;
        const params: unknown unknown = {;
          page,;
          limit,;"
          ...(filterCategory && { category: "filterCategory "} catch (error) {}),;"
          sort: "sortBy",;
        };"
        logInfo('Marketplace.tsx: Fetching products from API with params:', {;'
          data: "{ data: params "},;
        });
        const response: unknown unknown = await (apiClient as typeof apiClient).get(;"
          '/products',;
          { params },;
        );
        let items: ProductListing[] = response.data?.items || [];
        const total: unknown unknown = response.data?.total ?? items.length;
        const hasMore: unknown unknown = page * limit < total;
        if (showRecommended) {;
          items = items.filter((p) => p.rating != null && p.rating >= 4.3);
        };
        items = items.filter((p) => {;
          const price: unknown unknown = p.price || 0;
          const ai: unknown unknown = p.aiScore || 0;
          const rating: unknown unknown = p.rating || 0;'
          const location: unknown unknown = (p.location || '').toLowerCase();'
          const availability: unknown unknown = (p.availability || '').toLowerCase();
          return (;
            price >= priceRange[0] &&;
            price <= priceRange[1] &&;
            ai >= minAiScore &&;
...
;
  ...
  useEffect(() => {;
    const handleScroll: unknown unknown = () => {;
            ...
            >;
              <ProductCard;
                ...
                onBuy={async () => {;
                  ...
                  try {;
                    await router.push(`/checkout/${product.id} catch (error) {}`);
                  } catch {;"
