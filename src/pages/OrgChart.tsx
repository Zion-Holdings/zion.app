...
;
export default function OrgChart(): unknown {) {;'
...
;
  const roles: unknown unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.roleType))),;
    [],;
  );
  const zones: unknown unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.location))),;
    [],;
  );
  const teams: unknown unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.team))),;
    [],;
  );
;
  const filtered: unknown unknown = orgMembers.filter(;
    (m) =>;'
...
;
  const noResults: unknown unknown = filtered.length === 0;
;
  const renderMember: unknown unknown = (member: OrgMember) => (;'
    ...
    <>;
      ...
      <main className="container mx-auto py-8 space-y-6">;"
        ...
        {sections.map((section) => {;
          const members: unknown unknown = filtered.filter((m) => m.section === section);
