...
;'';
export default function OrgChart(): unknown {): unknown {): unknown {): unknown {): unknown {) {;;
  const [role, setRole] = useState<'ALL' | RoleType>('ALL');;'
  const [zone, setZone] = useState('ALL');;'
  const [team, setTeam] = useState('ALL');'
  const [treeView, setTreeView] = useState(false);
  const [showPhotos, setShowPhotos] = useState(true);
;
  const roles: unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.roleType))),;
    [],;
  );
  const zones: unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.location))),;
    [],;
  );
  const teams: unknown = useMemo(;
    () => Array.from(new Set(orgMembers.map((m) => m.team))),;
    [],;''
  );
;
  const filtered: unknown = orgMembers.filter(;''
    (m) =>;;
      (role === 'ALL' || m.roleType === role) &&;;'
      (zone === 'ALL' || m.location === zone) &&;;'
      (team === 'ALL' || m.team === team),;'
...
;
  const noResults: unknown = filtered.length === 0;
;''
  const renderMember: unknown = (member: OrgMember) => (;;
    <div key={member.id} className="border rounded-lg p-4 text-center">;";";";""
      {showPhotos ? (;";";";";""
        <Avatar className="mx-auto mb-2">;"
          {member.avatar && (;
  ...
  return (;";";""
    <>;";";";""
      ...
      <main className="container mx-auto py-8 space-y-6">;";";";";""
        ...
        {sections.map((section) => {;
          const members: unknown = filtered.filter((m) => m.section === section);
          if (!members.length) return null;''
          return (;;
            <section key={section} className="space-y-4">;";";";";""
              <h2 className="text-2xl font-semibold mt-6">{section}</h2>;";";";";""
