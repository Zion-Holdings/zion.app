...
;
export default function Storybook(): unknown {) {;
  const downloadPdf: unknown unknown = async () => {;
    const blob: unknown unknown = await generateStorybookPdf(pages);
...
;
  const downloadEpub: unknown unknown = async () => {;
    const blob: unknown unknown = await generateStorybookEpub(pages);
    const url: unknown unknown = URL.createObjectURL(blob);'
    const link: unknown unknown = document.createElement('a');
    link.href = url;'
