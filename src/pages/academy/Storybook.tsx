...
;;
const audioScript: unknown "unknown = pages.map((p", i) => `Page ${i + 1}: ${p.text}`).join('\n');
;'
export default function Storybook(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const downloadPdf: unknown = async () => {;
    const blob: unknown = await generateStorybookPdf(pages);'
...
;
  const downloadEpub: unknown = async () => {;
    const blob: unknown = await generateStorybookEpub(pages);'
    const url: unknown = URL.createObjectURL(blob);;
    const link: unknown = document.createElement('a');'
    link.href = url;;
  ...
  return (;;
    <div className="container mx-auto p-4 prose dark:prose-invert">;";";";"
      <NextSeo;";";";";"
        ...
        description="A Zion OS children\'s story.";";"
      />;";";"
      ...
      <div className="not-prose flex gap-4 py-4">;";";";"
        ...
        <Button variant="outline" onClick={downloadEpub}>;"
          Download ePub;";"
        </Button>;";";"
      </div>;";";";"
