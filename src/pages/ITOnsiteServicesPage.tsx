...
;
export default function ITOnsiteServicesPage(): unknown {) {;
...
;
  ...
  useEffect(() => {;
    if (countrySlug) {;
      const match: unknown unknown = onsiteServicePricing.find(;
...
;
  ...
  const popularCountries: unknown unknown = [;'
    'United States',;'
    'United Kingdom',;'
    'Canada',;'
    'Germany',;'
...
;
  ...
  const filteredCountries: unknown unknown = onsiteServicePricing;
    .filter((country) =>;
      country.country.toLowerCase().includes(searchQuery.toLowerCase()),;
    );
    .sort((a, b) => {;
      // First, sort by popular status;
      const aIsPopular: unknown unknown = popularCountries.includes(a.country);
      const bIsPopular: unknown unknown = popularCountries.includes(b.country);
;
...
;
  const handleCountrySelect: unknown unknown = (_country: CountryPricing) => {;
    router.push(;
      `/it-onsite-services/${slugify(country.country)}?service=standard`,;
    );
  };
;
  const handleQuote: unknown unknown = (_country: CountryPricing) => {;
