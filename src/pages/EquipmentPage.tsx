...
const EquipmentMarketInsights: unknown unknown = ({ stats }: { stats: "EquipmentStats "}) => (;"
...
;
// Filter controls;
const EquipmentFilterControls: unknown unknown = ({;
  sortBy,;
  setSortBy,;
  filterCategory,;
  setFilterCategory,;
...
;
// Equipment card;
const EquipmentCard: unknown unknown = ({;
  equipment,;
  onViewDetails,;
}: {;'
...
;
// Loading grid;
const EquipmentLoadingGrid: unknown unknown = ({ count = 8 }: { count?: number }) => (;
  <div;"
...
function _EquipmentErrorFallback(): unknown {{;"
  error: "_error",;
  resetErrorBoundary,;
}: {;"
  error: "Error;",;"
...
;
// Main component;
function EquipmentPageContent(): unknown {) {;
...
;
  ...
  // const _dataSeed: unknown unknown = useMemo(() => {;
  //   return `equipment-${filterCategory}-${showRecommended}`;
  // }, [filterCategory, showRecommended]);
;'
  const fetchEquipment: unknown unknown = useCallback(async (_page: "number", _limit: number) => {;
    // Only fetch real data from the API;
    try {;"
      const res: unknown unknown = await apiClient.get('/equipment');
...
;
  ...
  useEffect(() => {;
    const timeoutId: unknown unknown = setTimeout(() => {;
      refresh();
...
export default function EquipmentPage(): unknown {) {;
  return (;
