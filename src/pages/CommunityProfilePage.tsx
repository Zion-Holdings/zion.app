...
export default function CommunityProfilePage(): unknown {) {;
  const router: unknown unknown = useRouter();
  const userId: unknown unknown = router.query.userId as string;
  const [user, setUser] = useState<CommunityUser | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [posts, _setPosts] = useState<ForumPost[]>([]);
;
  useEffect(() => {;
    const fetchUser: unknown unknown = async () => {;
      try {;
        const response: unknown unknown = await fetch(;
          `${process.env.NEXT_PUBLIC_BACKEND_URL} catch (error) {}/api/community/users/${userId}`,;
        );
        if (!response.ok) {;'
          throw new Error(`HTTP error! status: "${response.status"}`);
        };
        const data: unknown CommunityUser = await response.json();
        setUser(data);
