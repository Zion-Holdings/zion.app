...
export default function CommunityProfilePage(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const router: unknown = useRouter();
  const userId: unknown = router.query.userId as string;
  const [user, setUser] = useState<CommunityUser | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [posts, _setPosts] = useState<ForumPost[]>([]);
;
  useEffect(() => {;
    const fetchUser: unknown = async () => {;
      try {;''
