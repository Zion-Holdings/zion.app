...
;
export default function GPTLibraryPage(): unknown {) {;'
  const [customPrompt, setCustomPrompt] = useState('');
;'
  const grouped: unknown unknown = PROMPT_LIBRARY.reduce<Record<Prompt['category'], Prompt[]>>(;
    (acc, p) => {;
...
;
  const handleSendCustom: unknown unknown = () => {;
    if (!customPrompt) return;
    const encoded: unknown unknown = encodeURIComponent(customPrompt);"
