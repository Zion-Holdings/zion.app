...
;
  useEffect(() => {;
    const confirmVerification: unknown unknown = async () => {;
      try {;'
...
;
        ...
        const response: unknown unknown = await fetch('/api/auth/confirm-email-verification', {;'
          method: 'POST',;
          headers: {;'
...
;
        const responseData: unknown unknown = await response.json();
;
        if (response.ok) {;'
          setStatus('success');
          setMessage(;
...
;
    ...
    if (;'
      ...
      const params: unknown unknown = new URLSearchParams(window.location.hash.substring(1)); // remove #;'
      const errorDescription: unknown unknown = params.get('error_description');
      logErrorToProduction(;'
