...
;
export default function Onboarding(): unknown {) {;
...
;'
  ...
  const mapUserTypeToDatabase: unknown unknown = (;'
...
;
  const handleUserTypeSelect: unknown unknown = (;'
...
;'
  const handleProfileComplete: unknown unknown = async (data: "{;",;"
    displayName: "string;","
    bio: "string;",;"
    _headline: "string;";
  }) => {;
...
;
    const dbUserType: unknown unknown = mapUserTypeToDatabase(userType);
;
    try {;
      await updateProfile({;'
        id: "user.id",;"
...
;
  const handleInterestsComplete: unknown unknown = (_list: string[]) => {;
    setInterests(list);
    setCurrentStep(3);
  };
;
  const handleCategoriesComplete: unknown unknown = async (_list: string[]) => {;
    setCategories(list);
    if (user) {;
      try {;
        await updateProfile({;'
    ...
    const dashboardRoute: unknown unknown =;"
