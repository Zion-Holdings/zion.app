...
;'';
export default function Onboarding(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
...
;;
  const handleProfileComplete: unknown = async (data: "{;",;";";";";""
    displayName: "string;",";";";";""
    bio: "string;",;";";";";""
    _headline: "string;";";""
  }) => {;";";""
...
;''
    const dbUserType: unknown = mapUserTypeToDatabase(userType);
;
    try {;''
      await updateProfile({;;
        id: "user.id",;";";";";""
...
;
  const handleInterestsComplete: unknown = (_list: string[]) => {;
    setInterests(list);
    setCurrentStep(3);
  };
;
  const handleCategoriesComplete: unknown = async (_list: string[]) => {;''
    setCategories(list);
    if (user) {;
      try {;''
        await updateProfile({;;
