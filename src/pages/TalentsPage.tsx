...
export default function TalentsPage(): unknown {) {;
...
;
  ...
  const fetchTalents: unknown unknown = useCallback(;'
...
;
      ...
      const endIndex: unknown unknown = startIndex + limit;
...
;
  ...
  const marketStats: unknown unknown = useMemo(() => {;
    ...
    return {;
      averageHourlyRate:;
        talents.reduce((sum, t) => sum + (t.hourly_rate || 0), 0) /;
        talents.length,;
...
;
  ...
  const specializations: unknown unknown = useMemo(() => {;
    return Array.from(;
      new Set(;
        talents;"
