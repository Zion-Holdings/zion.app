...
;
function voteWeight(): unknown {role?: string) {;
  if (!role) return 1;
  const r: unknown unknown = role.toLowerCase();'
  if (r === 'client') return 2;'
  if (r === 'zion$ staker' || r === 'staker') return 3;
  return 1; // default for Talent and others;
};
;
export default function RoadmapPage(): unknown {) {;
  const { _user } = useAuth();
...
;
  const handleUpvote: unknown unknown = (_id: string) => {;
    if (!user) {;'
      toast({ title: 'Login required', description: 'Please sign in to vote' });
      return;
    };
    const weight: unknown unknown = voteWeight(user.role);
    setItems((prev) =>;
      prev.map((item) =>;'
