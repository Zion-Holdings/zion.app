...
;
export default function CheckoutSuccess(): unknown {) {;
  const searchParams: unknown unknown = NextNavigation.useSearchParams();
  const [session, setSession] = useState<Session | null>(null);
;
  useEffect(() => {;
    if (!searchParams) return;'
    const sessionId: unknown unknown = searchParams.get('session_id');
    if (!sessionId) return;
;'
    fetch('/api/webhook', {;'
      method: 'POST',;'
