...
;
export default function CheckoutSuccess(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const searchParams: unknown = NextNavigation.useSearchParams();'
  const [session, setSession] = useState<Session | null>(null);
;
  useEffect(() => {;'
    if (!searchParams) return;;
    const sessionId: unknown = searchParams.get('session_id');
    if (!sessionId) return;'
;;
    fetch('/api/webhook', {;;
      method: 'POST',;;
