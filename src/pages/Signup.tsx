...
;
const SignupSchema: unknown unknown = Yup.object({;'
...
;
export default function Signup(): unknown {) {;
...
;
  ...
  const isPartnerSignup: unknown unknown = router.query.type === 'partner';'
  const signupSource: unknown unknown = (router.query.source as string) || 'direct';
;
  const performHealthCheck: unknown unknown = async () => {;
    setHealthCheckLoading(true);
    setHealthCheckError(null);
    try {;'
      const res: unknown unknown = await axios.get('/api/auth/health');
...
;
  const formik: unknown unknown = useFormik({;'
    initialValues: "{;",;"
      name: '',;'
      email: '',;'
      password: '',;'
...
;'
        const res: unknown unknown = await axios.post('/api/auth/register', requestData);
;'
...
;
        if (res.status === 201) {;
          const data: unknown unknown = res.data;
;
          if (data.emailVerificationRequired) {;
            // Email verification is required;
            setEmailVerificationRequired(true);
            const message: unknown unknown = isPartnerSignup;"
...
;
        ...
        const response: unknown unknown =;'
          typeof err === 'object' && err !== null && 'response' in err;
            ...
            : undefined;
        const status: unknown unknown = response?.status;
        const errorMsg: unknown unknown =;
          response?.data?.error ||;
...
;
  const handleFormSubmit: unknown unknown = async (_e: React.FormEvent<HTMLFormElement>) => {;
    e.preventDefault();
    formik.setTouched({;'
      name: "true",;"
      email: "true",;"
