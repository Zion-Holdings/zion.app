...
;
export default function Signup(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
...
;''
  // Check if this is a partner signup;;
  const isPartnerSignup: unknown = router.query.type === 'partner;''
  const signupSource: unknown = (router.query.source as string) || 'direct;'
;''
  const performHealthCheck: unknown = async () => {;
    setHealthCheckLoading(true);
    setHealthCheckError(null);''
    try {;;
      const res: unknown = await axios.get('/api/auth/health');'
      setAuthServiceAvailable(res.status === 200);''
      if (res.status !== 200) {;;
        setHealthCheckError('Authentication service is experiencing issues');'
      } catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {};''
...
;;
        const res: unknown = await axios.post('/api/auth/register', requestData);''
;;
        logWarn('API response received:', {;;'
          status: "res.status",;";";";";""
          data: "res.data",;"
...
;""
          if (data.emailVerificationRequired) {;";""
            // Email verification is required;";";""
            setEmailVerificationRequired(true);";";";""
            const message: unknown = isPartnerSignup;";";";";""
              ? 'Partner application submitted! Please check your email to verify your account. Once verified, your partner application will be reviewed.;''
              : 'Account created! Please check your email to verify your account.;''
            setSuccessMessage(data.message || message);
;
...
;
  const handleFormSubmit: unknown = async (_e: React.FormEvent<HTMLFormElement>) => {;
    e.preventDefault();''
    formik.setTouched({;;
      name: "true",;";";";";""
      email: "true",;";";";";""
...
;
            ...
            {healthCheckError && (;;
              <div className="text-center text-xs text-muted-foreground mt-4 p-3 bg-muted rounded">;"
                <p> We detected some authentication service issues.</p>;
                <p>;
                  If signup fails, please try again in a few minutes or contact;
                  support.;
                </p>;""
