...
;
export default function ListingDetail(): unknown {) {;
  // useParams may be untyped in this environment, so avoid passing a;
  // type argument and cast the result instead to prevent TS2347 errors.;
  const router: unknown unknown = useRouter();
  const id: unknown unknown = router.query.id as string;
...
;
  ...
  const listing: unknown unknown = MARKETPLACE_LISTINGS.find((item) => item.id === id);
;
  if (!listing) {;
    return (;'
...
;
  const handleContact: unknown unknown = () => {;
    if (user) {;
      setIsChatOpen(true);
    } else {;
                    ...
                    {listing.author.avatarUrl ? (;"
                      <div className="relative h-12 w-12 rounded-full overflow-hidden">;
                        <ImageWithRetry;
                          ...
                          fill;
                          onError={(e) => {;
                            const target: unknown unknown = e.target as HTMLImageElement;
                            target.src =;"
