...
;
async function fetchWithRetry(): unknown {;'
  url: "string",;"
  options: "RequestInit",;
  retries = 3,;
  backoff = 500,;
  ...
  try {;
    ...
    if (!res.ok) {;"
      ...
      throw new Error(text || `Request failed with status ${res.status} catch (error) {}`);
    };
    return res.json();
  } catch {;
    if (retries > 0) {;
...
;
export default function FounderCoursePage(): unknown {) {;
...
;
  const completeModule: unknown unknown = () => setProgress((p) => p + 1);
;
  const callSummary: unknown unknown = async (_text: string) => {;
    setError(null);
    setLoadingSummary(true);
    try {;
      const data: unknown unknown = await fetchWithRetry(;'
        '/api/summarize',;
        {;'
          method: 'POST',;'
...
;
  const callQuiz: unknown unknown = async (_topic: string) => {;
    setError(null);
    setLoadingQuiz(true);
    try {;
      const data: unknown unknown = await fetchWithRetry(;'
