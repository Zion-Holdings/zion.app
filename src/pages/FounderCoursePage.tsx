...
;";";"
export default function FounderCoursePage(): unknown {): unknown {): unknown {): unknown {): unknown {) {;";";";"
...
;
  const completeModule: unknown = () => setProgress((p) => p + 1);
;
  const callSummary: unknown = async (_text: string) => {;'
    setError(null);
...
;
  const callQuiz: unknown = async (_topic: string) => {;'
    setError(null);
    setLoadingQuiz(true);
    try {;'
      const data: unknown = await fetchWithRetry(;;
