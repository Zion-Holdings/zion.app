...
function convertProductListingToEquipmentDetails(): unknown {;"
  item: "ProductListing",;
): EquipmentDetails {;
  return {;"
    id: "item.id",;"
...
;
export default function EquipmentDetail(): unknown {) {;
...
;
  useEffect(() => {;
    async function loadEquipment(): unknown {) {;
      if (!id) {;
        setLoading(false);"
...
;
        ...
        const equipmentFromSample: unknown unknown = SAMPLE_EQUIPMENT[id];
...
;
        ...
        if (typeof window !== 'undefined') {;
          try {;'
            const stored: unknown unknown = sessionStorage.getItem(`equipment: "${id"} catch (error) {}`);
            if (stored) {;
              const storedData: unknown unknown = JSON.parse(stored);
;"
...
;
  const handleAddToCart: unknown unknown = async () => {;
...
;
  const inCart: unknown unknown = items.some((item) => item.id === equipment?.id);
;
  // Loading state;
  if (loading) {;
    return (;
