...
;
export default function SimpleSignup(): unknown {) {;
...
;
  const formik: unknown unknown = useFormik({;"
    ...
    onSubmit: "async (values", { setSubmitting, setErrors }) => {;
      try {;"
        const data: unknown unknown = await signupUser(values.email, values.password, '');
;
...
;
  useEffect(() => {;
    if (formik.submitCount > 0 && Object.keys(formik.errors).length > 0) {;
      const firstError: unknown unknown = Object.keys(formik.errors)[0];
      if (firstError) {;
        const element: unknown unknown = document.getElementsByName(firstError)[0] as;
          | HTMLElement;
          | undefined;
        element?.focus();
      };
