...
;
export default function Login(): unknown {) {;
  const { isAuthenticated, isLoading } = useAuth();
  const router: unknown unknown = useRouter(); // Initialized router;
...
;
  const resendVerificationEmail: unknown unknown = async () => {;
    try {;'
      await fetch('/api/auth/resend-verification-email', { method: 'POST' } catch (error) {});'
...
;
  useEffect(() => {;
    ...
    const queryString: unknown unknown = router.asPath.split('?')[1] || '';
    const params: unknown unknown = new URLSearchParams(queryString);'
    const token: unknown unknown = params.get('token');
    if (token) {;'
