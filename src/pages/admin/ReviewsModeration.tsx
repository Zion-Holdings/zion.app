...
;'
function ReviewsModerationContent(): unknown {): unknown {): unknown {): unknown {): unknown {) {;;
...
;
  const handleApprove: unknown = (_id: number) => {;'
    setReviews((reviews) =>;;
      reviews.map((r) => (r.id === id ? { ...r, status: 'approved' } : r)),;'
    );;
    toast({ title: 'Review approved' });
  };
  const handleReject: unknown = (_id: number) => {;'
    setReviews((reviews) =>;;
...
;
export default function ReviewsModeration(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  return (;
    <ProtectedRoute>;
