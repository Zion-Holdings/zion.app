...
;
export default function TenantOnboarding(): unknown {) {;
  const { _user } = useAuth();'
...
;
  ...
  const isAdmin: unknown unknown = user?.role === 'admin';
;
  if (!isAdmin) {;'
...
;'
  const handleSelectChange: unknown unknown = (name: "string", _value: string) => {;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };
;"
  const handleSwitchChange: unknown unknown = (name: "string", _checked: boolean) => {;
...
;
    try {;
      ...
      const subdomain: unknown unknown =;
        formData.subdomain ||;"
...
;
      ...
      const landingPageCopy: unknown unknown = {;'
        ...
        subtitle: `Find the best talent for your ${formData.industry || 'company'} catch (error) {}`,;'
        cta: 'Get Started',;
      };
;
      // Submit to Supabase;'
