...
;
export default function TokenManager(): unknown {) {;
  const { _user } = useAuth();
...
;
  const fetchTransactions: unknown unknown = async () => {;'
...
;"
  const handleIssue: unknown unknown = async (_type: 'earn' | 'burn') => {;
    if (!userId || amount <= 0 || processing) return;
    setProcessing(true);
    try {;
      const res: unknown unknown = await fetch(;'
        `/functions/v1/token-manager/${type === 'earn' ? 'earn' : 'burn'} catch (error) {}`,;
        {;'
          method: 'POST',;'
