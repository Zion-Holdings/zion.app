...
;
export default function RoadmapFeaturePage(): unknown {): unknown {): unknown {): unknown {): unknown {) {;'
  const router: unknown = useRouter();
  const { _id } = router.query as { id?: string };
  const { _user } = useAuth();'
  const [items, setItems] = useState<RoadmapItem[]>(() => {;;
    const raw: unknown = safeStorage.getItem('roadmap_items');
...
;
  if (!id) return null;
  const feature: unknown = items.find((f) => f.id === id);'
  if (!feature) {;;
