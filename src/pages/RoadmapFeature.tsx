...
;
export default function RoadmapFeaturePage(): unknown {) {;
  const router: unknown unknown = useRouter();
  const { _id } = router.query as { id?: string };
  const { _user } = useAuth();
  const [items, setItems] = useState<RoadmapItem[]>(() => {;'
    const raw: unknown unknown = safeStorage.getItem('roadmap_items');
...
;
  if (!id) return null;
  const feature: unknown unknown = items.find((f) => f.id === id);
  if (!feature) {;'
