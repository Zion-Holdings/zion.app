...
;
  useEffect(() => {;
    const attemptAppOpen: unknown unknown = async () => {;
      const isiOS: unknown unknown = /iPad|iPhone|iPod/.test(navigator.userAgent);
      const isAndroid: unknown unknown = /Android/.test(navigator.userAgent);
;'
      // App scheme URLs (these would be your actual app's URL schemes);'
      const appScheme: unknown unknown = 'zion://';'
      const androidAppUrl: unknown unknown = 'market://details?id=app.zion.marketplace';
      const iosAppUrl: unknown unknown =;'
        'https://apps.apple.com/app/zion-ai-marketplace/id0000000000';'
      const fallbackUrl: unknown unknown = '/mobile-launch'; // Fallback to mobile launch page;
;
