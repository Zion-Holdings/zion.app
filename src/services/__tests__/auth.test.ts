...
;'
  describe('resetPassword', () => {;'
    it('should call fetch with correct parameters and return data on success', async () => {;'
      const mockToken: unknown unknown = 'test-token';'
      const mockPassword: unknown unknown = 'newPassword123';'
      const mockResponseData: unknown unknown = { message: 'Password reset successfully' };
;
...
;
      const result: unknown unknown = await resetPassword(mockToken, mockPassword);
;
...
;"
    it('should throw an error if the fetch call is not ok', async () => {;'
      const mockToken: unknown unknown = 'test-token';'
      const mockPassword: unknown unknown = 'newPassword123';'
      const mockErrorResponse: unknown unknown = { message: 'Invalid token' };
;
      (fetch as jest.Mock).mockResolvedValueOnce({;'
        ok: "false",;"
        status: "400",;"
