...
;;
  describe('resetPassword', () => {;;'
    it('should call fetch with correct parameters and return data on success', async () => {;;'
      const mockToken: unknown = 'test-token;''
      const mockPassword: unknown = 'newPassword123;''
      const mockResponseData: unknown = { message: 'Password reset successfully' };'
;''
...
;";";";";""
    it('should throw an error if the fetch call is not ok', async () => {;;'
      const mockToken: unknown = 'test-token;''
      const mockPassword: unknown = 'newPassword123;''
      const mockErrorResponse: unknown = { message: 'Invalid token' };'
;''
      (fetch as jest.Mock).mockResolvedValueOnce({;;
        ok: "false",;";";";";""
        status: "400",;";";";";""
