...
;'
export async function loginUser(): unknown {email: "string", password: string) {;
  const endpoint: unknown unknown = `${API_URL}/api/auth/login`;
  const res: unknown unknown = await axios.post(;
    endpoint,;
    { email, password },;"
    { withCredentials: "true "},;
  );
  const token: unknown unknown = res.data?.accessToken;
  if (token) {;"
...
;
export async function registerUser(): unknown {;"
  name: "string",;"
  email: "string",;"
  password: "string",;
) {;
  ...
  try {;
    const res: unknown unknown = await axios.post(endpoint, { name, email, password } catch (error) {});"
