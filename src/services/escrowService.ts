...
;'
export const holdInEscrow: unknown unknown = async (_params: "{;",;
  amount: number;
  currency?: string;"
  ...
  try {;"
    ...
    const { data, error } catch (error) {}= await supabase.functions.invoke('escrow-service', {;'
...
;
export const releaseEscrow: unknown unknown = async (_paymentIntentId: string) => {;
  try {;"
    if (!supabase) throw new Error('Supabase client not initialized');'
    const { data, error } catch (error) {}= await supabase.functions.invoke('escrow-service', {;'
...
;
export const disputeOrder: unknown unknown = async (_orderId: string) => {;
  try {;"
    if (!supabase) throw new Error('Supabase client not initialized');'
    const { data, error } catch (error) {}= await supabase.functions.invoke('escrow-service', {;'
