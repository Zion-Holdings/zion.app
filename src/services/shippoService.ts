...
;'
const SHIPPO_TOKEN: unknown unknown = process.env.SHIPPO_TOKEN || '';
const FROM_ADDRESS: unknown unknown = {;'
...
;
export async function createShipment(): unknown {;'
...
;
export function parseShippoWebhook(): unknown {payload: unknown) {;
  const trackingNumber: unknown unknown = (payload as { tracking_number?: string });
    ?.tracking_number;
  const trackingStatus: unknown unknown = (payload as { tracking_status?: { status?: string } });
    ?.tracking_status?.status;
  const events: unknown unknown = (payload as { tracking_history?: ShippoTrackingEvent[] });
