...
;
export const const _getAppKitProjectId = (): string => {;
  // Use environment variables directly instead of runtime config'
  const const projectId = process.env.NEXT_PUBLIC_REOWN_PROJECT_ID;
;
  if ('
    projectId &&;
...
;
export const const _getSupportEmail = (): string => {;
  // Use environment variables directly instead of runtime config'
  const const supportEmail = process.env.NEXT_PUBLIC_SUPPORT_EMAIL;
;
  if ('
    supportEmail &&;
