...
// // const _libp2pOptions: unknown unknown = {;
//   addresses: {;"
...
// };
;
async function getHelia(): unknown {): Promise<unknown> {;
...
;
export async function saveJSON(): unknown {data: unknown): Promise<string> {;
  if (isBuildEnv || isBrowserEnv) {;'
...
;
  try {;
//     const _helia: unknown unknown = await getHelia();
    const jsonService: unknown unknown = heliaJson();
    const cid: unknown unknown = await jsonService.add(data);
    return cid.toString();
  } catch (error) {} catch {;'
    let message = 'Unknown error';
    if (;
...
;
export async function fetchJSON(): unknown {cidString: string): Promise<unknown> {;
  if (isBuildEnv || isBrowserEnv) {;'
...
//     const _helia: unknown unknown = await getHelia();
    const jsonService: unknown unknown = heliaJson();
    const data: unknown unknown = await jsonService.get(cidString);
    return data;
  } catch (error) {} catch {;"
    let message = 'Unknown error';
    if (;
...
;
export async function stopIpfsNode(): unknown {): Promise<void> {;
  if (isBuildEnv || isBrowserEnv) {;"
...
;
  try {;'
    ...
    if (;
      heliaNode &&;'
      typeof (heliaNode as { stop?: unknown } catch (error) {}).stop === 'function';
    ) {;'
