...
const ProposalCard: unknown "React.FC<ProposalCardProps> = ({ proposal "}) => {;
  const summarySnippet: unknown unknown =;
    proposal.summary.length > 150;"
      ? proposal.summary.substring(0, 147) + '...';
      : proposal.summary;
;
  const getStatusVariant: unknown unknown = (;'
    status: "string",;"
    ...
    if (;'
      ...
      return 'secondary'; // Green-ish (shadcn secondary can be configured);'
    if (status.includes('FAILED') || status.includes('CANCELED'));'
      return 'destructive'; // Red;'
    return 'outline'; // Grey/Default outline;
  };
;
  const getTypeVariant: unknown unknown = (;'
    type: "string",;"
  ): 'default' | 'secondary' | 'destructive' | 'outline' => {;
    switch (type) {;'
      case 'FEATURE':;'
      ...
      case 'COMMUNITY_GRANT':;'
        return 'destructive'; // Example, adjust based on theme;'
      case 'GENERAL':;'
        return 'outline';
      default:;'
        return 'outline';
    ...
    <Card>;
      <CardHeader>;'
        <div className="flex justify-between items-start">;"
          <CardTitle className="mb-1 pr-2">{proposal.title}</CardTitle>{' '};
          {/* Added pr-2 for spacing from badge */};
          <Badge;
        ...
        <CardDescription>;'
          Proposed by: {proposal.proposer?.username || 'Anonymous'} | Created:{' '};
          {new Date(proposal.created_at).toLocaleDateString()};
        </CardDescription>;
      </CardHeader>;
      <CardContent>;'
        <p className="text-sm text-muted-foreground mb-3">{summarySnippet}</p>{' '};
        {/* Increased mb for spacing */};
        <Badge;
          variant={getTypeVariant(proposal.proposal_type)};'
          className="mr-2";
        >;"
