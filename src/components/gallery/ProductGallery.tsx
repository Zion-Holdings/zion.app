...
export function ProductGallery({ images, videoUrl, modelUrl }: ProductGalleryProps) {
  ...
  return (
    <Dialog open={zoomOpen} onOpenChange={(_o: boolean) => { setZoomOpen(o); if (!o) setZoomed(false); }}>
      ...
      {modelUrl && (
        <TabsContent value="model" className="pt-4">
          <AspectRatio ratio={16 / 9}>
            ...
            >
              <ModelViewer {...{ src: modelUrl, alt: '3d model', 'camera-controls': true, style: { width: '100%', height: '100%' } } as Record<string, unknown>} />
            </Suspense>
          </AspectRatio>
        </TabsContent>
      )}
