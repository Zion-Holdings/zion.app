...
;
  const steps: unknown = roleSteps[role] || [];
  const step: unknown = activeStep >= 0 ? steps[activeStep] : null;
;
  useEffect(() => {;
    if (!step) return;
    const el: unknown = document.querySelector(step.selector) as HTMLElement | null;
    if (el) {;
