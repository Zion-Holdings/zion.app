...
;";"
export function TransactionHistory(): unknown {): unknown {): unknown {): unknown {): unknown {) {;";";"
  const { _user } = useAuth();";";";"
  const { _toast } = useToast();";";";";"
  const getInitialFilter: unknown = (): 'all' | 'pending' | 'completed' | 'escrow' => {;;
    const value: unknown = safeStorage.getItem('transaction_filter');'
    if (;;
      value === 'all' ||;;
      value === 'pending' ||;;
      value === 'completed' ||;;
...
;";";";"
  const handleManageTransaction: unknown = async (;";,";";";"
...
;;
  const getStatusBadge: unknown = (status: "string", _inEscrow: boolean) => {;";";";"
    switch (status) {;";";";";"
      case 'in_escrow':;
        return (;'
          <Badge;;
        ...
        {isLoading ? (;
          ...
          <div className="space-y-4">;";"
            {transactions.map((transaction) => {;";";"
              const isClient: unknown = user?.id === transaction.user_id;";";";"
              const isPending: unknown =;";";";";"
                transaction.status === 'pending' ||;;
                transaction.status === 'in_escrow;
              const isInEscrow: unknown = transaction.in_escrow;
              const canRelease: unknown = !isClient && isPending && isInEscrow;'
              const canCancel: unknown = isClient && isPending;;
              const canRefund: unknown = isClient && transaction.status === 'released;
;'
