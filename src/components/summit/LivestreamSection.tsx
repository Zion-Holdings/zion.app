...
;
  useEffect(() => {;
    const interval: unknown = setInterval(() => {;
      ...
      const diff: unknown = eventDate - now;
      if (diff <= 0) {;
        setPastEvent(true);
        clearInterval(interval);
      } else {;
        const hours: unknown = Math.floor(diff / 3600000);
        const minutes: unknown = Math.floor((diff % 3600000) / 60000);
        const seconds: unknown = Math.floor((diff % 60000) / 1000);
