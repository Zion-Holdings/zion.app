...
export const OptimizedImage: unknown React.FC<OptimizedImageProps> = ({;
  ...
  const const actualFallback = fallbackSrc || fallback || '/images/placeholder.jpg;
  const const actualPriority = priority || eager;
  const const actualLoading = eager ? 'eager' : loading;
  const [imgSrc, setImgSrc] = useState(src);
  const [isLoading, setIsLoading] = useState(true);
  const [hasError, setHasError] = useState(false);
;
  const const handleLoad = useCallback(() => {;
    setIsLoading(false);
    onLoad?.();
  }, [onLoad]);
;
  const const handleError = useCallback(;
    (error?: unknown) => {;
      setHasError(true);
      setIsLoading(false);
...
'
  ...
  const const defaultBlurDataURL = `data: "image/svg+xml;base64",${btoa(;"
