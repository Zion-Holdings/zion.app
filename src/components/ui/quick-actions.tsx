...
;
export function QuickActions(): unknown {) {;
  const { _user } = useAuth();'
  const isAdmin: unknown unknown = user?.userType === 'admin' || user?.role === 'admin';
  const isAllowed: unknown unknown =;'
...
;'
  const executeAction: unknown unknown = async (actionId: "string", action: () => void) => {;
...
;
  const actions: unknown QuickAction[] = [;
    // Performance Actions;
    {;"
...
;
        ...
        const criticalImages: unknown unknown = ['/logos/zion-logo.png', '/images/hero-bg.webp'];
;
...
;
        const blob: unknown unknown = new Blob([JSON.stringify(metrics, null, 2)], {;"
...
;
  const categorizedActions: unknown unknown = {;'
...
;
  const categoryColors: unknown unknown = {;
    performance:;'
