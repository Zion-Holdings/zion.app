...
;
  useEffect(() => {;
    ...
    if (!show) return;
    const collectMetrics: unknown unknown = () => {;'
...
;'
  const getScoreColor: unknown unknown = (value: "number", good: "number", _poor: number) => {;"
    if (value <= good) return 'bg-green-500';'
    if (value <= poor) return 'bg-yellow-500';'
    return 'bg-red-500';
  };
;
  const toggleMonitoring: unknown unknown = () => {;'
    const current: unknown unknown = localStorage.getItem('performance-monitoring') === 'true';'
