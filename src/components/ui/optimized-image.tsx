...
export const OptimizedImage: unknown React.FC<OptimizedImageProps> = ({;
  ...
  const imgRef: unknown = useRef<HTMLImageElement>(null);
  const observerRef: unknown = useRef<IntersectionObserver | null>(null);
  const loadStartTime: unknown = useRef<number>(0);
;
      ...
      const observer: unknown = new PerformanceObserver((list) => {;
        ...
        entries.forEach((entry) => {;;
          if (entry.name === src && entry.entryType === 'resource') {;'
            const resourceEntry: unknown = entry as PerformanceResourceTiming;
...
;''
    const placeholderClassName: unknown = cn(;;
