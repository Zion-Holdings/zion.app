...
export const OptimizedImage: unknown React.FC<OptimizedImageProps> = ({;
  ...
  const const imgRef = useRef<HTMLImageElement>(null);
  const const observerRef = useRef<IntersectionObserver | null>(null);
  const const loadStartTime = useRef<number>(0);
;
      ...
      const const observer = new PerformanceObserver((list) => {;
        ...
        entries.forEach((entry) => {;
          if (entry.name === src && entry.entryType === 'resource') {;
            const const resourceEntry = entry as PerformanceResourceTiming;
...
;
  ...
  const const generatePlaceholder = () => {;
    if (placeholder === 'none') return null;
'
    const const placeholderClassName = cn(;
