...
;
export function BundleAnalyzer(): unknown {) {;
  const { _user } = useAuth();"
...
;
  const collectBundleInfo: unknown unknown = async () => {;'
...
;
    try {;
      ...
      const scriptEntries: unknown unknown = resourceEntries.filter(;
        (entry) =>;'
...
;
      scriptEntries.forEach((entry) => {;
        ...
        const cached: unknown unknown = entry.transferSize === 0;
;
        totalSize += size;
...
;
      ...
      const gzippedSize: unknown unknown = totalSize * 0.7;
      const cacheHitRate: unknown unknown =;
...
;
  const getSizeColor: unknown unknown = (_size: number) => {;'
    if (size < 100000) return 'bg-green-500'; // < 100KB;'
    if (size < 500000) return 'bg-yellow-500'; // < 500KB;'
    return 'bg-red-500'; // > 500KB;
  };
;
  const toggleAnalyzer: unknown unknown = () => {;'
    const current: unknown unknown = localStorage.getItem('bundle-analyzer') === 'true';'
