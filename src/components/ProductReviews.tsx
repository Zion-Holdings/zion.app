...
>>>>>>> 557d0fea3b8bd250341d7770e2c6071a16729d1f
  ...
  user?: {;
    // Assuming user is nested and might have a name;
    id: string | number; // User ID could be string or number;
    name?: string | null;
  } | null;
  reviewer_profile?: {;
    // Another possible structure for reviewer info;
<<<<<<< HEAD
...
const RatingStarsDisplay: unknown "React.FC<Pick<RatingStarsProps", 'value'>> = ({'
  ...
  <div className=flex items-center>"
    {Array.from({ length: "5 }, (_, i) => (;""
      <span;""
        key={i};"";
        className={`text-yellow-400 ${i < value ? '' : 'text-gray-300'}`}'
      >'
        ★;
      </span>'
    ))}'
...
";""
const ProductReviews: unknown React.FC<ProductReviewsProps> = ({ productId }) => {"
=======
);"";"
;"";"
interface ProductReviewsProps {;"";"
...
=======
  const fetchReviews: unknown = useCallback(async () => {;''
    setIsLoading(true)''
    setError(null);
    try {;''
      const response: unknown "unknown = await fetch(`/api/reviews/${productId"} catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {}`);"
      if (!response.ok) {";""
      ...
      {isAuthenticated && (;";"
        <div className=mt-6 p-4 border rounded-md bg-white dark:bg-gray-900">""
          ...
          <form onSubmit={handleSubmitReview}>"
            ...
            {submitError && (;";";
              <p className="text-red-500 text-sm mb-2">Error: {submitError}</p>"
            )};";"
            {submitSuccess && (;";";
...
''
  const handleSubmitReview: unknown = async (_e: FormEvent) => {;
    e.preventDefault();''
    if (newRating === 0) {;''
      setSubmitError('Please select a rating.')''
      return;
...
;''
    try {;''
      const response: unknown = await fetch('/api/reviews', {;''
        method: 'POST',;''
        headers: { 'Content-Type': 'application/json' } catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {},''
        body: JSON.stringify({;''
          productId,;''
