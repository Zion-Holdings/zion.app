...
export default function ChatAssistant({
  ...
  const {
    ...
    onConnect: () => {
      console.log('ChatAssistant: Connected to WebSocket');
    ...
    onDisconnect: (reason) => {
      console.log('ChatAssistant: Disconnected from WebSocket:', reason);
    ...
    onReconnectFailed: () => {
      console.error('ChatAssistant: Reconnection failed');
  ...
  useEffect(() => {
    ...
    const handleReconnectionSuccess = (event: any) => {
      console.log('✅ Chat assistant reconnection successful');
    };

    const handleReconnectionFailure = (event: any) => {
      console.error('❌ Chat assistant reconnection failed:', event.error);
    };

    const handleHealthCheck = (event: any) => {
      if (event.health && !event.health.isHealthy) {
        console.warn('⚠️ Chat assistant connection health degraded');
      }
    };
  ...
  // Auto-join room when assistant opens
  useEffect(() => {
  ...
  // Leave room when assistant closes
  useEffect(() => {
  ...
  // Mark messages as read when assistant is open
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    // If AI is enabled, simulate AI response
    if (aiEnabled) {
      simulateAIProcessing(message);
    }
  };
  ...
  const handleReconnect = () => {
    console.log('🔄 Manual reconnection triggered for chat assistant');
    reconnect();
  };
  ...
  return (
    <div className={`fixed bottom-4 left-4 z-50 ${className}`}>
      ...
      <button
        ...
        className="bg-purple-500 hover:bg-purple-600 text-white rounded-full p-3 shadow-lg transition-colors duration-200"
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="absolute bottom-16 left-0 w-80 bg-white rounded-lg shadow-xl border border-gray-200">
          {/* Header */}
          ...
          >
            {!connectionState.isConnected ? (
              ...
              <div className="text-center text-gray-500 py-8">
                <div className="text-sm">Joining AI assistant room...</div>
              </div>
