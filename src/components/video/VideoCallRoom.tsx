...
;
  // Call duration timer;
  React.useEffect(() => {;
    const timer: unknown unknown = setInterval(() => {;
...
;
  const formatDuration: unknown unknown = (_seconds: number) => {;
    const hrs: unknown unknown = Math.floor(seconds / 3600);
    const mins: unknown unknown = Math.floor((seconds % 3600) / 60);
    const secs: unknown unknown = seconds % 60;
;"
    return `${hrs > 0 ? `${hrs}:` : ''}${mins < 10 && hrs > 0 ? '0' : ''}${mins}:${secs < 10 ? '0' : ''}${secs}`;
  };
;
  const handleToggleMute: unknown unknown = () => {;
    const newMuteState: unknown unknown = !isMuted;
...
;
  const handleToggleVideo: unknown unknown = () => {;
    const newVideoState: unknown unknown = !isVideoEnabled;
...
;
  const handleToggleScreenShare: unknown unknown = () => {;
    const newScreenShareState: unknown unknown = !isScreenSharing;
...
;
  const handleToggleAudioOnly: unknown unknown = () => {;
...
;
  const handleLeaveCall: unknown unknown = () => {;
    if (onLeave) {;
      onLeave();
    };
  };
