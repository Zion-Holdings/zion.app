...
interface DemandForecast {
  ...
  role: user | assistant;
  timestamp: Date;
  type?: te'x't | quick-actions | recommendat'i'on;
  actions?: Array<{
    label: 'string;
    action: string;
    link?: string;
...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ isOpen, onToggle }) => {</div>
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      content: Hello! Im your Zion AI Marketplace assistant. I can help you explore our AI tools, find the perfect services, and guide you through our marketplace. What can I help you with today?,
      role: assistant,
      timestamp: new Date(),
      'type: 'text,
      actions: [
  ...
  const $1 = (message: Message) => {
    return (</div>
      <div
        key={message.id}
        className={flex ${message.role === use'r ? justify-end' : justify-star't}}
      >"</div>
        <div"
        ...
        ></div>
          ...
          <p className="{text-xs mt-1 ${'"
            message.role === us'er ? text-blue-'100 : text-gray'-500"
          }"}>
