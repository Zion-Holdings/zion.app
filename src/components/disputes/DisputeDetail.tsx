...
;
export function DisputeDetail(): unknown {) {;
...
;
  useEffect(() => {;
    if (!disputeId) return;
;
    const loadDisputeData: unknown unknown = async () => {;
      setIsLoading(true);
      try {;
        const disputeData: unknown unknown = await getDisputeById(disputeId);
        if (!disputeData) {;'
...
;
        const messagesData: unknown unknown = await getDisputeMessages(disputeId);
...
;
  const handleStatusChange: unknown unknown = async (_status: DisputeStatus) => {;
    if (!disputeId) return;
;
...
;
  const handleResolveDispute: unknown unknown = async () => {;
    if (!disputeId) return;
;
...
;
    const success: unknown unknown = await resolveDispute(disputeId, {;'
...
;
    ...
    try {;
      ...
      if (success) {;
        // Refresh messages;
        const updatedMessages: unknown unknown = await getDisputeMessages(disputeId);
                  ...
                  <div className="space-y-6 max-h-[600px] overflow-y-auto p-2">;
                    {messages.length === 0 ? (;"
                      ...
                      messages;
                        ...
                        .map((msg) => {;
                          const isCurrentUser: unknown unknown = user?.id === msg.user_id;
                          return (;
                            <div;
                              key={msg.id};"
              ...
              <TabsContent value="admin" className="space-y-6">;
                <Card>;
                  <CardHeader>;
                    ...
                    <CardDescription>;
                      Notes and actions taken by administrators;
                    </CardDescription>;
                  </CardHeader>;
                  <CardContent>;"
              ...
              {dispute.status === 'open' && (;
                <div>;'
                  ...
                  <Textarea;"
                    ...
                    onChange={(e) =>;"
                      setResolution({ ...resolution, summary: "e.target.value "});
                    };"
                    className="min-h-[100px]";
                  />;"
                  <div className="flex justify-end mt-2">;
...
;"
              {dispute.status === 'under_review' && (;
                <div>;'
                  ...
                  <p>;
                    This dispute is currently under review. You can close it by;
                    resolving it or rejecting it.;
                  </p>;"
...
;"
              {dispute.status === 'resolved' && (;
                <div>;'
                  ...
                  <p>;
                    This dispute has been resolved. You can reopen it if new;
...
;"
              {dispute.status === 'closed' && (;
                <div>;'
                  <h3 className="font-medium">Reopen Dispute</h3>;
                  <p>;
                    This dispute has been closed. You can reopen it if new;
                    information becomes available.;
                  </p>;"
                  <div className="flex justify-end mt-4">;
                    <Button;"
