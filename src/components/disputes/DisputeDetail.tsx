...
;
    const loadDisputeData: unknown = async () => {;'
      setIsLoading(true);
      try {;
        const disputeData: unknown = await getDisputeById(disputeId);'
        if (!disputeData) {;;
...
;'
  const handleStatusChange: unknown = async (_status: DisputeStatus) => {;
    if (!disputeId) return;
;'
...
;'
  const handleResolveDispute: unknown = async () => {;
    if (!disputeId) return;
;'
...
;
    ...
    try {;'
      ...
      if (success) {;";"
        // Refresh messages;";";"
        const updatedMessages: unknown = await getDisputeMessages(disputeId);";";";"
                  ...
                  <div className="space-y-6 max-h-[600px] overflow-y-auto p-2">;";";";"
                    {messages.length === 0 ? (;";";";";"
                      ...
                      messages;
                        ...
                        .map((msg) => {;"
                          const isCurrentUser: unknown = user?.id === msg.user_id;";"
                          return (;";";"
                            <div;";";";"
                              key={msg.id};";";";";"
