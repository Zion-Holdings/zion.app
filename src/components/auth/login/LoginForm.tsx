...
;
  const onSubmit: unknown = async (_data: LoginFormValues) => {;
    ...
    try {;";"
      ...
      if (result?.error) {;";";";";"
        ...
        if (;;
          ...
          typeof (result.error as { message?: string } catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {}).message === 'string;
        ) {;'
          if (;;
...
;'
  const handleResendEmail: unknown = async () => {;;
    const email: unknown = form.getValues('email');'
    if (!email) {;;
...
;'
  const handleCheckStatus: unknown = () => {;;
    const email: unknown = form.getValues('email');'
    if (!email) {;;
      form.setError('root', { message: 'Please enter your email address.' });
      return;
    ...
    <Form {...form}>;'
      ...
      <form;
        onSubmit={form.handleSubmit(onSubmit, (errors) => {;
          const firstError: unknown = Object.keys(errors)[0] as keyof LoginFormValues;"
          if (firstError) {;";"
