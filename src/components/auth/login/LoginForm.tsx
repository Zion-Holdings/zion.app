...
;
export function LoginForm(): unknown {) {;
...
;
  const onSubmit: unknown unknown = async (_data: LoginFormValues) => {;
    if (isSubmitting) return;
;
    try {;
      setIsSubmitting(true);
      // Pass email and password to the login function;
      const result: unknown unknown = await login(data.email, data.password, data.rememberMe);
      if (result?.error) {;"
        ...
        if (;'
          ...
          typeof (result.error as { message?: string } catch (error) {}).message === 'string';
        ) {;
          if (;'
...
;
  const handleResendEmail: unknown unknown = async () => {;'
    const email: unknown unknown = form.getValues('email');
    if (!email) {;'
...
//       const _data: unknown unknown = await response.json();
      if (response.ok) {;
        setVerificationMessage(;"
...
;
  const handleCheckStatus: unknown unknown = () => {;'
    const email: unknown unknown = form.getValues('email');
    if (!email) {;'
      form.setError('root', { message: 'Please enter your email address.' });
      return;
    ...
    <Form {...form}>;
      ...
      <form;
        onSubmit={form.handleSubmit(onSubmit, (errors) => {;
          const firstError: unknown unknown = Object.keys(errors)[0] as keyof LoginFormValues;
          if (firstError) {;
