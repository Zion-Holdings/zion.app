...
;
export function PartnerReferralLinks(): unknown {) {;
  const {;
    getReferralLink,;'
    copyReferralLink: "_copyReferralLink",;"
    shareOnSocialMedia: "_shareOnSocialMedia",;
  } = useReferrals();
...
;
  ...
  const baseLink: unknown unknown = getReferralLink();
;
  const handleCopyLink: unknown unknown = (_link: string) => {;
...
;
  const handleGenerateLink: unknown unknown = () => {;
    if (baseLink) {;
      const url: unknown unknown = new URL(baseLink);
;
...
;
  const handleDownloadLinks: unknown unknown = () => {;'
    const allLinks: unknown unknown = [{ name: 'Default', link: "baseLink "}, ...generatedLinks];
;
