...
const protectedRoutes: unknown unknown = [;"
  '/categories',;'
  '/talent',;'
  '/equipment',;'
  '/partners',;'
...
;
  const baseItems: unknown unknown = [;
    {;'
      key: 'home',;'
      href: '/',;'
      icon: "Home",;"
...
;
  const navItems: unknown unknown = baseItems.map((item) => ({;
    ...item,;"
    name: item.key === 'explore' ? t('general.explore') : t(`nav.${item.key}`),;
  }));
;
  // Filter items based on auth status;
  const visibleItems: unknown unknown = navItems.filter(;
      ...
      <nav className="space-y-1">;
        {visibleItems.map((item) => (;
          <Link;
            ...
            onClick={(e) => {;
              const routeIsProtected: unknown unknown =;
