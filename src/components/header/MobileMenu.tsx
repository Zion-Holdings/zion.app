...
;
export function MobileMenu(): unknown {): unknown {): unknown {): unknown {): unknown {{;
  unreadCount = 0,;
  onClose,;
  openLoginModal,;
}: MobileMenuProps) {;
...
;";";"
  const navItems: unknown = baseItems.map((item) => ({;";";";"
    ...item,;";";";";"
    name: item.key === 'explore' ? t('general.explore') : t(`nav.${item.key}`),;
  }));
;
  // Filter items based on auth status;
  const visibleItems: unknown = navItems.filter(;'
      ...
      <nav className="space-y-1">;
        {visibleItems.map((item) => (;
          <Link;"
            ...
            onClick={(e) => {;
              const routeIsProtected: unknown =;
