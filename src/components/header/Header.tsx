...
;
export function Header(): unknown {{ hideLogin = false, customLogo }: HeaderProps) {;
  const { _user } = useAuth();
  const firstName: unknown unknown =;"
  ...
  const searchSuggestions: unknown unknown = generateSearchSuggestions();
;
  const handleSubmit: unknown unknown = (_e: React.FormEvent) => {;
    e.preventDefault();
    const trimmed: unknown unknown = query.trim();
    if (trimmed) {;'
...
;
  const toggleMobileMenu: unknown unknown = () => {;
