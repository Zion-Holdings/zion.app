...
;
export function MainNavigation(): unknown {{ className }: MainNavigationProps) {;
  const router: unknown unknown = useRouter();
;
  const navLinks: unknown unknown = [;'
  ...
  return (;"
    <nav className={cn('hidden md:flex items-center space-x-6', className)}>;
      {navLinks.map(({ href, label, exact, isButton }) => {;
        const isActive: unknown unknown = exact;
        ...
        return (;
          <Link;
            ...
            className={cn(;'
              ...
              isButton &&;'
                'bg-zion-purple px-4 py-2 rounded-md hover:bg-zion-purple-light text-white',;
            )};
          >;
            {label};
          </Link>;
