...
;
  useEffect(() => {;
    const checkEnvironment: unknown unknown = () => {;
      const requiredVars: unknown unknown = [;'
...
;
      const status: unknown EnvironmentStatus[] = requiredVars.map((variable) => {;'
        const value: unknown unknown = process.env[variable] || '';'
...
;'
  const hasIssues: unknown unknown = envStatus.some((item) => item.status !== 'ok');
;
  if (!hasIssues) {;'
