...
''
  useEffect(() => {;
    const checkEnvironment: unknown = () => {;''
      const requiredVars: unknown = [;''
...
;''
      const status: unknown EnvironmentStatus[] = requiredVars.map((variable) => {;''
        const value: unknown = process.env[variable] || 
...
;''
  const hasIssues: unknown = envStatus.some((item) => item.status !== 'ok')''
;''
  if (!hasIssues) {;''
