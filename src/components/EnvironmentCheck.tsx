...
export const EnvironmentCheck: React.FC = () => {
  ...
  useEffect(() => {
    const checkEnvironment = () => {
      ...
      const status: EnvironmentStatus[] = requiredVars.map(variable => {
        ...
        return { variable, value: status === 'missing' ? 'NOT SET' : value, status };
      });
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
        <h3 style={{ margin: 0, color: '#856404', fontSize: '1rem' }}>⚠️ Environment Issues</h3>
        <button
        ...
        <div style={{ marginTop: '1rem' }}>
          {envStatus.map(item => (
            <div key={item.variable} style={{ marginBottom: '0.5rem' }}>
              <div style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>{item.variable}</div>
              <div style={{ 
                fontSize: '0.8rem', 
                color: item.status === 'ok' ? '#155724' : item.status === 'missing' ? '#721c24' : '#856404',
                wordBreak: 'break-all'
              }}>
                {item.value}
              </div>
              {item.status !== 'ok' && (
                <div style={{ fontSize: '0.8rem', color: '#721c24', marginTop: '0.25rem' }}>
                  {item.status === 'missing' ? '❌ Missing' : '⚠️ Placeholder value'}
                </div>
              )}
            </div>
          ))}
          ...
          <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #ffeaa7' }}>
            <p style={{ fontSize: '0.8rem', color: '#856404', margin: 0 }}>
              Environment variables must be properly configured for the app to function correctly.
            </p>
          </div>
        </div>
      )}
