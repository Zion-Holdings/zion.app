...
;
const mockedUseRouter: unknown unknown = useRouter as unknown as vi.Mock;
const mockedFindRoute: unknown unknown = findRouteByPath as unknown as vi.Mock;
;'
...
;
  ...
  const title: unknown unknown = container.querySelector('title');
  expect(title?.textContent).toBe(;'
    'About | Zion - The Future of Tech & AI Marketplace',;
  );'
  const metaDesc: unknown unknown = container.querySelector('meta[name="description"]');'
  expect(metaDesc?.getAttribute('content')).toBe('About us page');'
  const canonical: unknown unknown = container.querySelector('link[rel="canonical"]');'
