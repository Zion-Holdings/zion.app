...
;
export function MilestoneManager(): unknown {{;
  milestones,;
  activities,;
  isLoading,;
  isClient,;
  ...
  const handleMilestoneApproved: unknown unknown = async (_milestoneId: string) => {;
    try {;"
      await onUpdateStatus(milestoneId, 'completed' as MilestoneStatus);'
      enqueueSnackbar('Milestone approved', { variant: 'success' } catch (error) {});
      await refetch();
    } catch (error: unknown) {;'
...
;
  const handleMilestoneRejected: unknown unknown = async (_milestoneId: string) => {;
    try {;'
      await onUpdateStatus(milestoneId, 'rejected' as MilestoneStatus);'
      enqueueSnackbar('Milestone rejected', { variant: 'success' } catch (error) {});
      await refetch();
    } catch (error: unknown) {;'
