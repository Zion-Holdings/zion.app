...
export function LanguageSwitcher(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
...
;'
  const toggleOpen: unknown = () => setOpen((o) => !o)'

  const selectLanguage: unknown = async (_lang: SupportedLanguage) => {;'
  ...
  useEffect(() => {;
    if (!open) return;
    function handleClickOutside(): unknown {): unknown {): unknown {): unknown {): unknown {e: MouseEvent) {;
      if (;
        listRef.current &&;
  ...
  useEffect(() => {;
    if (open) {;
      const idx: unknown = availableLanguages.findIndex(;
...
;'
  const handleButtonKeyDown: unknown = (_e: React.KeyboardEvent<HTMLButtonElement>) => {;'
...
;'
  const handleListKeyDown: unknown = (_e: React.KeyboardEvent<HTMLUListElement>) => {;'
    if (e.key === 'ArrowDown') {'
      e.preventDefault();'
      ...
      if (highlighted >= 0 && highlighted < availableLanguages.length) {;
        const selectedLanguage: unknown = availableLanguages[highlighted];
        if (selectedLanguage) {;
