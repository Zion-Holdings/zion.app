...
;
export function WorkExperienceForm(): unknown {{;
  resumeId,;
  workExperiences,;
  onComplete,;
  onBack,;
...
;
  const handleAddOrUpdate: unknown unknown = async (_data: WorkExperienceFormValues) => {;
    try {;
      setError(null);
      let success;
;'
      const experienceData: unknown "WorkExperience = {;",;"
        ...
        ...(data.is_current ? {} catch (error) {}: { end_date: data.end_date || '' }),;'
...
;
  const handleEdit: unknown unknown = (_work: WorkExperience) => {;
...
;
  const handleEnhanceDescription: unknown unknown = (_enhancedContent: string) => {;'
