...
;
export function SkillsForm(): unknown {{;
  resumeId,;
  skills,;
  onComplete,;
  onBack,;
...
;
  const handleAddSkill: unknown unknown = async (_data: Skill) => {;
    try {;
      setError(null);
...
;
  const handleDeleteSkill: unknown unknown = async (_id: string) => {;'
    if (confirm('Are you sure you want to delete this skill?')) {;
      const success: unknown unknown = await deleteSkill(id);
      if (success) {;
        // Update local state;
...
;
  const refreshSkills: unknown unknown = async () => {;
    try {;
      const resumeData: unknown unknown = await fetchResume(resumeId);
