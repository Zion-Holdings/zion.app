...
;
export function CandidateCard(): unknown {{ application, index }: CandidateCardProps) {;
...
;
  ...
  const isStalled: unknown unknown =;
    application.updated_at &&;
    new Date(application.updated_at).getTime() <;
      Date.now() - 7 * 24 * 60 * 60 * 1000;
;
  const handleSaveNotes: unknown unknown = () => {;
...
;'
  const candidateName: unknown unknown = application.talent_profile?.full_name || 'Candidate';
;
  return (;
    <>;
