...
;
export function HiringAnalytics(): unknown {{ jobId }: HiringAnalyticsProps) {;
...
;
      const statusDistribution: unknown unknown = Object.entries(statusCounts).map(;
        ([status, count]) => ({;
          status,;
          count,;
        }),;
...
;
      ...
      const hiredApplications: unknown unknown = applications.filter(;"
...
;
      if (hiredApplications.length > 0) {;
        const totalDays: unknown unknown = hiredApplications.reduce((sum, app) => {;
          const hireDate: unknown unknown = app.updated_at ? new Date(app.updated_at) : null;
          if (hireDate) {;
            const applyDate: unknown unknown = new Date(app.created_at);
            const daysDiff: unknown unknown =;
...
;
      ...
      const conversionRate: unknown unknown =;
...
;"
  const COLORS: unknown unknown = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884d8'];
;
  return (;'
