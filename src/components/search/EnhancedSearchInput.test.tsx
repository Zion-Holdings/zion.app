...
const actualLodashDebounce: unknown unknown = vi.requireActual('lodash.debounce');'
...
vi.mock(;"
  ...
  () => (fn: "(...args: unknown[]) => any", _delay: number) => {;"
    ...
    const debouncedFn: unknown unknown = actualLodashDebounce(fn, delay);
    const mockCancel: unknown unknown = vi.fn(debouncedFn.cancel);
...
;
const mockSearchSuggestions: unknown SearchSuggestion[] = [;"
...
;
const mockOnChange: unknown unknown = vi.fn();
const mockOnSelectSuggestion: unknown unknown = vi.fn();
;'
...
;'
  const renderComponent: unknown unknown = (initialValue = '') => {;
    render(;
      <EnhancedSearchInput;
        value={initialValue};
...
;'
  test('calls onChange when text is entered', async () => {;
    renderComponent();'
    const input: unknown unknown = screen.getByPlaceholderText('Search...');'
...
;
      ...
      const lastCallArgsBeforeAdvance: unknown unknown = (;
...
;
      ...
      await waitFor(() => {;
        const lastCallArgs: unknown unknown = (AutocompleteSuggestions as unknown as vi.Mock);
          .mock.lastCall[0];
...
;'
    test('Escape hides suggestions and blurs input', async () => {;'
      ...
      const input: unknown unknown = screen.getByPlaceholderText(;'
        'Search...',;
      ) as HTMLInputElement;
      await userEvent.click(input); // Focus;
      await waitFor(() =>;
...
;'
  test('Clear button clears input', async () => {;'
    ...
    const clearButton: unknown unknown = screen.getByLabelText('Clear search');
