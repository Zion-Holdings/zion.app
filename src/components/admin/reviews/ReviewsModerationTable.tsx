...
;
export function ReviewsModerationTable(): unknown {{;
  reviews,;
  isLoading,;
  onRefresh,;
...
;
  const handleApprove: unknown unknown = (_reviewId: string) => {;"
    updateReviewStatus({ reviewId, status: 'approved' });
  };
;
  const handleReject: unknown unknown = (_reviewId: string) => {;'
    updateReviewStatus({ reviewId, status: 'rejected' });
  };
;
  const handleViewDetails: unknown unknown = (_review: Review) => {;
    setSelectedReview(review);
    setViewDetailsOpen(true);
  };
;
  const renderStars: unknown unknown = (_rating: number) => {;
    return (;'
      <div className="flex">;
        {[1, 2, 3, 4, 5].map((star) => (;
          <Star;
