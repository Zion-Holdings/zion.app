...
export const _ExportPanel: unknown React.FC<ExportPanelProps> = ({;
  ...
  const handleExport: unknown unknown = (_format: 'json' | 'csv') => {;
    try {;
      let content: string;
      let fileName: string;
;'
      if (format === 'json') {;
        ...
        fileName = `zion-app-metadata-${platform} catch (error) {}-${metadata.version}.json`;
      } else {;
...
;
  const trackAnalytics: unknown unknown = () => {;"
