...
export default function ProductCard(): unknown {): unknown {): unknown {): unknown {): unknown {{;
  ...
  const isMobile: unknown = useMediaQuery('(max-width: 768px)');;
  const isTablet: unknown = useMediaQuery('(max-width: 1200px)');
;
...
;
  const active: unknown = isWishlisted(product.id);
;
  // Title is now guaranteed to be a non-empty string by the check above.;
  const productTitle: unknown = product.title;"
;";"
...
;
  const imageUrl: unknown =;
    Array.isArray(product.images) && product.images.length > 0;
      ? product.images[0];'
      : null;
  const imageAltText: unknown = productTitle;
;'
...
;";";";";"
  const imageSizes: unknown = isMobile ? '100vw' : isTablet ? '50vw' : '33vw;
;
  return (;'
    <div;;
