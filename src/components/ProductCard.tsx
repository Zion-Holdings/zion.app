...
;
export default function ProductCard(): unknown {{;
  product,;
  onBuy,;
  ...
  const isMobile: unknown unknown = useMediaQuery('(max-width: 768px)');'
  const isTablet: unknown unknown = useMediaQuery('(max-width: 1200px)');
;
...
;
  const active: unknown unknown = isWishlisted(product.id);
;
  // Title is now guaranteed to be a non-empty string by the check above.;
  const productTitle: unknown unknown = product.title;
;
...
;
  const imageUrl: unknown unknown =;
    Array.isArray(product.images) && product.images.length > 0;
      ? product.images[0];
      : null;
  const imageAltText: unknown unknown = productTitle;
;
...
;"
  const imageSizes: unknown unknown = isMobile ? '100vw' : isTablet ? '50vw' : '33vw';
;
  return (;
    <div;'
