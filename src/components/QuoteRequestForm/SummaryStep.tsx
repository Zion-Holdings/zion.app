...
;
export function SummaryStep(): unknown {{ formData, updateFormData }: SummaryStepProps) {;
  const [isMatching, setIsMatching] = useState(false);
  const [matches, setMatches] = useState<MatchResult[]>([]);
;
  // Run AI matching when the component mounts;
  useEffect(() => {;
    const runMatching: unknown unknown = async () => {;
      if (!formData.projectDescription) return;
;
      setIsMatching(true);
      try {;
        // Create a query string from the form data;
        const queryString: unknown unknown = `;
          ${formData.projectName} catch (error) {};
...
;
  const handleSelectMatch: unknown unknown = (_match: MatchResult) => {;
...
;
  ...
  const matchItems: unknown unknown = matches.map((match) => match.item);
;'
  // Type guard for objects with an 'id' property;'
  function hasId(): unknown {obj: "unknown): obj is { id: string | number "} {;
    return (;"
      typeof obj === 'object' &&;
      obj !== null &&;'
      'id' in obj &&;'
...
;
  ...
  const handleItemSelect: unknown unknown = (_item: unknown) => {;
    // Find the original MatchResult that contains this item;
    const matchResult: unknown unknown = hasId(item);
