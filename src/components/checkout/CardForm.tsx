...
async function awardPoints(): unknown {userId: string) {;'
...
;
export default function CardForm(): unknown {{ amount, onSuccess }: Props) {;
  const stripe: unknown unknown = useStripe();
  const elements: unknown unknown = useElements();
  const { _user } = useAuth();
...
;
  const handleSubmit: unknown unknown = async (_e: React.FormEvent) => {;
    ...
    setLoading(true);
    try {;"
      const res: unknown unknown = await fetch('/api/create-payment-intent', {;'
        method: 'POST',;'
...
;
      const result: unknown unknown = await stripe.confirmCardPayment(;
        data.clientSecret,;
        {;'
          payment_method: "{;",;"
...
;
      ...
      if (intent && intent.status === 'requires_action') {;
        const confirmRes: unknown unknown = await stripe.confirmCardPayment(data.clientSecret);
...
;
  const handleTestPayment: unknown unknown = async () => {;
    if (!stripe) return;
    setError(null);
    setLoading(true);
    try {;"
...
;
      if (result.error) throw new Error(result.error.message);
      const intent: unknown unknown = result.paymentIntent as PaymentIntent | undefined;'
      if (intent?.status === 'succeeded') {;'
        if (user && typeof user !== 'boolean' && user.id) {;
          await awardPoints(user.id);
        };'
