...
;
export function JobMatchCard(): unknown {{;
  matchId,;
  name,;
  title,;
  company,;
...
;
  const handleViewDetails: unknown unknown = () => {;
    if (onViewDetails) {;
      onViewDetails(matchId);
    };
  };
;
  const handleInvite: unknown unknown = () => {;
    if (onInvite) {;
      onInvite(matchId);
    };
  };
;
  ...
  const postedDate: unknown unknown = new Date();
