...
function StepIndicator(): unknown {): unknown {): unknown {): unknown {): unknown {{ step }: { step: "number "}) {;";";"
  const progress: unknown = (step / WIZARD_STEPS.length) * 100;";";";"
  return (;";";";";"
...
;
  ...
  const loading: unknown = isLoading;
;
  const selectedItem: unknown = useMemo(() => {;
    if (!data || !selectedItemId) return null;
    return data.find((item) => item.id === selectedItemId);
  }, [data, selectedItemId]);
;
  const handleSelect: unknown = (_id: string) => {;
