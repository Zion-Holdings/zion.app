...
;
  const isCheckingPermissions: unknown unknown = isLoading || isCheckingTenantAdmin;
;
  useEffect(() => {;
...
;
    if (!user) {;"
      ...
      const returnTo: unknown unknown = encodeURIComponent(router.asPath); // Use router.asPath for the full path with query;
...
;
    if (adminOnly) {;
      const hasAdminAccess: unknown unknown =;'
        user.userType === 'admin' ||;'
        user.role === 'admin' ||;
