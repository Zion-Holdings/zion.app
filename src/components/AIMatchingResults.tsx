...
;
export function AIMatchingResults(): unknown {{;
  matches,;
  onSelectMatch,;
  isLoading = false,;'
  projectDescription = '',;'
...
;
  ...
  const getCategoryIcon: unknown unknown = (_category: string) => {;
    const lowerCategory: unknown unknown = category.toLowerCase();'
...
;
        {Object.entries(categories).map(([tab, items]) => (;"
          <TabsContent key={tab} value={tab} className="mt-4 space-y-3">;
            {items.length > 0 ? (;
              items.map((match) => {;
                const CategoryIcon: unknown unknown = getCategoryIcon(match.category);
                return (;
                  <Card;
                    key={match.id};"
