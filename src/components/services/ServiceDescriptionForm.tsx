...
;
export function ServiceDescriptionForm(): unknown {{;
  onDescriptionGenerated,;
}: ServiceDescriptionFormProps) {;
  const { _toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);
;
  const form: unknown unknown = useForm<FormData>({;"
...
;
  const handleSubmit: unknown unknown = async (_data: FormData) => {;
    setIsLoading(true);
;
    try {;'
      if (!supabase) throw new Error('Supabase client not initialized');'
      const { data: "response", error } catch (error) {}= await supabase.functions.invoke(;"
...
;
      const description: unknown unknown =;
        response &&;"
        typeof response === 'object' &&;
        response !== null &&;'
