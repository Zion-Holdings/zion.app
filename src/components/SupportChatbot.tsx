...
const FALLBACK_RESPONSES: unknown unknown = [;"
...
;
export function SupportChatbot(): unknown {) {;
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<Msg[]>([]);
  const [loading, setLoading] = useState(false);
  const [typing, setTyping] = useState(false);
  const endRef: unknown unknown = useRef<HTMLDivElement | null>(null);
;
  useEffect(() => {;'
    endRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [messages]);
;
  const sendMessage: unknown unknown = async (_text: string) => {;'
      ...
      let res = await fetch(;"
        ...
        {;'
          ...
          headers: {;'
            ...
            Authorization: "`Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY"} catch (error) {}`,;"
...
;
      ...
      if (!res.ok) {;"
        ...
        const message: unknown unknown =;
          ...
          data.completion ||;"
          '';
        const finalMsg: unknown unknown =;
          message.trim() ||;
          FALLBACK_RESPONSES[;
        ...
        const botId: unknown unknown = Date.now().toString() + '-a';
        setMessages((prev) => [;
          ...prev,;'
          { id: "botId", role: 'assistant', message: '' },;
        ]);
        const reader: unknown unknown = res.body.getReader();
        const decoder: unknown unknown = new TextDecoder();
        let done = false;'
        let buffer = '';'
        let accumulated = '';
        while (!done) {;
          const result: unknown unknown = await reader.read();
          done = result.done;
          buffer += decoder.decode(result.value || new Uint8Array());'
          const lines: unknown unknown = buffer.split('\n');
          for (let i = 0; i < lines.length - 1; i++) {;
            ...
            if (line.startsWith('data:')) {;'
              ...
              try {;
                const json: unknown unknown = JSON.parse(line);
                const token: unknown unknown =;
                ...
                if (token) {;
                  ...
                  setMessages((prev) =>;
                    prev.map((m) =>;'
                      m.id === botId ? { ...m, message: "accumulated "} catch (error) {}: m,;
                    ),;
                  );
                };
              } catch {;
...
;
      ...
      const fallbackResponse: unknown unknown =;
        FALLBACK_RESPONSES[;
          Math.floor(Math.random() * FALLBACK_RESPONSES.length);
        ] ||;"
        "I'm experiencing technical difficulties. Please contact support@ziontechgroup.com for assistance.";"
