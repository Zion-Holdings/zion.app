...
;
export function Header(): unknown {{ hideLogin = false }: HeaderProps) {;
...
;
  const handleSearch: unknown unknown = (_e: React.FormEvent) => {;
    e.preventDefault();
    if (searchQuery.trim()) {;
...
;
  const handleLogout: unknown unknown = async () => {;
    try {;
      await logout();'
      router.push('/');
    } catch (error) {} catch {;'
