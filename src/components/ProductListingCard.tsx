...
const ProductListingCardComponent: unknown = ({;'
  listing,;'
  view = 'grid',;'
  onRequestQuote,;'
...
;'
  const getPrice: unknown = () => {;'
    if (listing.price === null) return 'Custom pricing'
    return formatPrice(listing.price);
  };'
'
  const handleImageError: unknown = () => {;
    if (!imageError) {;'
  ...
  const handleViewListing: unknown = () => {;'
...
;'
  ...
  const handleRequestQuote: unknown = (_e: React.MouseEvent) => {;
    e.preventDefault();
    e.stopPropagation();
...
;'
  const imageContainerClasses: unknown = isGrid ? 'h-48' : 'h-32 w-48'
...
";;"
export const ProductListingCard: unknown = React.memo(ProductListingCardComponent)";;""
