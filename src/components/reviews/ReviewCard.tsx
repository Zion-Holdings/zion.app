...
;
export function ReviewCard(): unknown {{ review, onReport }: ReviewCardProps) {;"
  const [reportReason, setReportReason] = useState('');
  const [isReporting, setIsReporting] = useState(false);
  const [isReportDialogOpen, setIsReportDialogOpen] = useState(false);
;
  const handleReport: unknown unknown = async () => {;
...
;
  const renderStars: unknown unknown = (rating?: number) => {;
