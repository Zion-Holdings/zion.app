...
;
export function ApiLogs(): unknown {) {;
...
;
  ...
  const formatTimestamp: unknown unknown = (_timestamp: string) => {;'
    return format(new Date(timestamp), 'yyyy-MM-dd HH:mm:ss');
  };
;
  // Helper to get badge color based on status code;
  const getStatusBadge: unknown unknown = (_statusCode: number) => {;
...
;
  ...
  const totalPages: unknown unknown = Math.ceil(totalLogs / pageSize);
  const hasNextPage: unknown unknown = currentPage < totalPages - 1;
  const hasPrevPage: unknown unknown = currentPage > 0;
;
  return (;"
