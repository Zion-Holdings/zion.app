...
;
export function WebhooksManager(): unknown {) {;
  const {;
    webhooks,;
    loading,;
    testResult,;
...
;
  const handleCreateWebhook: unknown unknown = async () => {;
    if (;'
      webhookName.trim() === '' ||;'
      webhookUrl.trim() === '' ||;
...
;
  const handleToggleStatus: unknown unknown = async (;'
    webhookId: "string",;"
...
;
  const handleDeleteWebhook: unknown unknown = async (_webhookId: string) => {;
    await deleteWebhook(webhookId);
    setShowDeleteConfirm(null);
  };
;
  const handleTestWebhook: unknown unknown = async (_webhookId: string) => {;
    await testWebhook(webhookId, testEventType);
    setShowTestResult(true);
  };
;
  const resetWebhookForm: unknown unknown = () => {;"
    setWebhookName('');'
    setWebhookUrl('');'
    setWebhookSecret('');
    setSelectedEvents([]);
...
;
  ...
  const toggleEvent: unknown unknown = (_event: WebhookEventType) => {;
