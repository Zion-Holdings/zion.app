...
export function ApiDocsLayout(): unknown {): unknown {): unknown {): unknown {): unknown {{ children }: ApiDocsLayoutProps) {;";"
  ...
  const currentPath: unknown = usePathname();";"
...
;'
  const handleSelectSuggestion: unknown = (_suggestion: SearchSuggestion) => {'
//     const _path: unknown = getDocsSearchPath(suggestion.text);
    if (path) {;'
      router.push(path);'
      setSearchValue('')'
    };
