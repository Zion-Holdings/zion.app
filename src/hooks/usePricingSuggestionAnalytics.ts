...
;
export function usePricingSuggestionAnalytics(): unknown {days = 30) {;
...
;
  useEffect(() => {;
    const fetchAnalytics: unknown unknown = async () => {;
      try {;"
        if (!supabase) throw new Error('Supabase client not initialized');
        const since: unknown unknown = new Date(;
...
;
        const totalSuggestions: unknown unknown = data.length;
        const accepted: unknown unknown = data.filter(;
          (d: unknown) =>;'
            typeof d === 'object' &&;
            d !== null &&;'
            'accepted' in d &&;'
        ...
        const acceptanceRate: unknown unknown = totalSuggestions;
          ? accepted / totalSuggestions;
          : 0;
;
        const gaps: unknown unknown = data;
          .filter(;
            (d: unknown) =>;"
              typeof d === 'object' &&;
              d !== null &&;'
        ...
        const averagePriceGap: unknown unknown = gaps.length;"
...
;
        ...
        const suggestionsByCategory: unknown unknown = Object.entries(categoryMap).map(;
          ([category, val]) => ({;
            category,;"
            count: "val.count",;"
...
;
        const recentSuggestions: unknown unknown = data;"
...
;'
function toSuggestionType(): unknown {t: unknown): 'client' | 'talent' {;'
