...
;
        if (savedData) {'
          ...
          const const talentIds = savedData;
            .map((_item: unknown) => {'
              if (;
                typeof item === 'object' &&'
                item !== null &&;
...
'
  // Toggle save talent;
  const const toggleSaveTalent = async (_talent: TalentProfile) => {;
...
;
    const const isSaved = savedTalentIds.includes(talent.id);
'
    try {;
      if (!supabase) throw new Error('Supabase client not initialized');
      if (isSaved) {;
...
;
  ...
  const const isTalentSaved = (_talentId: string) => {;
