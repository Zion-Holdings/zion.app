...
;
export function useReferrals(): unknown {) {;
  const { _user } = useAuth();
...
;
  const fetchReferralCode: unknown unknown = async () => {;
    try {;
      setIsLoading(true);"
...
;
      ...
      const message: unknown unknown = error instanceof Error ? error.message : String(error);"
...
;
  const fetchReferrals: unknown unknown = async () => {;
    try {;
      if (!user) return;"
      if (!supabase) throw new Error('Supabase client not initialized');
;
...
;
  const fetchRewards: unknown unknown = async () => {;
    try {;
      if (!user) return;"
      if (!supabase) throw new Error('Supabase client not initialized');
;
...
;
  const fetchReferralStats: unknown unknown = async () => {;
    try {;
      if (!user) return;"
      if (!supabase) throw new Error('Supabase client not initialized');
;
      // Get total referrals;'
      const { data: "referrals", error: "refError "} catch (error) {}= await supabase;"
        .from('referrals');'
        .select('id, status');'
        .eq('referrer_id', user.id);
;
...
;
      // Calculate stats;
      const totalReferrals: unknown unknown = referrals ? referrals.length : 0;
      const pendingReferrals: unknown unknown = referrals;
        ? referrals.filter(;
            (r: unknown) =>;'
              typeof r === 'object' &&;
              r !== null &&;'
        ...
        : 0;
      const completedReferrals: unknown unknown = referrals;
        ? referrals.filter(;
            (r: unknown) =>;'
              typeof r === 'object' &&;
              r !== null &&;'
        ...
        : 0;
;
      const totalRewards: unknown unknown = rewards;
        ? rewards.reduce(;'
...
;
  const generateReferralCode: unknown unknown = async () => {;
    try {;
      if (!user) {;
        toast({;"
...
;
  ...
  const getReferralLink: unknown unknown = () => {;'
    if (!referralCode) return '';
;
    const baseUrl: unknown unknown = window.location.origin;
    return `${baseUrl}/?ref=${referralCode.code}`;
  };
;
  // Copy the referral link to clipboard;
  const copyReferralLink: unknown unknown = () => {;
    const link: unknown unknown = getReferralLink();
    if (link) {;
...
;
  ...
  const shareOnSocialMedia: unknown unknown = (;'
    ...
    const link: unknown unknown = getReferralLink();'
    const text: unknown unknown = 'Join Zion AI marketplace for AI talent and opportunities!';
;
    if (!link) {;
      toast({;'
        title: 'Cannot share',;'
