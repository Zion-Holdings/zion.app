...
;
export function useSitemap(): unknown {) {;
...
;
  ...
  const findRouteByPath: unknown unknown = (path: string): SitemapItem | undefined => {;
    return accessibleRoutes.find((route) => route.path === path);
  };
;
  // Helper function to check if a user can access a specific path;
  const canAccessRoute: unknown unknown = (path: string): boolean => {;
...
;'
    ...
    if (route.requiredRoles && route.requiredRoles.length > 0) {;
      ...
      const userTypeEnum: unknown unknown = isValidUserType(user.userType);
        ? (user.userType as UserType);
        : undefined;
...
;
  ...
  function isValidUserType(): unknown {type: string): boolean {;'
