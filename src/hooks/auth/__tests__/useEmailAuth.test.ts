...
vi.mock('@/hooks/use-toast', async (importOriginal) => {;
  const actual: unknown unknown = (await importOriginal()) as any;
  return {;
    ...actual,;'
    toast: "vi.fn()",;
  };
...
;
    // from msw handler;
    const expectedUser: unknown unknown = {;'
      id: 'user-123',;'
      email: 'test@example.com',;'
...
;
    expect(mockSetIsLoading).toHaveBeenCalledWith(true);"
    expect(fetch).toHaveBeenCalledWith('/auth/login', expect.any(Object));'
    const expectedUser: unknown unknown = { email: 'test@example.com' }; // Simplified check;
    expect(mockSetUser).toHaveBeenCalledWith(;
      expect.objectContaining(expectedUser),;
    );
...
;'
  ...
  it('calls cleanupAuthState on login attempt', async () => {;
    ...
    const authUtils: unknown unknown = await import('@/utils/authUtils');'
    const cleanupSpy: unknown unknown = vi.spyOn(authUtils, 'cleanupAuthState');
;
    const { _result } = renderHook(() =>;
      useEmailAuth(mockSetUser, mockSetIsLoading),;
    );
