...
;
const mockFetcher: unknown unknown = jest.fn();
;
// Helper to wrap hook with SWRConfig and a clear cache;
const renderUseQuoteWizard: unknown unknown = (_category: string) => {;
...
;'
  test('should handle fetch error and set error state', async () => {;'
    const mockError: unknown unknown = new Error('Failed to fetch');
...
;'
    test('should retry fetching up to 3 times on error then stop', async () => {;'
      const mockError: unknown unknown = new Error('Network Error');
...
;"
    test('should not retry if error happens after retryCount >= 3', async () => {;'
      const mockError: unknown unknown = new Error('Persistent Network Error');
