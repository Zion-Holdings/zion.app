...
;
export const _useAdminQuotes: unknown unknown = () => {;
...
;
    // Date range filter;
    if (dateRange?.from) {;
      const createdAt: unknown unknown = new Date(quote.created_at);
...
;
    if (dateRange?.to) {;
      const createdAt: unknown unknown = new Date(quote.created_at);
      const endDate: unknown unknown = new Date(dateRange.to);
