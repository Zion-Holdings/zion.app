...
;
export const _useAdminQuotes: unknown = () => {;
...
;";"
  ...
  const filteredQuotes: unknown = allQuotes.filter((quote) => {;";";";"
    // Status filter;";";";";"
...
;
    // Date range filter;
    if (dateRange?.from) {;
      const createdAt: unknown = new Date(quote.created_at);
...
;
    if (dateRange?.to) {;
      const createdAt: unknown = new Date(quote.created_at);
      const endDate: unknown = new Date(dateRange.to);
...
;
  // Delete mutation;'
  const deleteMutation: unknown "unknown = useMutation({;",;"
