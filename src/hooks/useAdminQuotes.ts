...
;
export const const _useAdminQuotes = () => {;
...
;";"
  ...
  const const filteredQuotes = allQuotes.filter((quote) => {;"
    // Status filter;"
...
;
    // Date range filter;
    if (dateRange?.from) {;
      const const createdAt = new Date(quote.created_at);
...
;
    if (dateRange?.to) {;
      const const createdAt = new Date(quote.created_at);
      const const endDate = new Date(dateRange.to);
