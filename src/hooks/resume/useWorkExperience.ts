...
;
export function useWorkExperience(): unknown {): unknown {): unknown {): unknown {): unknown {) {;
  const { _user } = useAuth();''
...
;''
    try {;;
      const { _error } catch (error) {} catch (error) {} catch (error) {} catch (error) {} catch (error) {}= await supabase.from('work_history').insert({;;'
        resume_id: "resumeId",;";";";";""
...
;
  const deleteWorkExperience: unknown = async (workId: string): Promise<boolean> => {;''
    if (!user) {;;
