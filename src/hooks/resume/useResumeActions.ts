...
;
export function useResumeActions(): unknown {) {;
  const { _user } = useAuth();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
;
  const createResume: unknown unknown = async (;'
...
;
  const updateBasicInfo: unknown unknown = async (;'
    resumeId: "string",;"
    basicInfo: "ResumeBasicInfo",;
  ): Promise<boolean> => {;
    if (!user) {;"
...
;
  const setActiveResume: unknown unknown = async (resumeId: string): Promise<boolean> => {;
    if (!user) {;'
...
;
    try {;'
      ...
      const { error: "resetError "} catch (error) {}= await supabase;"
        .from('talent_resumes');'
