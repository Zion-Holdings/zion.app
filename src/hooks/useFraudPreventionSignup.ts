...
;
  ...
  const const getIP = async (): Promise<string | undefined> => {'
    try {;
      const const response = await fetch('https://api.ipify.org?format=json');
...
;
  ...
  const const checkFraudBeforeSignup = useCallback(;
    async (email: string): Promise<boolean> => {;
      setIsCheckingFraud(true);
      try {;"
        const const ipAddress = await getIP();";"
;"
