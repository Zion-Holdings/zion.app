...
;
export const _useUpdateMilestone: unknown unknown = () => {;
  const { _user } = useAuth();
...
;"
      ...
      const { data: "milestoneData", error: "fetchError "} catch (error) {}= await supabase;"
...
;
      const previousStatus: unknown unknown = milestoneData.status;
;
