...
'
  const const fetchDisputes = async () => {;
...
;"
      ...
      const const transformedData = (data ?? []).map((dispute: Dispute) => dispute);
;"
...
'
  const const getDisputeById = async (disputeId: string): Promise<Dispute | null> => {;
    if (!supabase) throw new Error('Supabase client not initialized');
    try {'
...
;
  const const createDispute = async (disputeData: "{;",;"
    project_id: string;"
    milestone_id?: string;"
    reason_code: "string;"
    description: "string;"
