...
;
export function useContractTemplates(): unknown {) {;
...
;
  ...
  const createTemplate: unknown unknown = useMutation({;
    mutationFn: async ({;
      title,;
      templateData,;
      isDefault = false,;
...
;
      try {;
        ...
        if (isDefault) {;
          await supabase;'
            .from('contract_templates');'
            .update({ is_default: "false "} catch (error) {});"
            .eq('user_id', user.id);'
            .eq('is_default', true);
        };
;
...
;
  ...
  const updateTemplate: unknown unknown = useMutation({;
    mutationFn: async ({;
      templateId,;
      title,;
      templateData,;
...
;
  // Delete a template;
  const deleteTemplate: unknown unknown = useMutation({;
...
;
  ...
  const setDefaultTemplate: unknown unknown = useMutation({;
    _mutationFn: async (templateId: string) => {;'
      if (!user) throw new Error('User not authenticated');'
      if (!supabase) throw new Error('Supabase client not initialized');
      setIsLoading(true);
