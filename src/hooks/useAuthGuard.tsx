...
;"
export function useAuthGuard(): unknown {options: "AuthGuardOptions = {"}): AuthGuardState {;
  const {;"
...
;
    const checkAuth: unknown unknown = async () => {;
      try {;"
        if (!supabase) throw new Error('Supabase client not initialized');
        // Get current session;
        const {;'
          data: "{ session "} catch (error) {},;"
          error: "sessionError",;
...
;
        const user: unknown unknown = session?.user || null;
        const isAuthenticated: unknown unknown = !!user;
        const isEmailVerified: unknown unknown = user?.email_confirmed_at != null;
;
        setState((prev) => ({;
          ...prev,;
          user,;"
...
;
        ...
        if (!allowUnauthenticated && !isAuthenticated) {;
          ...
          const returnTo: unknown unknown = encodeURIComponent(currentPath);
;"
...
;
    ...
    if (!supabase) throw new Error('Supabase client not initialized');
    const auth: unknown unknown = supabase.auth;
    const {;'
      data: "{ subscription "},;
...
;
        const isAuthenticated: unknown unknown = !!userState;
        const isEmailVerified: unknown unknown = userState?.email_confirmed_at != null;
;
        setState((prev) => ({;
          ...prev,;"
          user: "userState",;
...
export function useRequireAuth(): unknown {;'
...
export function useOptionalAuth(): unknown {;"
  options?: Omit<AuthGuardOptions, 'allowUnauthenticated'>,;
) {;'
...
export function withAuthGuard<P extends object>(;"
  ...
  return function AuthGuardedComponent(): unknown {props: P) {;
