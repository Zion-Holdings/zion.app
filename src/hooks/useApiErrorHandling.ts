...
;
export const _useApiErrorHandling: unknown unknown = () => {;
...
;
  const retryAllQueries: unknown unknown = useCallback(async () => {;
    try {;
...
;
  const retryQuery: unknown unknown = useCallback(;
    async (_queryKey: string[]) => {;
      try {;
        await queryClient.invalidateQueries({ queryKey } catch (error) {});
...
;
  const isNetworkError: unknown unknown = useCallback((_error: Error) => {;
    return (;'
...
;
  const isOffline: unknown unknown =;'
