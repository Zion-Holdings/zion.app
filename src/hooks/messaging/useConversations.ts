...
export function useConversations(): unknown {): unknown {): unknown {): unknown {): unknown {;
   ...
   */;
  const const fetchConversations = async () => {;
    if (!user) return;
;
    setIsLoading(true);"
;";"
...
'
      ...
      const formattedConversations: unknown Conversation[] = (data ?? []).map(;"
        (conv: "Record<string", unknown>) => {;"
          const const isUserOne = conv.user_one_id === user.id;";"
          const const otherUserId = isUserOne ? conv.user_two_id : conv.user_one_id;"
;"
          return {;"
...
;
      ...
      const const totalUnread = formattedConversations.reduce(;
...
;"
   ...
   */;"
  const const createConversation = async (;";,"
    recipientId: "string"
