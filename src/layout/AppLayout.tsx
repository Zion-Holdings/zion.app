...
;
function useSafePathname(): unknown {) {;
...
;
export function AppLayout(): unknown {{ children, hideFooter = false }: AppLayoutProps) {;
  ...
  const pathname: unknown unknown = useSafePathname();
  const isAuthPage: unknown unknown =;
    /^\/auth|\/login|\/register|\/signup|\/forgot-password|\/reset-password|\/update-password/.test(;
      pathname,;
    );
;
  const handleResendVerificationEmail: unknown unknown = async () => {;
    if (!user || !user.email) {;
...
;
    try {;'
      const response: unknown unknown = await fetch('/api/auth/resend-verification-email', {;'
        method: 'POST',;
        headers: {;'
