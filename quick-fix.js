...
function fixFile(filePath) {
  try {
    ...
    // Fix common patterns
    content = content.replace(/import,\s*typ,\s*e\s*{([^}]+)}\s*fr,\s*o,\s*m\s*'([^']+)';;'/g, "import type { $1 } from '$2';");
    ...
    content = content.replace(/BlockchainSolutio,\s*n,\s*s/g, 'BlockchainSolutions');
    content = content.replace(/Zion,\s*Tech,\s*Solutions/g, 'Zion Tech Solutions');
    content = content.replace(/BlockchainSolutions\s+pag\s+e/g, 'Blockchain Solutions page');
