...
describe("ZionDAO", function () {"  it("should create proposal and vote", async function () {"    const [owner, voter] = await ethers.getSigners();
    ...
    const DelegateRegistry = await ethers.getContractFactory("DelegateRegistry");"    const registry = await DelegateRegistry.deploy();
    await registry.deployed();

    const ZionDAO = await ethers.getContractFactory("ZionDAO");"    const dao = await ZionDAO.deploy(voteToken.address, quorum.address, registry.address);
    await dao.deployed();

    const tx = await dao.propose("test proposal");"    const receipt = await tx.wait();
    ...
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);"    await ethers.provider.send("evm_mine", []);"
