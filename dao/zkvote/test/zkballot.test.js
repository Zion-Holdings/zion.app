...
describe("ZkBallot", function () {"  it("creates proposal and accepts vote", async function () {"    const [owner] = await ethers.getSigners();
    ...
    const ZkBallot = await ethers.getContractFactory("ZkBallot");"    const ballot = await ZkBallot.deploy(1, verifier.address, token.address);
    await ballot.deployed();

    const tx = await ballot.propose("upgrade");"    const receipt = await tx.wait();
